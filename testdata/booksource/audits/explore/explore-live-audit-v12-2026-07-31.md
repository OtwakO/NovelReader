...
### 1. Rule-driven AJAX cannot run after an outer non-2xx response — raw 197
...
### 2. Missing crypto/login Java bridge APIs — raw 197
...
### 3. Default class traversal does not recognize arbitrary attribute getters — raw 163
...
### 4. Default singleton bracket indexes are unsupported — raw 17
...
### 5. Multi-match attribute extraction does not skip blank leading values — raw 17
...
- `source_incomplete_or_invalid`: raws 251, 504, 125, 910, 673, and 633. Raw 251 needs a source-declared mobile User-Agent; upstream Legado also defaults to desktop when none is declared, so automatic hostname-based switching would overfit.
- `audit_infrastructure`: raw 714; the audit resolver returned only `::`, while public DNS plus the exact rules produced 30 books

## Recommendation

Fix the five verified shared seams test-first through existing Explore workflow, Analyzer, and Java-bridge interfaces, then exact-rerun raws 197, 163, and 17. Keep source-specific stale, invalid, blocked, and upstream outcomes unchanged. After those fixes, another unrestricted sample is justified because only 171 eligible identities remain unsampled and batches 9–12 continue to expose reusable gaps.
