...
if args.phase != "initial":
    ...
    for index in range(len(manifest["selection"])):
        prior = json.loads((root / "initial" / f"{index}.json").read_text())
...
print(json.dumps({"phase": args.phase, "count": len(indices), "observations_not_final_classifications": dict(counts)}))
