...
const outputPath = '/tmp/search-bookinfo-v1-rerun.json';
...
async function invoke(args, timeout = 60_000) {
  ...
  try {
    ...
    return { durationMs: Date.now() - started, invocationError: error.killed ? 'audit command timeout' : error.message, stdout: String(error.stdout ?? '').slice(0, 2000), stderr: String(error.stderr ?? '').slice(0, 2000) };
  }
}
...
async function rerun(entry) {
  const common = ['-sources', initial.frozen.corpus.path, '-indices', String(entry.rawIndex), '-timeout', '45s'];
  ...
  return { rawIndex: entry.rawIndex, bookSourceUrl: entry.bookSourceUrl, search, selectedResult: selectedResult ?? null, detail };
}
