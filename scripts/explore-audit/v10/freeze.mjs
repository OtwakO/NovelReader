...
if (excluded.size !== 400) throw new Error(`expected 400 excluded identities, got ${excluded.size}`);
if (eligible.length !== 321) throw new Error(`expected 321 eligible identities, got ${eligible.length}`);
...
const frozen = {
  ...
  selection: {
    size,
    rationale: 'Same unrestricted 50-identity breadth as v9 because that batch exposed four shared compatibility seams.',
...
fs.writeFileSync('/tmp/explore-v10-frozen.json', `${JSON.stringify(frozen, null, 2)}\n`);
