...
const seed = 'NovelReader-explore-random-v14-2026-08-11';
const size = 50;
...
if (excluded.size !== 600) throw new Error(`expected 600 excluded identities, got ${excluded.size}`);
if (eligible.length !== 121) throw new Error(`expected 121 eligible identities, got ${eligible.length}`);
...
const frozen = {
  ...
  selection: {
    size,
    rationale: 'User requested another unrestricted random-sampling round after v13; retain the established 50-identity breadth without changing eligibility or classification rules.',
...
fs.writeFileSync('/tmp/explore-v14-frozen.json', `${JSON.stringify(frozen, null, 2)}\n`);
