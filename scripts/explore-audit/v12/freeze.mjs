...
const seed = 'NovelReader-explore-random-v12-2026-07-31';
const size = 50;
...
if (excluded.size !== 500) throw new Error(`expected 500 excluded identities, got ${excluded.size}`);
if (eligible.length !== 221) throw new Error(`expected 221 eligible identities, got ${eligible.length}`);
...
const frozen = {
  ...
  selection: {
    size,
    rationale: 'User selected another unrestricted 50-identity batch because v11 exposed four shared compatibility seams.',
...
fs.writeFileSync('/tmp/explore-v12-frozen.json', `${JSON.stringify(frozen, null, 2)}\n`);
