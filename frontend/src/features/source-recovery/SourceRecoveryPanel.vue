...
export default defineComponent({
  ...
  components: { AppDisclosure, AppButton, SearchControls, SearchStatus },
  props: {
...
<template>
  <section class="recovery" aria-labelledby="source-recovery-heading">
    <header><div><p class="eyebrow">{{ $t('sourceRecovery.eyebrow') }}</p><h2 id="source-recovery-heading">{{ $t('sourceRecovery.title') }}</h2><p>{{ $t('sourceRecovery.description') }}</p></div><div class="app-actions header-actions"><AppButton v-if="state.searching" variant="secondary" @click="controller?.stop()">{{ $t('sourceRecovery.stop') }}</AppButton><AppButton variant="quiet" :busy="rescanning" @click="confirmingRescan = true">{{ rescanning ? $t('sourceRecovery.clearing') : $t('sourceRecovery.rescan') }}</AppButton></div></header>
    ...
    <AppDisclosure class="targeted-search"><template #summary>{{ $t('sourceRecovery.targetedTitle') }}</template><p>{{ $t('sourceRecovery.targetedDescription') }}</p><form @submit.prevent="searchCurrentSource"><label><span>{{ $t('sourceRecovery.targetedLabel') }}</span><input v-model="targetedQuery" type="search" :placeholder="$t('sourceRecovery.targetedPlaceholder')" autocomplete="off"></label><AppButton type="submit" variant="secondary" :busy="targetedSearching" :disabled="!targetedQuery.trim()">{{ $t('sourceRecovery.targetedAction') }}</AppButton></form><p v-if="targetedError" class="error" role="alert">{{ targetedError }}</p><p v-else-if="targetedMessage" class="message" role="status">{{ targetedMessage }}</p></AppDisclosure>
...
<style scoped>
.recovery { display: grid; gap: .85rem; padding: 1rem; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-paper-raised); }.targeted-search>p{margin:.55rem 0;color:var(--color-ink-muted);line-height:1.5}.targeted-search form{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.65rem;align-items:end}.targeted-search label{display:grid;gap:.3rem;color:var(--color-ink-muted);font-size:var(--text-caption);font-weight:var(--weight-strong)}.targeted-search input{width:100%;min-height:2.75rem;border:1px solid var(--color-border);border-radius:var(--radius-md);padding:.55rem .7rem;background:white;color:var(--color-ink)}.source-filter{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.75rem;align-items:end}.source-filter label{display:grid;gap:.3rem;color:var(--color-ink-muted);font-size:var(--text-caption);font-weight:var(--weight-strong)}.source-filter input{width:100%;min-height:2.75rem;border:1px solid var(--color-border);border-radius:var(--radius-md);padding:.55rem .7rem;background:white;color:var(--color-ink)}.source-filter div{display:flex;gap:.35rem}.source-filter button{min-height:2.75rem;border:1px solid var(--color-border);border-radius:var(--radius-md);padding:.5rem .7rem;background:var(--color-paper);color:var(--color-ink)}.source-filter button[aria-pressed=true]{border-color:var(--color-accent);background:var(--color-accent-soft);color:var(--color-accent-strong)} header { display: flex; justify-content: space-between; gap: 1rem; } h2 { margin: .15rem 0; font: var(--weight-strong) var(--text-section) var(--font-literary); } header p { margin: .25rem 0 0; color: var(--color-ink-muted); line-height: 1.55; }.eyebrow { color: var(--color-warm); font-size: var(--text-caption); font-weight: var(--weight-strong); letter-spacing: .1em; text-transform: uppercase; }.header-actions { align-self: flex-start; justify-content: flex-end; }.header-actions :deep(.app-button) { white-space: nowrap; }.sources { list-style: none; display: grid; gap: .55rem; margin: 0; padding: 0; }.sources li { display: grid; grid-template-columns: minmax(0,1fr) auto; gap: .75rem; align-items: center; padding: .7rem; border: 1px solid var(--color-border); border-radius: var(--radius-md); }.sources li.current{border-color:color-mix(in srgb,var(--color-accent) 35%,var(--color-border))}.source-content { min-width: 0; display: grid; gap: .2rem; }.source-heading{display:flex;flex-wrap:wrap;align-items:center;gap:.45rem}.current-badge{padding:.15rem .4rem;border:1px solid var(--color-border);border-radius:var(--radius-sm);background:var(--color-paper);color:var(--color-ink-muted);font-size:var(--text-caption);font-style:normal;font-weight:var(--weight-regular);line-height:1.35}.sources strong, .sources small { overflow-wrap: anywhere; }.sources small, .empty { color: var(--color-ink-muted); }.last-chapter{color:var(--color-ink);font-weight:var(--weight-strong)}.discovery-query{color:var(--color-accent-strong);font-weight:var(--weight-strong)}.badges{display:flex;flex-wrap:wrap;gap:.25rem;margin-top:.2rem}.badges em{border-radius:999px;padding:.18rem .42rem;background:var(--color-accent-soft);color:var(--color-accent-strong);font-size:var(--text-caption);font-style:normal}.confirmation { padding: .85rem; border: 1px solid color-mix(in srgb, var(--color-danger) 45%, var(--color-border)); border-radius: var(--radius-md); background: #fff5f1; }.confirmation p { margin: .35rem 0 .7rem; color: var(--color-ink-muted); }.confirmation div { display: flex; flex-wrap: wrap; gap: .5rem; }.error { color: var(--color-danger); }.message { color: var(--color-success); }
