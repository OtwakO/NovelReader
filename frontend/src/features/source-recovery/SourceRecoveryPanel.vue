...
type KnownSource = AltSource & { current: boolean; stored: boolean };
...
export default defineComponent({
  ...
  computed: {
    ...
    sources(): KnownSource[] {
      ...
      return [...bindings.entries()].map(([key, source]) => ({ ...source, current: key === activeKey, stored: storedKeys.has(key) }));
    },
    ...
    visibleSources(): KnownSource[] {
      ...
      return this.sources.filter(source => {
        ...
        return !query || `${source.sourceName}\n${source.sourceUrl}\n${source.lastChapter || ''}`.toLocaleLowerCase().includes(query);
      });
    },
    ...
    acceptResults(items: SearchResult[], discoveryQuery = '') {
      ...
      for (const item of items) {
        ...
        for (const source of [{ sourceId: item.sourceId, sourceUrl: item.sourceUrl, bookUrl: item.bookUrl, sourceName: item.sourceName, sourceGroup: item.sourceGroup, capabilities: item.capabilities, discoveryQuery, lastChapter: item.lastChapter }, ...(item.alternateSources ?? []).map(source => ({ ...source, discoveryQuery }))]) {
          ...
          const existing = this.sources.find(item => this.key(item) === key);
...
<template>
  <section class="recovery" aria-labelledby="source-recovery-heading">
    ...
    <ul v-if="visibleSources.length" class="sources"><li v-for="source in visibleSources" :key="key(source)" :class="{ current: source.current }"><span><span class="source-heading"><strong>{{ source.sourceName || source.sourceUrl }}</strong></span><small v-if="source.lastChapter" class="last-chapter">{{ source.lastChapter }}</small><small>{{ source.sourceUrl }}</small><small v-if="source.discoveryQuery" class="discovery-query">{{ $t('sourceRecovery.discoveredByQuery', { query: source.discoveryQuery }) }}</small><span v-if="source.sourceGroup || source.capabilities?.length" class="badges"><em v-if="source.sourceGroup">{{ source.sourceGroup }}</em><em v-for="capability in source.capabilities" :key="capability">{{ $t(`sources.capabilities.${capability}`) }}</em></span></span><span v-if="source.current" class="current-label">{{ $t('sourceRecovery.currentSource') }}</span><AppButton v-else variant="secondary" :disabled="switching" @click="$emit('select', source)">{{ switching ? $t('sourceRecovery.switching') : $t('sourceRecovery.use') }}</AppButton></li></ul>
