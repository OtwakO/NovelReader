...
describe('SourceRecoveryPanel', () => {
  it('shows persisted alternate sources immediately', () => {
    const wrapper = mount(SourceRecoveryPanel, { global: { plugins: [i18n] }, props: { book: { name: 'Book', author: 'Author' }, currentSourceId: 'current', storedSources: [{ sourceId: 'stored', sourceUrl: 'stored', bookUrl: '/book', sourceName: 'Stored source', sourceGroup: 'Group A', capabilities: ['javascript', 'webview'] }], onClearAndRescan: vi.fn(async () => undefined) } });
  ...
  it('filters known sources locally without changing discovery state', async () => {
    const wrapper = mount(SourceRecoveryPanel, { global: { plugins: [i18n] }, props: { book: { name: 'Book', author: 'Author' }, currentSourceId: 'current', storedSources: [{ sourceId: 'alpha', sourceUrl: 'alpha', bookUrl: '/a', sourceName: 'Alpha' }, { sourceId: 'beta', sourceUrl: 'beta', bookUrl: '/b', sourceName: 'Beta' }], onClearAndRescan: vi.fn(async () => undefined) } });
  ...
  it('keeps an active scan when the same logical book receives refreshed props', async () => {
    ...
    const wrapper = mount(SourceRecoveryPanel, { global: { plugins: [i18n] }, props: { book: { name: 'Book', author: 'Author' }, currentSourceId: 'current', storedSources: [], onClearAndRescan: vi.fn(async () => undefined) } });
    ...
    await wrapper.setProps({ book: { name: 'Book', author: 'Author' }, currentSourceId: 'found', storedSources: [{ sourceId: 'found', sourceUrl: 'found', bookUrl: '/book', sourceName: 'Found' }, { sourceId: 'current', sourceUrl: 'current', bookUrl: '/old', sourceName: 'Previous' }] });
