...
<template>
  <section class="import-section" aria-labelledby="preview-title">
    <component :is="compact ? 'h3' : 'h2'" id="preview-title">{{ $t(compact ? 'imports.flow.preview' : 'imports.preview') }}</component>
    ...
    <div class="import-preview">
      <div class="import-chapters">
        ...
        <ol :start="start + 1" tabindex="0" :aria-label="$t(compact ? 'imports.flow.chapters' : 'imports.headings')"><li v-for="heading in preview.headings" :key="heading.index"><div class="import-heading-row"><span>{{ heading.title }} <small v-if="heading.generated && !compact">({{ $t('imports.generated') }})</small></span><slot name="heading-action" :heading="heading" /></div></li></ol>
