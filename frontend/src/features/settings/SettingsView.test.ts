...
it('puts everyday preferences first and mounts diagnostics only when opened', async () => {
  const view = mount(SettingsView, { global: { mocks: { $t: (key: string) => key, $i18n: { locale: 'en' } }, stubs: { WebViewStatusCard: { template: '<div data-diagnostics />' } } } });
  await flushPromises();
  const sections = view.findAll('.settings > section');
  expect(sections.map(section => section.classes()[1])).toEqual(['language', 'reader-defaults', 'fonts']);
