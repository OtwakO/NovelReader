...
describe('mergeSearchResults', () => {
  ...
  it('consolidates the same book and keeps the alternate source', () => { const merged = mergeSearchResults([result('a', '/a')], [result('b', '/b', '凡人修仙传', '作者：忘语')], '凡人'); expect(merged).toHaveLength(1); expect(merged[0]?.alternateSources).toEqual([{ sourceId: 'b', sourceUrl: 'b', bookUrl: '/b', sourceName: 'b', lastChapter: '' }]); });
  ...
  it.each([0, 100])('retains each binding snapshot when incoming score is %s', (score) => {
    const [merged] = mergeSearchResults(
      [result('a', '/a', undefined, undefined, { score: 60, variableMap: '{"owner":"a"}', lastChapter: 'chapter-a' })],
      [result('b', '/b', undefined, undefined, { score, variableMap: '{"owner":"b"}', lastChapter: 'chapter-b' })], '凡人');
    for (const binding of [merged!, ...merged!.alternateSources!]) {
      expect(binding.variableMap).toBe(`{"owner":"${binding.sourceId}"}`);
      expect(binding.lastChapter).toBe(`chapter-${binding.sourceId}`);
    }
  });
