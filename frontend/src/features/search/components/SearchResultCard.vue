...
.info { min-width: 0; display: grid; gap: .2rem; color: var(--color-ink-muted); font-size: .82rem; }.info strong { overflow: hidden; color: var(--color-ink); font: 700 1rem var(--font-literary); text-overflow: ellipsis; white-space: nowrap; }.chapter { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.source { color: var(--color-accent); font-size: .75rem; }.details { color: var(--color-accent); font-size: .82rem; font-weight: 700; }
@media (max-width: 35rem) { .result-card { grid-template-columns: 1fr; }.main { grid-template-columns: 3.25rem minmax(0, 1fr); }.details { display: none; }.result-card :deep(.app-button) { width: 100%; } }
</style>
