...
import { clearBookSources, getBook, getBookSource, mergeBookSources, type Book, type LibraryBook } from '../../api/books';
...
import { queueProgressWrite, setProgressVersion, waitForProgressWrites } from './progress-writer';
...
export default defineComponent({
  ...
  data() { return { chapterLoader: null as ReturnType<typeof createChapterLoader> | null, convertDisplay: markRaw(createReaderDisplayConverter()), navigating: false, refetching: false, book: null as LibraryBook | null, nativeBook: null as Book | null, chapters: [] as Chapter[], displayChapters: [] as Chapter[], content: null as ChapterContent | null, displayContent: null as ChapterContent | null, fonts: [] as Font[], fontsLoaded: false, fontsLoading: null as Promise<void> | null, conversionCapability: null as ChineseConversionCapability | null, currentIndex: 0, sourceUrl: '', contentRevision: -1, catalogRevision: 0, loading: true, error: '', catalogFailed: false, catalogRetrying: false, progressError: '', conversionError: '', sourceError: '', sourceMessage: '', switching: false, chromeVisible: true, activeSheet: '' as ''|'toc'|'settings'|'bookmarks'|'sources', preferences: loadReaderPreferences(), lastPosition: 0, restoring: false, generation: 0, conversionGeneration: 0, wakeLockController: null as ReturnType<typeof createReaderWakeLock>|null, wakeLockWarning: '', progressTimer: undefined as ReturnType<typeof setTimeout>|undefined, persistence: Promise.resolve() as Promise<void>, suppressRouteLoad: false };  },
  computed: {
  ...
  methods: {
    async load() {
      ...
      try { await this.chapterLoader?.dispose(); if(request!==this.generation)return; this.chapterLoader=null; this.convertDisplay=markRaw(createReaderDisplayConverter()); await waitForProgressWrites(this.bookId); const bookRequest=getBook(this.bookId).then(value=>({value,error:null as unknown}),error=>({value:null,error})); const chaptersRequest=waitForCatalog(this.bookId,{isCurrent:()=>request===this.generation}).then(value=>({value,error:null as unknown,catalogError:false}),error=>({value:null,error,catalogError:true})); const bookResult=await bookRequest; if(request!==this.generation)return; if(bookResult.error)throw bookResult.error; const book=bookResult.value; if(!book)throw new Error(this.$t('reader.errors.load')); this.book=book;this.catalogRevision=book.contentRevision;
        const nativeBook=book.provider==='booksource'?await getBookSource(this.bookId):null;
        if(request!==this.generation)return;
        this.nativeBook=nativeBook;this.sourceUrl=nativeBook?.sourceUrl||'';setProgressVersion(this.bookId,book.stateVersion); const chaptersResult=await chaptersRequest; if(request!==this.generation)return; if(chaptersResult.error){const error=chaptersResult.error;this.catalogFailed=chaptersResult.catalogError;throw error} const catalog=chaptersResult.value;if(!catalog)throw new Error(this.$t('reader.errors.load'));const chapters=catalog.chapters;this.catalogRevision=catalog.contentRevision; this.chapters=chapters; const index=resolveChapterIndex(chapters,this.routeChapter,book.durChapterIndex); if(index===null)throw new Error(this.$t('reader.errors.noReadable')); const position=this.routePosition ?? (index===book.durChapterIndex?book.durChapterPos:0); await this.loadContent(index,position,request); }
    ...
    async retryCatalog(){if(this.catalogRetrying)return;this.catalogRetrying=true;const request=++this.generation;this.loading=true;this.error='';this.catalogFailed=false;try{await this.chapterLoader?.dispose();if(request!==this.generation)return;this.chapterLoader=null;this.convertDisplay=markRaw(createReaderDisplayConverter());const catalog=await waitForCatalog(this.bookId,{retry:true,isCurrent:()=>request===this.generation});if(request!==this.generation)return;const chapters=catalog.chapters;this.catalogRevision=catalog.contentRevision;this.chapters=chapters;const index=resolveChapterIndex(chapters,this.routeChapter,this.book?.durChapterIndex??0);if(index===null)throw new Error(this.$t('reader.errors.noReadable'));const position=this.routePosition??(index===this.book?.durChapterIndex?this.book.durChapterPos:0);await this.loadContent(index,position,request)}catch(cause){if(request!==this.generation)return;this.catalogFailed=true;this.error=cause instanceof Error?cause.message:this.$t('reader.errors.load');this.loading=false;this.chromeVisible=true}finally{if(request===this.generation)this.catalogRetrying=false}},
    async loadContent(index:number,position:number,request?:number) {
      request ??= this.generation;
      this.chapterLoader ??= markRaw(createChapterLoader(this.bookId,this.catalogRevision));
    ...
    persistProgress(){if(!this.content||!this.book||this.contentRevision!==this.catalogRevision)return Promise.resolve();return this.queueProgress(this.currentIndex,this.lastPosition)},
    async queueProgress(index:number,position:number){try{await queueProgressWrite(this.book?.id||this.bookId,{contentRevision:this.contentRevision,chapterIndex:index,position});this.progressError=''}catch{this.progressError=this.$t('reader.errors.progress')}},
