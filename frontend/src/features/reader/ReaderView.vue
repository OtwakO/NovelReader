...
export default defineComponent({
  ...
  computed: {
    ...
    previousIndex(): number | null {
      return this.revisionConflict || this.navigation?.current.note
        ? null
        : adjacentChapterIndex(this.chapters, this.currentIndex, -1);
    },
  ...
  watch: {
    ...
    'preferences.prefetchNextChapter'(enabled: boolean) {
      if (enabled) this.prefetchNext();
    },
  ...
  methods: {
    ...
    async navigate(index: number | null, position = 0, proposal?: ReaderNavigation) {
      ...
      try {
        ...
        if (request === this.generation) this.navigating = false;
      }
    },
    ...
    prefetchNext() {
      if (
        !this.revisionConflict &&
        this.preferences.prefetchNextChapter &&
        this.content &&
        !this.error &&
        !this.loading &&
        !this.switching &&
        ...
        this.chapterLoader?.prefetch(this.nextIndex);
    },
