...
export default defineComponent({
  ...
  computed: {
    ...
    previousIndex(): number|null { return this.revisionConflict?null:adjacentChapterIndex(this.chapters,this.currentIndex,-1); },
    nextIndex(): number|null { return this.revisionConflict?null:adjacentChapterIndex(this.chapters,this.currentIndex,1); },
  ...
  methods: {
    async load(retry = false) {
      ...
      this.loading=true; this.content=null; this.displayContent=null; this.displayChapters=[]; this.navigating=false;
      ...
      try {
        ...
        this.catalogRevision=catalog.contentRevision; this.chapters=catalog.chapters;
        ...
        if(this.$route.query.contentRevision===undefined){
          ...
          await this.$router.replace(readerLocation(this.bookId,index,this.catalogRevision,position));
        }
      } catch(cause){
        if(request!==this.generation)return;
    ...
    async navigate(index:number|null,position=0) {
      if(this.revisionConflict||index===null||this.switching||this.refetching)return;
      if(this.progressTimer)clearTimeout(this.progressTimer);
      ...
      try {
        ...
        if(index!==this.routeChapter||position!==(this.routePosition??0)) {
          this.suppressRouteLoad=true;
          await this.$router.push(readerLocation(this.bookId,index,this.catalogRevision,position));
        }
      } catch(cause) {
        if(request!==this.generation)return;
        if(isReaderRevisionConflict(cause)){this.stopStaleSession();return;}
