...
import { forwardChapters } from './chapter-cache-policy';
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      navigating: false,
      requested: null as { index: number; position: number; proposal?: ReaderNavigation } | null,
      refetching: false,
      book: null as LibraryBook | null,
      nativeBook: null as Book | null,
      chapters: [] as Chapter[],
      displayChapters: [] as Chapter[],
      content: null as ReadingContent | null,
      displayContent: null as ReadingContent | null,
      fonts: [] as Font[],
      fontsLoaded: false,
      fontsLoading: null as Promise<void> | null,
      conversionCapability: null as ChineseConversionCapability | null,
      currentIndex: 0,
      sourceUrl: '',
      contentRevision: -1,
      catalogRevision: 0,
      ...
      switching: false,
      chromeVisible: true,
      activeSheet: '' as '' | 'toc' | 'settings' | 'bookmarks' | 'sources',
      preferences: loadReaderPreferences(),
      lastPosition: 0,
      restoring: false,
      generation: 0,
      conversionGeneration: 0,
      wakeLockController: null as ReturnType<typeof createReaderWakeLock> | null,
      wakeLockWarning: '',
      progressTimer: undefined as ReturnType<typeof setTimeout> | undefined,
  ...
  computed: {
    ...
    routeChapter(): number | undefined {
      const value = Number(this.$route.params.chapterIndex);
      return Number.isInteger(value) && value >= 0 ? value : undefined;
    },
    routePosition(): number | undefined {
      const value = Number(this.$route.query.position);
      return Number.isFinite(value) ? clampProgress(value) : undefined;
    },
    previousIndex(): number | null {
      return this.revisionConflict || this.navigation?.current.note
        ? null
        : adjacentChapterIndex(this.chapters, this.requested?.index ?? this.currentIndex, -1);
    },
    ...
    routeAnchor(): string | undefined {
      return typeof this.$route.query.anchor === 'string' ? this.$route.query.anchor : undefined;
    },
    mainSections(): Chapter[] {
      return this.chapters.filter(isMainChapter);
    },
    readingNote(): boolean {
      return Boolean(
        this.navigation?.current.note ||
          this.chapters.find((chapter) => chapter.index === this.currentIndex)?.auxiliary,
      );
    },
    readerCatalog() {
      return { chapters: this.chapters, contentRevision: this.catalogRevision };
    },
    recordsProgress(): boolean {
      return (
        !this.revisionConflict &&
        Boolean(this.navigation && canRecordReaderProgress(this.readerCatalog, this.navigation))
      );
    },
    currentTitle(): string {
      if (this.requested) return (this.displayChapters.length ? this.displayChapters : this.chapters).find(chapter => chapter.index === this.requested!.index)?.title ?? '';
      return (
        this.displayContent?.document.title ||
        this.displayChapters.find((chapter) => chapter.index === this.currentIndex)?.title ||
        ''
      );
    },
    fontFamily(): string {
      return this.preferences.fontId === 'system'
        ? 'var(--font-literary)'
        : `reader-font-${this.preferences.fontId}, var(--font-literary)`;
    },
    readerStyle(): Record<string, string> {
      return {
        '--reader-bg': this.preferences.background,
        '--reader-text': this.preferences.textColor,
        '--reader-size': `${this.preferences.fontSize}px`,
        '--reader-line': String(this.preferences.lineHeight),
        '--reader-weight': String(this.preferences.fontWeight),
        '--reader-width': `${this.preferences.pageWidth}px`,
        '--reader-font': this.fontFamily,
      };
    },
  ...
  watch: {
    ...
    preferences: {
      deep: true,
      handler(value: ReaderPreferences) {
        saveReaderPreferences(value);
      },
    },
    'preferences.chineseConversion'() {
      this.prefetchNext();
      void this.changeConversion();
    },
    'preferences.prefetchNextChapter'() {
      this.prefetchNext();
    },
    'preferences.keepScreenAwake'() {
      this.wakeLockWarning = '';
      void this.wakeLockController?.sync();
    },
  },
  async mounted() {
    ...
    this.wakeLockController = createReaderWakeLock(
      ...
      () => {
        this.wakeLockWarning = this.$t('reader.settings.wakeLockUnavailable');
      },
    );
  ...
  methods: {
    async load(retry = false) {
      ...
      this.displayNavigation = undefined;
      ...
      try {
        ...
        const position =
          this.routePosition ?? (index === book.durChapterIndex ? book.durChapterPos : 0);
        const initial = createReaderNavigation(this.readerCatalog, index, position);
        await this.loadContent(index, position, request, {
          ...initial,
          current: {
            ...initial.current,
            note: this.$route.query.note === '1',
            anchor: this.routeAnchor,
          },
        });
        if (request !== this.generation) return;
        if (this.$route.query.contentRevision === undefined) {
          ...
          await this.$router.replace(
            readerLocation(this.bookId, index, this.catalogRevision, position, initial.current),
          );
        }
      } catch (cause) {
        if (request !== this.generation) return;
        if (isReaderRevisionConflict(cause)) {
    ...
    async revalidateVisibleReader() {
      this.chapterLoader?.prepare([], async () => undefined);
    ...
    async reopenCurrent() {
      ...
      if (this.$route.params.chapterIndex === undefined && !Object.keys(this.$route.query).length)
        await this.load();
    ...
    async loadContent(
      ...
      do {
        ...
        try {
          display = await this.convertDisplay(this.chapters, content, mode, this.catalogNavigation);
          conversionError = '';
        } catch {
          display = { chapters: this.chapters, content, navigation: this.catalogNavigation };
          conversionError = this.$t('reader.settings.conversionFailed');
        }
      ...
      const previous = {
        ...
        displayChapters: this.displayChapters,
      ...
      this.displayNavigation = display.navigation;
      ...
      if (this.book && this.recordsProgress) {
        this.book.durChapterIndex = index;
        this.book.durChapterPos = position;
      }
      this.prefetchNext();
      document.title = `${this.displayContent?.document.title || content.document.title} · ${this.book?.name || 'NovelReader'}`;
    },
    async changeConversion() {
      if (this.navigating || this.switching || this.refetching) return;
      const request = this.generation;
      const host = this.$refs.scrollHost as HTMLElement | undefined;
      const position = host
        ? normalizedScroll(host.scrollTop, host.scrollHeight, host.clientHeight)
        : this.lastPosition;
      await this.refreshDisplay();
      await nextTick();
      if (request !== this.generation || !host) return;
      this.restoring = true;
      host.scrollTop = scrollTopForProgress(position, host.scrollHeight, host.clientHeight);
      this.lastPosition = position;
    ...
    async refreshDisplay() {
      const request = ++this.conversionGeneration;
      try {
        const display = await this.convertDisplay(
          this.chapters,
          this.content,
          this.preferences.chineseConversion,
          this.catalogNavigation,
        );
        if (request !== this.conversionGeneration) return;
        this.displayChapters = display.chapters;
        this.displayContent = display.content;
        this.displayNavigation = display.navigation;
        this.conversionError = '';
        if (display.content)
          document.title = `${display.content.document.title} · ${this.book?.name || 'NovelReader'}`;
      } catch {
        if (request !== this.conversionGeneration) return;
        this.displayChapters = this.chapters;
        this.displayContent = this.content;
        this.displayNavigation = this.catalogNavigation;
        this.conversionError = this.$t('reader.settings.conversionFailed');
      }
    },
    ...
    onScroll() {
      if (
        ...
        return;
      const host = this.$refs.scrollHost as HTMLElement;
      this.lastPosition = normalizedScroll(host.scrollTop, host.scrollHeight, host.clientHeight);
      if (this.progressTimer) clearTimeout(this.progressTimer);
      this.progressTimer = setTimeout(() => void this.persistProgress(), 600);
    },
    onKeydown(event: KeyboardEvent) {
      const action = readerKeyboardAction(
        event,
        Boolean(this.activeSheet),
      ...
      if (action === 'escape') {
        ...
        void this.$router.push(`/books/${encodeURIComponent(this.bookId)}`);
        return;
      }
      if (action === 'previous') {
        void this.navigate(this.previousIndex);
        return;
      }
      if (action === 'next') {
        void this.navigate(this.nextIndex);
        return;
      }
      const host = this.$refs.scrollHost as HTMLElement | undefined;
      if (!host) return;
      ...
      host.scrollBy({
        top: (action === 'page-up' ? -1 : 1) * host.clientHeight * 0.82,
        behavior: 'smooth',
      });
    },
    onReaderTap(event: MouseEvent) {
      if (this.loading || this.requested || !this.content || this.activeSheet) return;
      const host = this.$refs.scrollHost as HTMLElement | undefined;
      if (!host) return;
      ...
      if (isAtScrollBoundary(direction, host.scrollTop, host.scrollHeight, host.clientHeight)) {
        void this.navigate(
          direction < 0 ? this.previousIndex : this.nextIndex,
          direction < 0 ? 1 : 0,
        );
        return;
      }
      host.scrollBy({ top: direction * host.clientHeight * 0.82, behavior: 'smooth' });
    },
    persistProgress() {
      if (
        this.restoring ||
        ...
        !this.content ||
        !this.book ||
        this.contentRevision !== this.catalogRevision
      )
        return Promise.resolve();
      return this.queueProgress(this.currentIndex, this.lastPosition);
    },
    async queueProgress(index: number, position: number) {
      if (!this.recordsProgress) return;
      const bookId = this.book?.id || this.bookId;
      const contentRevision = this.contentRevision;
      try {
        await queueProgressWrite(bookId, { contentRevision, chapterIndex: index, position });
        if (bookId === this.bookId && contentRevision === this.catalogRevision)
          this.progressError = '';
      } catch (cause) {
        if (bookId !== this.bookId || contentRevision !== this.catalogRevision) return;
        if (isReaderRevisionConflict(cause)) this.stopStaleSession();
        else this.progressError = this.$t('reader.errors.progress');
      }
    },
    async navigate(index: number | null, position = 0, proposal?: ReaderNavigation) {
      ...
      this.requested = { index, position, proposal };
      this.chapterLoader?.prepare([], async () => undefined);
      this.chromeVisible = true;
      this.error = '';
      try {
        ...
        if (
          index !== this.routeChapter ||
          this.lastPosition !== (this.routePosition ?? 0) ||
          proposal?.current.anchor !== this.routeAnchor ||
          this.$route.query.note !== undefined
        ) {
          this.suppressRouteLoad = true;
        ...
        this.error = cause instanceof Error ? cause.message : this.$t('reader.errors.load');
        this.chromeVisible = true;
        if (proposal) this.requested = null; // Failed anchors/notes restore the committed location.
      } finally {
        if (request === this.generation) { this.navigating = false; this.prefetchNext(); }
      }
    },
    async returnFromNote() {
      ...
      if (proposal)
        await this.navigate(proposal.current.chapterIndex, proposal.current.position, proposal);
    },
    targetHref(target: ReadingTarget, note: boolean): string {
      return this.$router.resolve(
        readerLocation(this.bookId, target.chapterIndex, target.contentRevision, undefined, {
    ...
    async followTarget(target: ReadingTarget, note: boolean) {
      if (!this.navigation || this.restoring || this.revisionConflict || this.navigating) return;
      try {
        const proposal = followReaderTarget(
          this.readerCatalog,
          this.navigation,
          target,
          this.lastPosition,
    ...
    prefetchNext() {
      const enabled = !document.hidden && !this.revisionConflict && this.preferences.prefetchNextChapter && this.content && !this.error && !this.loading && !this.navigating && !this.switching && !this.refetching && !this.readingNote;
      const targets = enabled ? forwardChapters(this.chapters, this.currentIndex) : [];
      const mode = this.preferences.chineseConversion;
      const convert = this.convertDisplay;
      const chapters = this.chapters;
      const navigation = this.catalogNavigation;
      this.chapterLoader?.prepare(targets, content => convert(chapters, content, mode, navigation));
    },
    retryDestination() {
      const target = this.requested;
      return target ? this.navigate(target.index, target.position, target.proposal) : this.load();
    },
    async refetchChapter() {
      ...
      try {
        ...
        if (request === this.generation) this.prefetchNext();
      }
    },
    async captureBookmark() {
      ...
      if (
        ...
        throw new Error(this.$t('reader.errors.position'));
      this.lastPosition = normalizedScroll(host.scrollTop, host.scrollHeight, host.clientHeight);
      if (this.progressTimer) clearTimeout(this.progressTimer);
      const location = {
        contentRevision: this.contentRevision,
    ...
    loadFonts() {
      ...
      this.fontsLoading = (async () => {
        try {
          this.fonts = await listFonts();
          this.fontsLoaded = true;
          for (const font of this.fonts) {
            const family = `reader-font-${font.id}`;
            const face = new FontFace(family, `url(${getFontUrl(font.id)})`);
            void face
              .load()
    ...
    persistMatches(sources: AltSource[]) {
      ...
      this.persistence = this.persistence
        ...
        .catch((cause) => {
          this.sourceError =
            cause instanceof Error ? cause.message : this.$t('sourceRecovery.persistFailed');
        });
    },
    async clearAndRescan() {
      try {
        await this.persistence;
        if (!this.book) throw new Error(this.$t('reader.errors.load'));
        this.book = this.nativeBook = await clearBookSources(this.book.id);
        this.sourceMessage = this.$t('sourceRecovery.cleared');
        this.sourceError = '';
      } catch (cause) {
        this.sourceError =
          cause instanceof Error ? cause.message : this.$t('sourceRecovery.clearFailed');
        throw cause;
      }
    },
    async selectSource(source: AltSource) {
      ...
      try {
        ...
        this.sourceUrl = result.book.sourceUrl;
        ...
        const mapped = resolveChapterIndex(
          chapters,
          result.book.durChapterIndex,
          result.book.durChapterIndex,
        );
        ...
        if (request !== this.generation) return;
        if (
          mapped !== this.routeChapter ||
          String(this.catalogRevision) !== this.$route.query.contentRevision ||
          (result.book.durChapterPos || 0) !== (this.routePosition ?? 0)
        ) {
          this.suppressRouteLoad = true;
          await this.$router.replace(
            readerLocation(
        ...
        const message =
          cause instanceof Error ? cause.message : this.$t('sourceRecovery.switchFailed');
        this.sourceError = switched
          ? this.$t('reader.errors.switchedReload', { message })
          : message;
...
<template>
  <section class="reader" :style="readerStyle">
    <header v-show="chromeVisible" class="reader-header">
      <RouterLink :to="`/books/${encodeURIComponent(bookId)}`" class="back">
        {{ $t('reader.back') }}
      </RouterLink>
      <div>
        <strong>{{ book?.name || $t('reader.title') }}</strong>
        <span>{{ currentTitle }}</span>
      </div>
      <div class="app-actions reader-header-actions">
        <span v-if="content && !revisionConflict" class="chapter-position">
          <template v-if="readingNote">{{ $t('reader.note') }}</template>
          <template v-else>
            {{
              mainSections.findIndex(
                chapter => chapter.index === (requested?.index ?? currentIndex),
              ) + 1
            }}
            / {{ mainSections.length }}
          </template>
        ...
        <ReaderActionsMenu
          ...
          :refresh-disabled="!content || !!requested || navigating || switching"
          :refreshing="refetching"
          @bookmarks="activeSheet = 'bookmarks'"
    ...
    <main
      ...
      class="reader-scroll"
      tabindex="0"
      :aria-label="$t('reader.readingArea')"
      @scroll.passive="onScroll"
      @click="onReaderTap"
    >
      <article class="prose">
        <p
          v-if="(navigating && requested) || (loading && !content)"
          class="state"
          role="status"
          aria-busy="true"
        >
          {{ currentTitle }} — {{ $t('reader.loading') }}
        </p>
        <section v-else-if="error && (!content || requested)" class="failure">
          <h1>
            {{ currentTitle || book?.name || $t('reader.errors.chapter') }}
          </h1>
          <p role="alert">{{ error }}</p>
          <div class="failure-actions">
            <AppButton v-if="revisionConflict" @click="reopenCurrent">
              {{ $t('reader.reopenCurrent') }}
            </AppButton>
            <AppButton
              ...
              @click="retryCatalog"
            >
              {{ $t('reader.retryCatalog') }}
            </AppButton>
            <AppButton v-else :busy="loading" @click="retryDestination">
              {{ $t('app.common.retry') }}
            </AppButton>
            <AppButton
              v-if="book?.provider === 'booksource'"
              variant="secondary"
              @click="activeSheet = 'sources'"
            >
              {{ $t('reader.recover') }}
            </AppButton>
          </div>
        </section>
        <template v-else-if="displayContent">
          <aside v-if="error" class="content-failure" role="alert">
            <span>{{ error }}</span>
            <AppButton v-if="revisionConflict" @click="reopenCurrent">
              {{ $t('reader.reopenCurrent') }}
            </AppButton>
          ...
          <p v-if="refetching" class="conversion-warning" role="status">
            {{ $t('reader.actions.refetching') }}
          </p>
          <p v-if="conversionError" class="conversion-warning" role="status">
            {{ conversionError }}
          </p>
          <p v-if="wakeLockWarning" class="conversion-warning" role="status">
            {{ wakeLockWarning }}
          </p>
          <ProseRenderer
            ref="proseRenderer"
            :target-href="targetHref"
            :document="displayContent.document"
            :fallback-image-alt="
              $t('reader.imageAlt', { title: displayContent.document.title })
            "
            :image-unavailable="$t('reader.imageUnavailable')"
            :cover-unavailable="$t('reader.coverUnavailable')"
            :show-images="preferences.showImages"
            @navigate="followTarget($event.target, $event.note)"
          />
          <p class="chapter-end">{{ $t('reader.chapterEnd') }}</p>
        </template>
      </article>
    </main>
    <p v-if="progressError" class="progress-error" role="status">
      {{ progressError }}
    </p>
    <footer v-show="chromeVisible" class="reader-controls">
      <div class="reader-controls-inner">
        <button
          v-if="navigation?.returns.length"
          :disabled="revisionConflict || navigating || refetching || switching"
          @click="returnFromNote"
        >
          <AppIcon name="previous" />
          <span>{{ $t('reader.returnFromNote') }}</span>
        </button>
        <button
          v-else
        ...
        <button :disabled="revisionConflict" @click="activeSheet = 'toc'">
          <AppIcon name="toc" />
          <span>{{ $t('reader.toc.title') }}</span>
        </button>
        <button
          v-if="book?.provider === 'booksource'"
          :class="{ attention: recoveryNeeded }"
          @click="activeSheet = 'sources'"
        >
          <AppIcon name="source" />
          <span>{{ $t('reader.sources.short') }}</span>
        </button>
        <button @click="openSettings">
          <span class="reader-aa" aria-hidden="true">Aa</span>
          <span>{{ $t('reader.settings.title') }}</span>
        </button>
        <button :disabled="nextIndex === null" @click="navigate(nextIndex)">
          <AppIcon name="next" />
    ...
    <ReaderSettingsSheet
      v-if="activeSheet === 'settings'"
      v-model="preferences"
      :fonts="fonts"
      :conversion-capability="conversionCapability"
      @close="activeSheet = ''"
    />
    <ReaderTocSheet
      v-if="activeSheet === 'toc'"
      :navigation="displayNavigation"
      :current-anchor="navigation?.current.anchor"
      :chapters="displayChapters"
      :current-index="currentIndex"
      @open-target="followTarget($event, false)"
      @open="navigate"
      @close="activeSheet = ''"
    />
    <ReaderBookmarksSheet
      v-if="activeSheet === 'bookmarks' && book"
      :key="book.id"
      :current-revision="catalogRevision"
      :book-id="book.id"
      :capture="captureBookmark"
      @open="openBookmark"
      @stale="stopStaleSession"
      @close="activeSheet = ''"
    />
    <ReaderSourceSheet
...
.back {
  ...
  border: 1px solid color-mix(in srgb, var(--reader-text) 28%, transparent);
  border-radius: var(--radius-md);
  padding: 0.5rem 0.8rem;
  background: color-mix(in srgb, var(--reader-bg) 88%, var(--reader-text) 4%);
...
.back:hover,
.back:focus-visible {
  border-color: color-mix(in srgb, var(--reader-text) 48%, transparent);
  background: color-mix(in srgb, var(--reader-bg) 78%, var(--reader-text) 8%);
  box-shadow: 0 0.35rem 0.7rem
    color-mix(in srgb, var(--reader-text) 12%, transparent);
}
...
.reader-header-actions button:focus-visible {
...
.prose {
  ...
  padding: max(
      ...
      calc(4.85rem + env(safe-area-inset-top))
    )
    0 max(7rem, calc(5rem + env(safe-area-inset-bottom)));
  font: var(--reader-weight) var(--reader-size) / var(--reader-line)
    var(--reader-font);
...
.content-failure {
  ...
  border-radius: var(--radius-md);
  background: color-mix(in srgb, var(--color-danger) 8%, var(--reader-bg));
  color: var(--color-danger);
  font: var(--weight-strong) var(--text-small) var(--font-ui);
}
.chapter-end {
...
.conversion-warning {
  ...
  border-radius: var(--radius-md);
  background: color-mix(in srgb, var(--reader-bg) 84%, #b37a2d 16%);
  font: var(--weight-strong) var(--text-small) var(--font-ui);
...
.progress-error {
  ...
  color: white;
  font: var(--weight-strong) var(--text-caption) var(--font-ui);
}
.reader-controls {
  position: absolute;
  z-index: 20;
  ...
  border-top: 1px solid color-mix(in srgb, var(--reader-text) 14%, transparent);
  background: color-mix(in srgb, var(--reader-bg) 97%, transparent);
...
.reader-controls button {
  ...
  background: transparent;
  color: inherit;
  font: var(--weight-strong) var(--text-caption) / 1.15 var(--font-ui);
...
.reader-controls button > span:last-child {
  max-width: 100%;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
...
.reader-aa {
  ...
  display: grid;
  place-items: center;
  font: var(--weight-strong) var(--text-small) / 1 var(--font-literary);
  letter-spacing: -0.04em;
}
@media (max-width: 36rem) {
  .prose {
    width: min(var(--reader-width), calc(100% - 2rem));
    padding-top: max(4.85rem, calc(4.35rem + env(safe-area-inset-top)));
  }
  .reader-controls {
    padding-inline: 0.35rem;
  }
