...
<template><div class="overlay" @click.self="$emit('close')" @keydown.esc="$emit('close')"><section class="sheet" role="dialog" aria-modal="true" :aria-label="$t('reader.bookmarks.title')"><header><h2>{{ $t('reader.bookmarks.title') }}</h2><AppButton variant="quiet" @click="$emit('close')">{{ $t('reader.close') }}</AppButton></header><form @submit.prevent="submit"><textarea v-model="note" maxlength="1000" :placeholder="$t('reader.bookmarks.note')" /><AppButton type="submit" :busy="busy"><AppIcon name="bookmark" />{{ $t('reader.bookmarks.add') }}</AppButton></form><p v-if="error" class="error" role="status">{{ error }}</p><p v-if="loading">{{ $t('reader.bookmarks.loading') }}</p><p v-else-if="!bookmarks.length">{{ $t('reader.bookmarks.empty') }}</p><ul v-else><li v-for="mark in bookmarks" :key="mark.id" :class="{orphaned:mark.orphaned}"><div><strong>{{ mark.chapterTitle || mark.chapterIndex + 1 }}</strong><small>{{ $t('reader.bookmarks.position',{percent:Math.round(mark.position*100)}) }}<template v-if="mark.orphaned"> · {{ $t('reader.bookmarks.orphaned') }}</template></small><p v-if="mark.note">{{ mark.note }}</p></div><div class="actions"><AppButton variant="secondary" :disabled="mark.orphaned" @click="$emit('open',mark.chapterIndex,mark.position,mark.contentRevision)">{{ $t('reader.bookmarks.open') }}</AppButton><AppButton variant="danger" @click="remove(mark)">{{ $t('reader.bookmarks.delete') }}</AppButton></div></li></ul></section></div></template>
