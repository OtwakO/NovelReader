...
describe('reader preferences', () => {
  ...
  it('persists and bounds reader settings', () => { saveReaderPreferences({ ...defaultReaderPreferences, fontSize: 25, pageWidth: 900, chineseConversion: 'traditional', keepScreenAwake: true }); expect(loadReaderPreferences()).toMatchObject({ fontSize: 25, pageWidth: 900, chineseConversion: 'traditional', keepScreenAwake: true }); localStorage.setItem('novelreader.reader.preferences.v1', JSON.stringify({ fontSize: 99, lineHeight: 0, chineseConversion: 'invalid' })); expect(loadReaderPreferences()).toMatchObject({ fontSize: 32, lineHeight: 1.3, chineseConversion: 'original' }); });
});
