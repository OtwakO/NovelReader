...
<template>
  <div class="reader-actions" @keydown="onKeydown" @focusout="onFocusOut">
    ...
    <div v-if="open" :id="menuId" class="actions-panel" role="menu" :aria-label="$t('reader.actions.title')">
      <button type="button" role="menuitem" @click="choose('bookmarks')">{{ $t('reader.bookmarks.title') }}</button>
      <button type="button" role="menuitem" :disabled="refreshDisabled || refreshing" @click="choose('refetch')">{{ $t(refreshing ? 'reader.actions.refetching' : 'reader.actions.refetch') }}</button>
    </div>
  </div>
</template>
...
.actions-panel button{min-height:2.75rem;padding:.65rem .8rem;border:0;border-radius:var(--radius-md);background:transparent;color:inherit;font:inherit;text-align:start;cursor:pointer}
