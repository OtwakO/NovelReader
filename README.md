...
- **Chapter caching** — BookSource reads reuse qualified backend copies for 24 hours from retrieval, without contacting the upstream site. Missing/expired copies fetch upstream; expired copies are never served as outage fallback. TXT/EPUB validity follows the imported book's content revision. Browser memory and IndexedDB retain up to three recently read books and five chapters per book; reopening validates current book/catalog identity online before reuse. Browser storage failure falls back to memory/network, not a reading failure. This is not offline/PWA chapter downloading.
