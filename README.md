...
On Windows, double-click `run-local.bat` (or run it from Command Prompt) to install dependencies, build the frontend, and start the server. The launcher uses `backend\data` as the current per-reader data root, generates and prints a temporary first-Administrator setup token on each run (the server ignores it after setup closes), and enables ordinary-reader registration for local testing unless `REGISTRATION_ENABLED` was set explicitly.
