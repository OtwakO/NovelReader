...
### Pre-public development schema policy

NovelReader is in rapid, pre-public development. All local accounts, reader homes, databases, and other user data created during this stage are disposable test data unless the user explicitly says otherwise.

- Change the canonical fresh schema directly when the data model changes.
- Delete and recreate the affected development database, reader home, or complete data root instead of preserving disposable state.
- Do not add schema-upgrade migrations, compatibility adapters, data backfills, backup/rollback workflows, or migration-specific tests for development data.
- Keep tests for the current clean schema and current behavior; remove tests and code that exist only to upgrade an older development schema.
- Existing initialization seams may create the current schema for a fresh database, but they must not accumulate historical upgrade steps during rapid iteration.
- Migration work becomes justified only after NovelReader has public users or irreplaceable data, or when the user explicitly approves it for a specific core architectural boundary.

When a schema change encounters incompatible local data, stop the server, remove the disposable data, and verify clean initialization. Do not preserve it by default or design migration machinery around it.

### Authentication prerequisite (accepted design)

- `CONTEXT.md` defines Reader Account, Reader Data, Application Session, Source Session, Source Login State, Administrator, invite code, and reset-token language.
- `docs/adr/0001-user-owned-data-and-local-authentication.md` records the hard-to-reverse identity and ownership decision.
- `docs/AUTHENTICATION_DESIGN.md` is the implementation contract: `system.db` for identity; one self-contained plaintext `reader.db` plus ordinary files per immutable user ID; a separate encrypted source-credential store; a deep `readerstore` module that owns paths, database lifecycle, transactional fresh-schema initialization, exact epoch-and-catalog refusal, export, and import; Argon2id admission controls; persistent independently revocable cookie sessions; setup/recovery tokens; request-context identity; same-origin unsafe methods; and fail-closed ownership conversion.
- `docs/USER_STORAGE_IMPLEMENTATION_TASKS.md` is the execution checklist. It breaks the accepted design into small completion-gated tasks, records exclusions, and is the source of truth for implementation progress. Phase 1.2 is complete: `readerstore` validates canonical UUID v4 user IDs, stages portable reader homes, owns schema-versioned plaintext databases and safe file paths, and bounds cached database handles.
- Stopped-server copy of the complete `data/` directory is a supported backup. Portable per-reader exports exclude credentials and authentication authority. Losing administrator access or the credential key must not make Reader Data unreadable. Exactly one NovelReader server process may own a writable `DATA_DIR`; shared writable multi-process access is unsupported because database transactions cannot serialize adjacent reader-home filesystem workflows.
- Storage foundation comes first, but authentication and conversion of every global Reader Data route ship as one atomic release boundary. This is a clean pre-release replacement: do not dual-write, preserve global-store fallbacks, or add compatibility adapters. Delete superseded schemas, startup wiring, configuration, paths, modules, and legacy-only tests as their replacements become complete. Do not add globally keyed login state, expose authenticated-looking routes over global stores, accept client-supplied user IDs as authorization, or hardcode reader-directory paths outside `readerstore`.

### Directory layout

The feature-oriented layout remains canonical. The production frontend is a Vue 3 application; backend BookSource execution remains framework-independent and server-owned.

```text
backend/internal/
  api/          HTTP/SSE transport only
  booksource/   canonical source model, import/export, persistence
  sourceexec/   unified URL/request/session/transport execution
...
frontend/src/
  api/          typed HTTP/SSE transport and backend DTOs only
  app/          Vue bootstrap, Router topology, route policy, and application shells
  stores/       narrowly shared Pinia state (session and durable UI preferences)
  ui/           semantic tokens, icons, and reusable interaction primitives
  features/     feature-owned pages, workflows, view models, and internal UI
...
testdata/booksource/
  conformance/{core,explore}/   deterministic offline compatibility fixtures by operation
  audits/{explore,search-bookinfo}/ dated live evidence and targeted reruns by operation
```

New directories require a PLAN update before creation. Frontend dependencies flow from route/page UI through feature modules to the typed `api` transport seam; frontend modules never construct BookSource requests or interpret crawling rules. Vue pages use the Options API consistently, while complex reusable workflows live in typed feature modules or composables behind small interfaces. Pinia is reserved for genuinely cross-route state rather than mirroring every backend response. `frontend/package-lock.json` remains tracked for reproducible `npm ci`; generated `frontend/dist/` assets are ignored and must not be committed.
...
### Phase 5.7 — Canonical book detail and reusable source discovery

**Goal:** separate inspecting a discovered book from persisting it, make book detail the canonical decision surface, render source descriptions safely as readable prose, and give detail/reader source discovery the same bounded batch controls as main search.

Accepted behavior:

- Clicking a Search result opens a tab-scoped, unshelved preview. Opening or reloading that preview fetches source Book Info and TOC without creating book/chapter rows.
- Preview actions are explicit: **Add to shelf** persists and stays on detail; **Read** proves a readable chapter before persistence, then opens the canonical stored reader. Failed validation creates no shelf row.
- Stored shelf cards continue to open `#/book?id=...`; their separate Continue action may open the Reader directly.
- Source-provided descriptions remain untrusted. The backend decodes entities, converts HTML block/line boundaries to newlines, strips all tags and attributes, and returns plain prose. The frontend never injects source HTML.
- Alternate-source discovery reuses one controller for preferences, SSE lifecycle, cursor/retry/restart, progress, and Search more. Book detail embeds an inline shell; Reader uses a compact panel. Logical-book filtering, source merging, progress flushing, and explicit switching remain context-owned.

Implementation sequence:

- [x] Add deterministic description-normalization tests and apply the shared normalizer to Search, Book Info, and persistence boundaries.
- [x] Add a strict non-persistent preview API returning enriched metadata plus TOC, with tests proving no book/chapter writes.
- [x] Add validate-before-persist Read orchestration with tests proving failures leave no shelf row and logical-book merges preserve canonical reading state.
- [x] Add a separate shelf operation that persists Book Info + TOC without requiring chapter-content availability.
- [x] Add tab-scoped preview routing/state and split Search-result Open from Add actions.
- [x] Extract and test the alternate-source batch controller; route both stored detail and Reader source discovery through it.
- [x] Verify focused/full backend and frontend tests, production build, and deterministic real-browser Search → preview → shelf → detail → Reader source-discovery flow.

Completion gate passed: preview is non-persistent; descriptions contain no source-controlled markup while preserving prose breaks; Read persists only after successful content validation; Add to shelf persists detail + TOC independently; shelf cards open canonical detail; and source discovery exposes bounded batch/concurrency controls plus Stop/Retry/Search more in both detail and Reader without corrupting reading progress. Exact matches found for shelved books are persisted immediately and survive detail/Reader reloads; **Rescan sources** explicitly clears stored alternates and restarts at the first batch. Deterministic Playwright verification rendered real chapter content from a local imported BookSource; live multi-source transport switching remains covered by the normal compatibility verification loop.
...
#### Third live compatibility audit (2026-07-19)

Run a deterministic random 50-source sample of unique Explore-capable URLs, excluding every raw index from the first two audits. Rank the remaining candidates by SHA-256 of seed `NovelReader-explore-audit-v3-2026-07-19`, raw index, and source URL. Execute the first selectable category/page in a fresh isolated database, then independently reproduce every empty/error and suspiciously small result with Playwright direct requests and targeted DOM selector counts.
...
**Vue frontend foundation (2026-08-13):** The active production frontend is now a clean Vue 3 SPA foundation with no Svelte runtime or mixed-framework mounting. Vue Router owns hash-based public/authenticated routes and administrator route policy; Pinia initially owns only session/bootstrap state; the typed `frontend/src/api/client.ts` remains the single backend transport seam. Components use the Options API by convention, with ESLint enforcing component organization. The shared Chinese-parchment tokens, responsive public shell, authenticated desktop-secondary/mobile-hub navigation, setup/login/registration/password-reset/recovery forms, and a backend-backed Shelf page are established. Vue I18n lazy-loads feature-owned English, Simplified Chinese, and Traditional Chinese message modules; first visit follows supported browser preferences with English fallback, and the stored UI preference does not alter routes or BookSource-provided content. Translation tests require identical message-key coverage across all supported locales. Search is now a complete backend-backed Vue vertical slice: batched SSE streaming, cancellation, continuation and retry, stale-stream protection, consolidated cross-source results, tab-local restoration across preview navigation, partial-failure reporting, canonical `/books/preview` → `/books/shelve` persistence, responsive result UI, and three-locale coverage are implemented. The API transport is split into domain modules (`auth`, `sources`, `search`, `explore`, `books`, `reader`) over one shared transport; `api/client.ts` remains only a temporary compatibility barrel for unmigrated callers. Stored-book Detail is now a complete backend-backed Vue slice: book metadata and TOC load independently, persisted alternatives render after refresh even when the current TOC fails, user-triggered exact-book scanning persists matches incrementally, source switching validates and restores the TOC through existing backend contracts, and explicit clear-and-rescan preserves the active source behind a destructive confirmation. A reusable `features/source-recovery` discovery module owns scan mechanics and will be consumed by Reader next. Reader is now a complete primary backend-backed Vue slice for text/image reading, progress, TOC, bookmarks, settings, fonts, and source recovery. Explore is now a complete strict single-BookSource Vue slice with source-native catalogs, controls, diagnostics, category caching, and sequential pagination. BookSource management is now a complete backend-backed Vue slice for listing, filtering, selective import, identity-preserving definition updates, enabled/Explore toggles, advanced raw JSON editing, and guarded deletion. Application Settings is now a complete Vue slice for locale, Reader defaults, preview, and reader-home font management. The Reader account page and administrator reader-account management are complete Vue slices. Every authenticated route now has a backend-backed Vue implementation; remaining frontend work is parity hardening and integrated end-to-end verification rather than placeholder replacement.

The bookshelf-model correction is complete: normalized title+author is the logical book identity, while source URL/book URL pairs are bindings beneath it. Adding or rediscovering the same logical book merges unique sources into one shelf row while preserving its ID, selected source, reading progress, chapters, cache, and bookmarks. Book details own a batched **Book sources** workflow using the existing search cursor protocol, accept only normalized exact title+author matches, and never switch automatically. Current-source UI displays the imported source title rather than its URL. The incompatible disposable development data root was deleted; the next local run starts from the current clean schema.


**Account storage checkpoint (2026-08-07):** The atomic ownership boundary now includes disabled-by-default ordinary-reader registration. `REGISTRATION_ENABLED=true` enables the public policy/form and account creation; optional `REGISTRATION_INVITE_CODE` adds constant-time deployment admission. Registration durably reserves a recognizable disabled reader identity, idempotently publishes its isolated reader home, activates the account, and creates an authenticated session only after storage exists; same-credential retries resume interruptions without treating Administrator-disabled accounts as provisioning claims. Authenticated password change now verifies the current credential, uses optimistic hash/version matching to reject stale concurrent changes, atomically revokes every browser session, and forces a clean re-login through a dedicated Account page. Administrator-only ordinary-reader listing, active↔disabled transitions, and one-time password reset are complete. Administrator targets remain protected; disable atomically revokes all target sessions; re-enable preserves credentials and Reader Data; reset tokens are random, hash-only, returned once with no-store headers, expire after exactly 30 minutes, supersede earlier unused tokens, and are consumed atomically with password replacement and all-session revocation while preserving active/disabled status. The durable ordinary-reader deletion slice is complete: exact username confirmation creates an independent roll-forward job and commits the target to `deleting` with session revocation; runtime quiescence rejects new acquisitions, drains in-flight requests, purges per-reader Search/Explore/Source Session state, and closes home handles; `readerstore` then removes only the immutable-ID contained directory; finally system account records are removed and the surviving job is marked complete. Failures retain the non-authenticating account/job and retry idempotently; removed files are never reconstructed or reported as rolled back.

**Phase:** Systematic Legado field-coverage implementation; Phase 7 Explore/Discover audit, Phase 4 crawl compatibility, Phase 0 baseline, and the production-container slice are complete.
...
**In progress:** The ground-up Vue frontend migration is proceeding feature by feature against existing backend/domain contracts. The application, routing, session, localization, Shelf, domain-split transport, Search, preview, and canonical shelving path are now verified; stored-book Detail, reusable source recovery, and the primary Reader vertical slice are now verified. Reader loads text/image chapter blocks, restores normalized scroll position, serializes state-versioned progress writes, navigates around volume rows, manages bookmarks, persists typography/color/width preferences, loads uploaded fonts, and reuses failure-triggered source recovery while preserving already rendered prose. Strict per-source Explore is now verified: Vue lists only Explore-enabled sources, opens one opaque backend session at a time, renders source-native controls and category order, invalidates cached pages when controls change, obeys sequential server-authoritative pagination, recovers expired sessions and page conflicts, and restores the selected source/category/results after Preview → Back. BookSource management is now verified: Vue lists and filters backend-owned definitions, previews valid single/array JSON imports with no default selections, imports only explicitly selected entries, preserves unknown fields through identity-keyed updates, supports enabled and Explore toggles, exposes a guarded complete-JSON editor for rules/headers, and confirms deletion. Application Settings is now verified: Vue manages device-local UI language, device-local Reader defaults shared with Reader, live reading preview, and backend-persisted custom fonts with guarded upload/deletion and selected-font fallback. The Reader account page is now verified: Vue renders authenticated identity/role, changes passwords with backend bounds and exact confirmation, clears all credential fields after attempts, closes private state after successful all-session revocation, and exposes retryable logout revocation failure only on the public Login route. Administrator reader-account management is now verified: the protected Vue route lists ordinary readers, filters lifecycle state, performs idempotent enable/disable transitions, discloses one-time reset credentials only in the issuing dialog, and guards durable deletion with exact case-sensitive username confirmation plus retry for server-authoritative `deleting` state. All authenticated migration placeholders have now been replaced with backend-backed Vue routes. Prototype-only behavior must not expand backend scope; every migrated feature must preserve supported production behavior and provide English, Simplified Chinese, and Traditional Chinese messages before completion. Phase 5.7 canonical book-detail/source-discovery UX is approved and implementation is starting test-first. Search-result inspection will become a non-persistent Book Info+TOC preview; Read will validate before shelving; source descriptions will normalize to plain prose; and stored detail plus Reader will share bounded alternate-source search mechanics while retaining context-specific switching. The compatibility queue described below remains paused at its recorded stopping point while this user-facing workflow correction and the remaining ownership legacy-removal gate are completed. The atomic ownership cutover is now mounted: public health/auth/setup/configured-recovery routes precede identity, every Reader Data route authenticates before selecting an exact-version reader home, Search/Explore/workflow/JavaScript mutable state is per reader, fonts use the home `FileStore`, and production no longer opens `novelreader.db` or accepts `DATABASE_PATH`. The frontend resolves setup and current account before mounting private components and provides setup, login, logout, and configured recovery. Registration policy/UI, password change, and ordinary-account administration/deletion are complete. Centralized mid-session 401 handling is complete, and the clean-data-root browser setup→logout/login→real import/search→shelf/detail/TOC/content→logout workflow passed on 2026-08-13 with explicit post-logout 401 denial for private Reader Data. Phase 2.3 backend setup core now has a durable singleton claim, one-winner in-process serialization across Store/Manager instances, staged reader-home publication, atomic initial-Administrator activation/setup closure, bootstrap-token validation without persistence, and deterministic claim recovery including expired claims. The strict one-time setup HTTP handler and modest Svelte setup form are implemented and tested, including pasted-token handling, bounded/rate-limited requests, deterministic claim recovery, retry-safe auto-login for the immutable initial Administrator, and late-session cleanup. They are now mounted with the atomic ownership cutover. Environment-token Administrator recovery is implemented, tested, and mounted only while configured: reset reactivates the selected Administrator, replaces the password, revokes sessions, and prevents stale authenticated results from minting sessions; replacement creation uses a generation-bound durable claim and a new empty reader home without attaching existing data. The server now enforces one writable NovelReader process per data root with an OS-level lifetime lock, making setup/recovery/home publication and session revocation linearizable in the supported deployment topology. Reader-storage foundation phases 1.1–1.3 are complete: the server gates the data root before SQLite opens, `readerstore` owns portable reader homes and bounded handles, and legacy development state is reset manually rather than carried through migration machinery. Account and Reader Data routes now open only through the completed authenticated ownership boundary. The durable compatibility queue and audit qualifications remain tracked in `docs/LEGADO_COMPATIBILITY_TASKS.md`. `ruleContent.replaceRegex`, shared Search/Explore list-result metadata, `enabledCookieJar`, `ruleBookInfo.canReName`, and transient file-source `ruleBookInfo.downloadUrls` extraction are complete. Upstream verification reclassified `ruleSearch.checkKeyWord` as source-validation metadata rather than an ordinary-search override; the next queue item is focused explicit CSS/Jsoup compatibility. User approved implementation of the verified v12 compatibility gaps, constrained by the raw booksource contracts and upstream Legado semantics. Primary-source review against `reference/legado` and the official source-rule documentation rejected raw 251's automatic mobile User-Agent inference: Legado uses a configurable desktop fallback unless the source declares another header, so that finding will be reclassified rather than generalized. The approved TDD seams are Analyzer for Default grammar/value extraction, production Explore for non-2xx response-body rule execution, and the public JavaScript bridge for byte-array crypto/login APIs. Verified implementation scope is: received non-2xx HTTP responses continue into rules while retaining diagnostics; exact `base64DecodeToByteArray`, `createSymmetricCrypto`, and `source.getLoginHeaderMap` public shapes; generic terminal Default attributes; complete bracket index/list/exclusion/range/step grammar; and blank/deduplicated Default attribute extraction. The unrestricted deterministic twelfth Explore audit is complete. The user selected 50 identities because v11 exposed four shared compatibility seams. Seed `NovelReader-explore-random-v12-2026-07-31` excluded all 500 stable identities from batches 1–11 and selected the first 50 of 221 remaining eligible identities by `SHA-256(seed + NUL + rawIndex + NUL + bookSourceUrl)`, without substitutions. Production returned 27 credible identities and 2,768 distinct book URLs; all 24 initial non-passes or diagnostics were replayed sequentially. Three identities expose five verified reusable seams: rule-driven AJAX blocked by an outer non-2xx plus missing crypto/login bridge APIs (197), arbitrary Default attribute getters after class traversal (163), and singleton bracket indexes plus blank-skipping multi-match attributes (17). Raw 251 was corrected to an incomplete source contract because upstream Legado also uses a desktop fallback unless the source declares a mobile header. Other outcomes are 6 invalid/incomplete sources, 5 site-drift, 3 upstream DNS, 2 upstream HTTP, 2 stale contracts, 1 blocked/auth, and 1 audit-infrastructure resolver failure. Evidence is `testdata/booksource/audits/explore/explore-live-audit-v12-2026-07-31.{json,md}`. No source/parser behavior changed during sampling. All four user-approved v11 shared-gap fixes are implemented and exact-identity live-verified through fresh production Explore. Raw 701 returns 30 distinct books after inline replacement Java identity-escape normalization; raw 54 returns 180 after Default `[8:]` range support and mode detection; raw 220 returns 100 after 16.156 seconds through configurable 30-second Explore context and HTTP-client deadlines; raw 443 returns 12 after preserving structured JSON list elements for direct JavaScript. Evidence is `testdata/booksource/audits/explore/explore-live-v11-fixes-rerun-2026-07-31.json`. The user approved implementation of all four v11 shared gaps through existing interfaces. Work proceeds as four vertical TDD slices: (1) Analyzer inline replacement rules normalize Java identity escapes; (2) Analyzer Default selectors support Legado positional ranges such as `[8:]`; (3) Explore page requests use a configurable `SearcherLimits.ExploreSourceTimeout`, exposed as positive integer `EXPLORE_SOURCE_TIMEOUT_SECONDS` with a 30-second default while search retains its existing ten-second deadline; and (4) production Explore preserves structured JSON list elements for direct per-result JavaScript. Each slice must have a red public-seam regression before the smallest implementation change. Exact raws 701, 54, 220, and 443 will be production-rerun after implementation. The unrestricted deterministic eleventh Explore live audit is complete. Seed `NovelReader-explore-random-v11-2026-07-23` excluded all 450 stable identities from batches 1–10 and selected the first 50 of 271 remaining eligible identities without substitution. Production returned 39 credible sources and 1,142 distinct book URLs; all 12 initial non-passes were rerun sequentially, with raw 423 recovering to 50 distinct books. Four identities expose four shared seams: inline replacement Java identity escapes (701), Default positional ranges (54), the fixed ten-second Explore source deadline (220), and structured JSON result access from direct per-result JavaScript (443). Other outcomes are 3 upstream HTTP, 2 upstream DNS, 1 blocked/auth, and 1 site-drift result. Evidence is `testdata/booksource/audits/explore/explore-live-audit-v11-2026-07-23.{json,md}`. No source/parser behavior changed during sampling. The user-approved raw-927 fix is implemented and live-verified. Public-seam TDD disproved the audit's initial rule-selection diagnosis: `exploreResultRules` already selects the entire `ruleSearch` object when `ruleExplore.bookList` is blank. The exact live-shaped regression exposed the shared Default string seam: unpositioned selectors such as `class.mh-item@a@text` returned only the first matched element, which was empty, while Legado collects all non-empty matches and joins them. The shared analyzer now collects all non-empty Default `text` getter values, joins them with newlines, and de-duplicates attribute values as upstream does; unrelated `ownText`, `textNodes`, `html`, and `all` behavior remains unchanged. Fresh production Explore for exact identity `(927, https://www.jjmh.top/)` returns 28 distinct books in category `连载`; evidence is `testdata/booksource/audits/explore/explore-live-v10-fixes-rerun-2026-07-23.json`. An earlier 21-book `性感` rerun used duplicate raw 317 and is explicitly superseded in that evidence. Other outcomes are 6 stale source contracts, 4 upstream HTTP failures, 1 blocked/auth response, and 1 invalid source script. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v10-2026-07-23.{json,md}`. No source/parser behavior changed. Java `\uXXXX` regex escapes translate to Go-compatible code points while malformed escapes still fail; Default traversal uses an independent accumulator; dotted `.[*]` JSON wildcards normalize outside quoted strings; and simple root-array wildcard predicates preserve order. Evidence is in `testdata/booksource/audits/explore/explore-live-v9-fixes-rerun-2026-07-22.json`. Stale contracts and URL-pattern enforcement remain unchanged.

The ninth unrestricted deterministic Explore live audit is complete. The user-selected 50-source batch used seed `NovelReader-explore-random-v9-2026-07-22`, excluded all 350 identities from batches 1–8, and froze the first 50 of 373 remaining eligible identities by stable SHA-256 ranking with no substitutions. A fresh isolated production run with four clients returned 33 credible sources and 932 distinct books; all 17 non-passes were confirmed sequentially. Three identities expose four shared compatibility seams: Java `\uXXXX` regex normalization and multi-parent Default shorthand traversal (raw 66), dotted JSON array wildcards (57), and root-array wildcard predicates (581). The other outcomes are 7 stale source contracts, 1 DNS failure, 3 upstream failures, 2 blocked/auth, and 1 invalid source script. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v9-2026-07-22.{json,md}`. No source/parser behavior changed.

The eighth unrestricted deterministic Explore live audit is complete. The user-selected 50-source batch used seed `NovelReader-explore-random-v8-2026-07-22`, excluded all 300 identities from batches 1–7, and froze the first 50 of 423 remaining eligible identities by stable SHA-256 ranking with no substitutions. A fresh isolated production run with four clients returned 34 credible sources and 1,045 distinct books; all 16 non-passes were confirmed sequentially. Diagnosis found no shared engine gap: 2 upstream DNS, 3 upstream HTTP, 2 blocked/auth, 6 stale source contracts, 1 invalid source script, and 2 site-drift outcomes. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v8-2026-07-22.{json,md}`. No source/parser behavior changed.

The user-approved v7 shared Explore fixes are implemented and live-verified. Work will proceed test-first through the public Explore catalog/page boundaries: complete source metadata in rule JavaScript (raw 177), executable `@js:` source headers with bounded source/session context (5), JSON object wildcard/filter list selection (503), and Java-compatible Jsoup `ownText()` without breaking existing properties/enumeration (288). Fresh production reruns repair raw 177 to 1 distinct book, raw 5 to 21, raw 503 to 150, and raw 288 to a 118-entry catalog plus 30 books. Live verification exposed and fixed two downstream root causes rather than overclaiming the first seams: encoded page-selector expansion for raw 503 and mutable per-result book context for raw 5. Adjacent corpus inventory identifies 28 source-metadata, 14 executable-header, 8 JSON wildcard-filter, 2 `ownText()`, and 8 mutable-book-context identities; deterministic regressions pin the affected semantics without claiming every live upstream repaired. Stale, blocked, upstream, and invalid source outcomes remain unchanged. The committed Explore audit skill completed the user-selected seventh targeted 25-source batch. Seed `NovelReader-explore-targeted-v7-2026-07-22` will rank identities after excluding all 275 prior stable `(rawIndex, bookSourceUrl)` samples. Planned strata are 6 redirected/custom-header, 6 Jsoup collection, all 3 remaining source-variable identities plus 2 other catalog-JavaScript identities, 4 nullable/missing-prone JSON, and 4 ordinary controls. The source-variable adjustment is required because only three unsampled eligible identities remain; no prior identity will be reused. No substitutions or source/parser changes occurred. Twelve credible sources returned 274 distinct books. Four identities expose four new shared seams: full source metadata missing from rule JavaScript (raw 177), executable `@js:` source headers discarded (5), JSON object wildcard/filter list selection returning no elements (503), and Jsoup `ownText()` exposed as a property instead of a method (288). The remaining nine non-passes are blocking/authentication, upstream failure, site drift, legitimate empty data, or invalid source JavaScript. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v7-2026-07-22.{json,md}`. The user-approved v6 shared Explore fixes are implemented and live-verified test-first through public production boundaries. Three deterministic gaps are now green: catalog JavaScript `source.setVariable` (raw 933), Jsoup collection `size()`/`eq()` (749/301), and nullable optional fields in Explore fallback extraction (757), while required `name`/`bookUrl` failures remain strict. The redirected-header failure (112/311) was traced beyond transport: the exact fingerprint client succeeds, but the full Searcher silently drops both source headers because those imported configurations use JavaScript-style single-quoted maps and `parseHeaderJSON` accepted strict JSON only. A public Explore regression is now green through the shared lenient map parser. Fresh production reruns repair raw 112/311 to 10 distinct books each, raw 749 to 20, and raw 757 to 8. Raw 933 and 301 now generate valid 26/11-entry catalogs rather than bridge failures; their first current live pages are empty without engine diagnostics, so only catalog repair is claimed. All 21 adjacent corpus identities using `source.setVariable`, Jsoup `size()`, or Jsoup `eq()` were replayed individually: 16 generate valid catalogs and five reach unrelated existing source/bridge boundaries. Stale source contracts remain unchanged. The project-local `.agents/skills/explore-live-audit/SKILL.md` workflow is committed and its first invocation is complete. The unrestricted deterministic sixth batch used seed `NovelReader-explore-random-v6-2026-07-21`, sampled 50 identities disjoint from all 225 stable batch 1–5 identities, and made no substitutions or source/parser changes. Twenty-three sources returned 488 distinct books. Six identities expose four shared seams: redirected custom headers (raw 112/311), missing `source.setVariable` (933), Jsoup `size()`/`eq()` compatibility (749/301), and nullable optional JSON fields aborting fallback extraction (757). The remaining 21 outcomes are stale contracts, upstream failures, blocking/authentication, invalid sources, or site drift. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v6-2026-07-21.{json,md}`. It will encode the established deterministic, disjoint live-sampling workflow, require an explicit batch-size choice with a recommendation, preserve machine-readable and narrative evidence, classify failures through sequential and browser checks, and stop for approval before any engine or source fix. The two approved final-audit compatibility fixes are implemented and live-verified. Raw 84 now returns 18 distinct books instead of one; the two remaining live rows use non-numeric slugs rejected by the unchanged imported pattern. Raw 565 now generates 470 categories (458 selectable) instead of failing. All nine corpus identities containing `java.t2s` were replayed individually through the production runtime: none fails on `t2s`; four generate valid catalogs and the other five reach unrelated existing source/bridge boundaries. Side-effect gates preserve CSS `div@id`, `div@class`, `div@data-*`, positioned child traversal, exclusion routing, and unchanged script/session behavior. The final deterministic 25-source Explore audit is complete. It excluded all 200 prior audit identities and sampled 15 affected-rule-family sources plus 10 controls. Seventeen sources returned 355 distinct books; two shared compatibility gaps remain: unpositioned `tbody@tr` still treats the bare child token as an attribute getter, and generated catalogs cannot call Legado's focused `java.t2s(text)` helper. The other six outcomes are upstream HTTP/DNS, stale or incomplete source contracts, site drift, or malformed source JavaScript. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v5-2026-07-21.{json,md}`. No parser or source behavior changed during sampling. The approved shared Explore compatibility implementation and verification are complete. Generic fixes cover Default traversal/exclusion, connector and interpolation semantics, optional mixed-mode fallback, exact focused Java helpers, URL-option AJAX with GET/POST/HEAD, retry, response charset, and form-body charset handling, transformed element fallbacks, JSON-list extraction, `@@` Default forcing, and JSoup numeric enumeration. Targeted live reruns repaired 13 engine identities: raw 76, 82, 204, 209, 342, 410, 429, 471, 516, 614, 816, 897, and 920 now return distinct books. A fourth deterministic 50-source audit, disjoint from all 150 earlier sample identities, is complete: 31 credible passes returning 927 books, 2 shared engine gaps, and 17 upstream/source-contract/catalog outcomes. The two engine gaps are raw 830 losing table context when selected `<tr>` fragments are reparsed for field extraction, and raw 615 failing positioned `img.0@title` while unpositioned `img@title` succeeds. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v4-2026-07-21.{json,md}`. The third, fully random disjoint 50-source audit found 25 credible passes, 6 shared engine gaps, 14 stale/unavailable/blocked/auth-dependent sources, 4 invalid or incomplete raw Explore contracts, and 1 legitimate empty API response. Confirmed shared families are multi-parent Default traversal, Java-regex parity, mixed HTML/JSON template fallback, bounded handling for valid pages above 2,000 results, and `java.getElement`. No raw source was changed during this audit. V3 implementation slices 1–5 are complete: bare element chains route into per-parent Default traversal, embedded JSONPath against HTML contributes an empty template value without hiding hard errors, the regex boundary translates focused Java syntax, `java.getElement` returns the existing JSoup-compatible selection, and Explore capacity now retains the ordered prefix up to 2,000 with a warning, exhaustion, and idempotent replay. Verification is complete. Full backend and race suites pass; frontend has 13/13 passing tests and the production build succeeds to an isolated output directory; Docker E2E passes. Fresh production-transport reruns return raw 737=10, 462=20, 305=16, 215=100, and raw 726=2,000 distinct books with `result_truncated`, warning severity, exhaustion, and no ceiling increase. Playwright independently sees 10, 20, 20, 16, 100, and 2,432 live rows for raw 737, 788, 462, 305, 215, and 726 respectively. Raw 788’s deterministic embedded-template regression passes, but its live server alternated between an upstream-empty HTTP 200 and a retryable transport failure while Playwright still saw 20 rows; this is reported as externally blocked rather than a claimed live repair. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v3-2026-07-19.{json,md}` and `testdata/booksource/audits/explore/explore-live-v3-fixes-rerun-2026-07-19.json`. The user's pre-existing generated frontend/package-lock changes remain unstaged.

**Atomic ownership cutover (complete):** The server has only public health/static/setup/auth/configured-recovery boundaries before identity; every Reader Data API request authenticates first, derives immutable `UserID` from context, acquires a bounded per-reader runtime/home, and constructs feature access exclusively from `reader.db`/`files/`. Feature-owned current DDL initializes together in one transaction. Before writable open, `readerstore` rebuilds that same authoritative DDL in memory and requires the published reader database to match its complete SQLite catalog and schema epoch; opening never runs DDL against or upgrades published data. Font files use `Home.Files()`; Search/Explore/workflow and JavaScript compatibility state are per-reader while process capacity remains shared; global feature startup and `DATABASE_PATH` are removed; and equal-ID isolation/anonymous-denial regressions pass. No legacy Reader Data migration or dual-mode fallback exists.

**Latest Search → Book Info audit:** v4 sampled 50 unrestricted deterministic identities from unchanged `test_booksource3.json`, disjoint from all 100 v1–v3 identities, with fixed query `凡人修仙传`. Exactly the frozen raw definitions were executed through the production conformance path; all 41 initial non-passes replayed sequentially. Nine completed both stages. No audit-proven recoverable shared-engine gap was found: outcomes were 20 persistent upstream timeouts, 4 upstream HTTP failures, 3 legitimate empties, 3 blocked/authentication boundaries, 3 explicit WebView dependencies, 3 site/source drift cases, 2 invalid source contracts, and one each DNS, transport, and Rhino/JVM dependency. Raw 72 exposed a non-recovering bridge omission: Legado supports `java.encodeURI(value, charset)`, while NovelReader ignored the charset argument. The separately approved shared fix is complete: one-argument UTF-8 behavior remains unchanged, optional charset uses the existing shared encoder, and unsupported charsets fail clearly. A fresh untouched raw-72 replay sends GB2312 bytes and displays `凡人修仙传` correctly while still returning an explicit zero-result page, so v4's historical legitimate-empty classification and zero-recoverable-gap result remain unchanged. Audit evidence is `testdata/booksource/audits/search-bookinfo/search-bookinfo-live-audit-v4-2026-08-12.{json,md}`; post-fix evidence is `search-bookinfo-live-v4-fixes-rerun-2026-08-12.json`.

**Explore audit v13 complete:** The user-selected unrestricted 50-source round used seed `NovelReader-explore-random-v13-2026-08-11`, excluded all 550 identities in Explore audits v1–v12, and froze the first 50 of 171 remaining eligible identities from unchanged `test_booksource4.json` by stable SHA-256 ranking with no substitutions. A fresh disposable authenticated production run returned 42 credible sources and 1,211 distinct book URLs; all eight non-passes and one bounded HTTP-status diagnostic were confirmed sequentially. No shared NovelReader gap was found: outcomes were 3 incomplete/invalid source contracts, 2 upstream DNS failures, 2 upstream HTTP failures, and 1 blocked source. Playwright was used only to prove Cloudflare blocking and distinguish two working browser responses from missing source result rules. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v13-2026-08-11.{json,md}`. No source/parser behavior changed.

**Explore audit v14 complete:** The unrestricted 50-source round used seed `NovelReader-explore-random-v14-2026-08-11`, excluded all 600 identities from Explore audits v1–v13, and froze the first 50 of 121 remaining eligible identities with no substitutions. Exact-definition execution found 31 credible sources and one shared Legado divergence: NovelReader incorrectly applied `bookUrlPattern` as a per-result filter. The approved shared fix removed only that rejection and preserved normal URL resolution; fresh exact-source reruns repaired raws 669, 703, and 80 to 60, 15, and 30 books. Evidence is in `testdata/booksource/audits/explore/explore-live-audit-v14-2026-08-11.{json,md}` and `explore-live-v14-fixes-rerun-2026-08-11.json`.

**Latest ownership verification:** The clean-data-root Playwright account-shell workflow passed on 2026-08-13. A fresh disposable root completed first-Administrator setup → logout/login → import of 939 real source definitions → live `凡人修仙传` search → shelving `凡人修仙传仙界篇` from `365小说网` → detail and 1,412-chapter TOC → genuine `第一章 狐女` content → persisted `Ch.3 / 1412` shelf state → logout. In the same browser context, `/api/books`, `/api/sources`, and `/api/fonts` each changed from authenticated HTTP 200 to HTTP 401, and direct `#/shelf` navigation rendered login rather than Reader Data. The isolated root created `system.db` plus one immutable-ID `users/<id>/reader.db` home and no root-level `novelreader.db`. Exact evidence and environment limitations are recorded in `docs/verification/ACCOUNT_SHELL_CLEAN_ROOT_2026-08-13.md`.

**Next action:** Finish the remaining Phase 2 legacy-removal checks in `docs/USER_STORAGE_IMPLEMENTATION_TASKS.md`: remove or justify cutover-only adapters/flags and direct-global-store tests, audit every legacy path/table reference, and prove an old global root fails clearly with no runtime route capable of opening it as Reader Data. Public registration remains disabled by default; no legacy reader-data migration is in scope. After that gate, resume the paused compatibility queue at focused explicit CSS/Jsoup compatibility. LC-016 remains blocked until the ownership boundary is fully verified, and LC-015 is complete.

**Candidate book resolution:** The current journey contract is documented in `docs/CANDIDATE_BOOK_JOURNEY.md`.  Search and Explore results open as non-destructive full Book Detail candidates or start direct shelf operations. The backend owns account-scoped asynchronous candidate-resolution operations with reconnectable SSE snapshots, explicit cancellation, a stable queue containing every discovered binding, and at most five immediately filled concurrent source attempts. Each source validates Book Info, TOC, and credible first-chapter content with per-stage deadlines; the operation has a bounded wall-clock lifetime and reports structured per-source progress instead of holding one HTTP request open. A verified operation holds server-owned metadata and TOC for an idempotent shelf commit without recrawling; direct card adds commit automatically. Search and Explore show this as an inline disclosure owned by the feature-local candidate component: source names and user-facing validation stages are visible on demand, collapse affects presentation only, navigation closes only the SSE subscription, remount fetches the authoritative snapshot and reconnects, cancellation is acknowledged by a terminal backend snapshot immediately while runtime cleanup drains privately, and terminal outcomes remain visible on the card—success as a persistent Added status and failure as an expanded retryable explanation. Operations are transient, bounded per reader, and release reader-runtime leases on commit, cancellation, exhaustion, expiry, eviction, or shutdown. Fallback preserves the consolidated title/author identity and every discovered alternate binding.

**Maintainability cleanup:** `docs/CODEBASE_CLEANUP_AUDIT.md` records the completed vertical removal of superseded candidate-ingestion and unbatched Search paths. The private bundled-frontend API now has one candidate boundary (`/api/candidate-resolutions`) and one Search transport (batched `/api/search/stream`); raw add, enrich, preview, shelve, readable, unbatched Search, and their unused Vue wrappers/tests are removed. Remaining cleanup areas are limited to separately scoped ownership-cutover, archived artifact, and tooling audits. Unused-symbol reports remain hints rather than proof because Vue templates, Pinia aliases, routes, types, and test-only entry points produce false positives.

**Shelf and BookSource presentation:** The Vue Shelf now uses a distinct editorial Continue Reading panel and a cover-led responsive collection. `GET /api/books` derives `currentChapterTitle` from the stored chapter at `durChapterIndex`, avoiding one TOC request per shelf item while keeping the title tied to persisted reading state. BookSource Management keeps the existing full-list/filter/statistics API for now but renders filtered results in deterministic 25-item client-side pages; server-side pagination remains a possible later optimization if source collections materially outgrow the current imported corpus.

**Reader Chinese conversion:** Reader settings now include a device-local, manual `Original` / `Simplified Chinese` / `Traditional Chinese` display preference with Original as the default. The frontend lazy-loads pinned `opencc-js` conversion dictionaries only when conversion is enabled and derives converted chapter titles, TOC rows, prose paragraphs, and text content blocks without mutating backend responses, stored/cache content, progress, bookmarks, Shelf metadata, Search, or Explore results. Generic OpenCC Traditional (`cn` → `t`) is used rather than region-specific phrase conversion; mode changes preserve normalized reading position and fall back visibly to original text if conversion cannot load.
...
| Discovered-book detail | Full non-persistent candidate detail backed by an account-scoped transient resolution operation; canonical stored detail remains ID-based | Inspecting a result must not imply shelf ownership, progress must reconnect without tying work to one request, verified metadata/TOC should commit without recrawling, and stored reading/progress contracts keep one explicit identity. |
| Description rendering | Backend-normalized plain prose; never source HTML injection | Preserves entities and paragraph breaks without granting untrusted BookSource content DOM/script/style authority. |
| Alternate-source discovery | Shared batch/SSE controller with detail and Reader shells | Reuses preferences, bounded concurrency, retry, and cursor behavior without coupling source switching to a monolithic search page. |
...
| Reader storage | `system.db` plus one self-contained plaintext reader directory per immutable user ID | Keeps authentication separate while making books, sources, state, and files independently inspectable and portable. |
| Storage seam | One deep `readerstore` module owns paths, DB lifecycle, transactional fresh-schema initialization, exact epoch-and-catalog refusal, backup, export/import, and deletion coordination | Future features contribute one authoritative current-DDL function and owned files without hardcoded paths or coupling HTTP/authentication to storage layout. |
| Backup | Stopped-server copy of complete `data/` is supported | Disaster recovery must work even when NovelReader cannot start. |
| Credential portability | Source credentials encrypted separately and excluded from reader exports | Losing keys affects external logins only; portable Reader Data remains readable. |
...
| Frontend boundary | Domain/API contracts only | Vue features remain independent of scraping syntax and backend implementation details. |
| Frontend framework | Vue 3 SPA with Vue Router, Pinia, Options API, and Vite | The ground-up pre-public replacement benefits from conventional routing, narrow shared state, mature tests/devtools, and predictable component organization without mixing frameworks. |
| Client/server workload | Go owns source execution and resource-intensive domain workflows; Vue owns interaction and presentation | Keeps mobile clients lightweight without introducing HTML SSR or moving crawler behavior into the browser. |
| Frontend routing | Hash history initially | Preserves direct-link/static-file deployment compatibility without requiring a backend routing change; route topology remains owned by Vue Router. |
| UI localization | Vue I18n with feature-owned lazy message modules for English, Simplified Chinese, and Traditional Chinese | Browser preference selects the first locale with English fallback; a locally persisted preference changes UI language without altering routes or source-provided book content. |
## Deferred Work
...
### [2026-08-11] BookSource test data organized by purpose and operation
- **Problem**: Deterministic fixtures and dated live evidence for Explore and Search → Book Info shared one flat directory, making ownership and future placement unclear.
- **Fix**: Split `testdata/booksource/` into `conformance/{core,explore}/` and `audits/{explore,search-bookinfo}/`; update tests, scripts, reports, and operation skills. Add the shared `booksource-audit-workflow` skill to require scoped funnels, stable identities, smallest-tool diagnosis, warranted Playwright use, shared-gap proof, operation-owned paths, and an approval gate before fixes.
- **Verified**: Core and Explore fixture tests load their new directories; audit verifiers resolve the moved evidence; stale root-level operation paths are absent; unrelated generated frontend files remain untouched.

### [2026-08-12] Search → Book Info v4 found no recoverable shared gap

- **Context:** A 50-source deterministic Search → Book Info audit sampled exact definitions disjoint from all 100 v1–v3 identities and replayed every non-pass sequentially.
- **Evidence:** Nine sources completed Search and Book Info. Exact-request cross-checks confirmed 20 persistent timeouts and the remaining upstream, blocked, WebView, source-contract, DNS, transport, and Rhino/JVM outcomes. Raw 72 proved a non-recovering public-bridge omission: `java.encodeURI(value, charset)` was not honored, but correcting only its GB2312 output still produced an explicit zero-result page.
- **Decision:** Preserve v4's zero-recoverable-gap result. The separately approved bridge fix is now complete through the existing shared charset encoder; it does not add WebView or Rhino/JVM scope.
- **Verified:** The v4 verifier reconstructs the disjoint ranking and exact definitions, recomputes the complete 50-source histogram, requires all 41 sequential replays, validates timeout/browser evidence, and rejects any shared-gap classification. The post-fix verifier binds raw 72 to its frozen identity, confirms the real GB2312 request and correctly decoded query text, and confirms the legitimate-empty outcome is unchanged.

### [2026-08-12] Search → Book Info v3 compatibility fixes completed

- **Context:** A 50-source deterministic Search → Book Info audit found two recoverable gaps and two additional documented URL-JavaScript omissions.
- **Change:** Standalone OnlyOne regex rules now return only the first matched-and-replaced value; declared non-UTF-8 URL charsets encode GET query variables before request execution. The URL builder now executes whole and inline `<js>...</js>` segments, and Search/Explore URL construction receives source `jsLib` through the existing `URLContext`.
- **Reason:** These are direct Legado contracts in the rulebook and `reference/legado`. The implementation stays at existing shared seams and does not add source-specific handling, a second script engine, WebView behavior, or Rhino/JVM APIs.
- **Verified:** Deterministic regressions cover match/no-match regex behavior, GB2312 GET queries, whole/inline URL `<js>`, and Search/Explore `jsLib`. Exact untouched frozen-source reruns now return 20 Search results and successful Book Info for raws 375 and 364. Post-fix evidence is `testdata/booksource/audits/search-bookinfo/search-bookinfo-live-v3-fixes-rerun-2026-08-12.json`.
- **Watch out:** URL `<js>` and `jsLib` support do not imply Rhino/JVM compatibility; raws 46/47/51/192 retain independent site, HTTP, or JVM blockers and are not claimed as live recoveries.

### [2026-08-11] Second Search → Book Info audit found two shared workflow gaps
- **Problem**: A disjoint exact-contract 25-source sample found raw 49 joining two extracted hrefs into one malformed newline-delimited `bookUrl`, and raw 179 returning zero Search results after its POST redirected to a valid detail page.
- **Evidence**: Upstream Legado extracts Search `bookUrl` with URL semantics that keep the first match, and attempts Book Info when Search list extraction is empty and `bookUrlPattern` is absent. Production counterfactuals succeed: raw 49 enriches from its first href; raw 179 enriches from the final redirected URL/body. The audit verifier regenerates the disjoint ranking and exact definitions and checks all non-pass replays.
- **Decision**: Report both shared seams without changing production during the audit. Proposed separate fixes are URL-valued first-match extraction and empty-list Search-to-Book-Info fallback. WebView-dependent raw 396 remains deferred.
- **Verified**: Eight complete passes, two shared gaps, one deferred WebView dependency, and fourteen evidence-backed upstream/source/empty outcomes are preserved in v2 JSON/Markdown evidence.

### [2026-08-11] Search → Book Info audit found one shared detail URL gap
- **Problem**: In a deterministic 25-source Search → Book Info sample, a live source returned valid detail links ending in a lenient Legado request-option object such as `,{Cookie:"..."}`. NovelReader encoded that suffix into the detail path and received 404, while the URL portion with the declared cookie returned current HTTP 200 detail HTML.
- **Evidence**: Official rule documentation defines trailing request options; upstream `AnalyzeUrl` at local revision `44e07fea541287804cc58d0168940a756cd11cfd` parses strict JSON then lenient Gson. Reduced production probes reproduce unquoted-key option objects remaining URL text. Raw 267 establishes working-source impact; raw 151 uses the same syntax but its correctly formed upstream POST returns 500 and is not counted as working proof.
- **Fix**: Preserve any balanced trailing option object while resolving relative search-result URLs, then normalize single-quoted strings and bare map keys before the existing typed URL-option decoder. Unknown legacy fields are ignored after the suffix is removed; no general JSON5 evaluator or source-specific branch was added. The separately observed whole-URL `<js>` difference remains out of scope.
- **Verified**: Public regressions cover raw-267-style detail options and raw-151-style POST options. Analyzer/source-executor/book/conformance tests and analyzer race tests pass. A fresh raw-267 production search preserves `,{Cookie:"xmanhua_lang=2"}` and its Book Info fetch now succeeds with current detail metadata. The original 25 identities, all 15 sequential replays, corpus SHA-256, and summary counts remain verified.


### [2026-08-06] LC-015 portable chapter-image scope approved
- **Context**: The current chapter API converts all content to plain paragraphs, discarding `<img>` structure before the reader. Six active `imageDecode` rules include portable AES/obfuscated byte transforms and two Android `BitmapFactory`/`Canvas` rotation or slice transforms.
- **Decision**: Add a backward-compatible ordered text/image block model plus stored-resource endpoints. Support portable byte decoders first; Android bitmap scripts must fail explicitly as unsupported rather than returning encrypted/scrambled images.
- **Data flow**: Content extraction preserves ordered image URLs while still producing legacy paragraphs. Chapter cache stores both shapes. Reader image elements point only to `/api/books/{id}/chapters/{idx}/images/{imageIndex}`; the backend resolves stored book/source/chapter and the indexed extracted URL, applies source/URL headers and session state, performs a bounded binary fetch, then applies `imageDecode` with byte `result`, original resource `src`, source, and book context.
- **Compatibility/rollback**: Existing clients continue using `paragraphs`; new `blocks` is additive. No general arbitrary-URL proxy or new dependency is introduced. Schema migration adds a JSON blocks column that can be ignored by older binaries; rollback leaves the column unused.

### [2026-08-06] Stored-book covers gained an owned decode boundary
- **Problem**: `coverDecodeJs` was imported but browsers fetched remote cover URLs directly, leaving no layer with both raw bytes and source/session JavaScript context.
- **Fix**: Add `GET /api/books/{id}/cover`, deriving source and target from stored state, evaluating source/URL headers, reading at most 10 MiB, and applying `jsLib + coverDecodeJs` to byte-array input. Bookshelf and detail covers use the endpoint; search/Explore previews remain direct by explicit scope choice.
- **Compatibility**: Non-byte/null decoder output preserves original bytes like upstream. The sole active obfuscated decoder required a narrow `Packages.java.io.ByteArrayOutputStream`/`InputStream` Rhino shim; no general Java namespace was added.
- **Verified**: API regressions cover source and URL-scoped headers, byte transforms, null fallback, and rejection of arbitrary target URLs. The exact active script initializes with byte input. Analyzer, fetcher, book, API, and conformance tests pass; frontend 13 tests and production build pass.
- **Watch out**: This is not a general image proxy/cache and does not cover search/Explore previews, chapter `imageDecode`, WebView covers, or non-GET image requests.

### [2026-08-06] Source lifecycle callbacks are dormant
- **Context**: Upstream defines 20 reader, shelf, and interactive-button events through `eventListener` plus `ruleContent.callBackJs`; callbacks are asynchronous, can use authenticated bridges, and a truthy interactive result suppresses the default UI action.
- **Decision**: Do not build a speculative partial event bus. The audited corpora contain zero nonblank callback scripts and zero sources with `eventListener=true`; explicit false values are inactive defaults.
- **Future gate**: Start from an active source fixture, define exact owned event boundaries/order, scope bridge capabilities, preserve default-action suppression only where applicable, bound runtime, and isolate failures from core reading/state transitions.
- **Affected**: Compatibility tracker and `PLAN.md`; no runtime behavior changed.

### [2026-08-06] `payAction` remains intentionally unsupported
- **Context**: Upstream runs source-defined purchase actions only from an explicit reader menu for unpaid VIP chapters, after chapter-specific confirmation. Active rules include real paid-chapter requests, archive loan/return mutations, browser handoffs, and generated payment URLs.
- **Decision**: Do not implement or expose `payAction` now. Ordinary content fetch must never execute it, and no action is assumed safe or idempotent from its script shape.
- **Future gate**: Require user initiation and per-chapter confirmation, authenticated session scope, no automatic retries, in-flight deduplication/idempotency, explicit result states, redacted logs, and cache/TOC refresh only after confirmed success. Price/currency must not be implied unless independently available because source rules do not reliably provide them.
- **Affected**: Compatibility tracker and `PLAN.md`; no runtime behavior changed.
- **Watch out**: Revisit only as an intentional transactional product feature with owned backend API and reader UX, not as a parser compatibility patch.

### [2026-08-06] Content `sourceRegex` resource sniffing was absent
- **Problem**: WebView content requests could return rendered HTML but could not observe and return a media/file URL loaded by the page, leaving active audio and file sources unsupported.
- **Fix**: Route `ruleContent.sourceRegex` only through already-WebView initial and paginated requests. Browser protocol v2 performs full regex matches against request events and returns the first matching URL immediately, without fetching the resource or adding media-domain behavior.
- **Verified**: 15 active rules span text, audio, and archive/file source types. Public content tests cover browser-only routing, pagination, and URL output; protocol tests cover field forwarding and old-worker rejection; worker tests cover full matching, first-match interruption, blank values, and invalid patterns. A real local Patchright page requested two matching resources and returned the first (`/first.mp3`). Targeted Analyzer, sourceexec, webview, book, API, conformance, and worker tests pass.
- **Affected**: `backend/internal/book`, `backend/internal/sourceexec`, `backend/internal/webview`, `webview-worker`, compatibility tracker, and `PLAN.md`.
- **Watch out**: Backend and worker must be upgraded together because the browser protocol is now v2. This does not add playback, media downloading/decoding, UI, or browser bridge APIs.

### [2026-08-06] Content-rule `webJs` was not routed to WebView requests
- **Problem**: Content workflows preserved URL-option browser scripts but ignored the distinct `ruleContent.webJs` fallback passed by upstream for browser-backed initial and paginated chapter requests.
- **Fix**: Copy the rule-level script into request metadata only when the request already selects WebView and no URL-level script exists. Apply the same rule to pagination, preserve script text unchanged, and do not escalate ordinary HTTP requests.
- **Verified**: Injected browser-transport regressions cover initial fallback, pagination reuse, URL-option precedence, full active-corpus `<js>`/regex preservation, and no WebView invocation for HTTP-only chapter URLs. Targeted Analyzer, sourceexec, webview, book, API, and conformance tests pass.
- **Affected**: `backend/internal/book`, compatibility tracker, and `PLAN.md`.
- **Watch out**: This is request wiring, not browser bridge emulation. `java.webView*` helpers and other browser-side Legado APIs remain out of scope.

### [2026-08-06] TOC `formatJs` was ignored
- **Problem**: NovelReader returned extracted chapter titles without running the post-processing script defined by `ruleToc.formatJs`; the pinned corpus includes an active suffix-removal script.
- **Fix**: Run formatting after the final pagination/reversal/dedup/index pipeline in one JavaScript evaluation with persistent `gInt`, one-based `index`, mutable chapter context, current `title`, and source/book/session bindings. Synchronize only exposed chapter fields back and continue after per-chapter errors.
- **Verified**: Public TOC regressions cover final-order and dedup timing, one-based versus zero-based indices, persistent bindings, mutable state before failure, fail-soft continuation, a one-runtime pool, and the exact active six-digit suffix-removal contract. Targeted Analyzer, sourceexec, book, API, and conformance tests pass.
- **Affected**: `backend/internal/book`, compatibility tracker, and `PLAN.md`.
- **Watch out**: Format errors intentionally do not fail the TOC workflow, matching upstream. They are logged with the affected one-based chapter index.

### [2026-08-06] `reGetBook()` required safe precise-search workflow re-entry
- **Problem**: TOC pre-update scripts could not request upstream's precise name+author search followed by detail refresh. A direct synchronous callback would also deadlock a one-runtime JavaScript pool because search/detail rules borrow runtimes while `preUpdateJs` still owns one.
- **Fix**: Changed pre-update network helpers into ordered workflow commands executed after JavaScript returns its runtime. Added uncapped exact name+author search for this internal path, search-result-to-book replacement that preserves shelf/progress identity, stale-TOC clearing, session-aware detail refresh, and replacement book/session aliases so cookies and variables persist through TOC and content.
- **Verified**: Public regressions use a one-runtime pool and prove search → detail → TOC ordering, an exact match after 20 near-matches, search metadata transfer, stale TOC fallback to the replacement book URL, exact-author fail-closed behavior, and cookie continuity through first-chapter content. Targeted Analyzer, sourceexec, book, API, and conformance tests pass.
- **Affected**: `backend/internal/analyzer`, `backend/internal/sourceexec`, `backend/internal/book`, compatibility tracker, and `PLAN.md`.
- **Watch out**: The pinned corpus currently has no active `reGetBook()` call, so this is deterministic upstream-contract coverage rather than a live-source repair claim. Workflow commands intentionally return no synchronously consumable network result to script code.

### [2026-08-02] `preUpdateJs` could not refresh an expiring TOC URL
- **Problem**: Active sources calling `java.refreshTocUrl()` failed because the Java bridge did not expose that pre-update-only helper, leaving stale TOC URLs in use.
- **Fix**: Added a workflow-scoped JavaScript bridge and bound `refreshTocUrl` only for TOC pre-update evaluation. The callback reuses the existing book-detail workflow, rebuilds the shared typed context, and lets the refreshed TOC URL select the subsequent request target.
- **Verified**: Public TOC regressions prove detail-before-TOC ordering, refreshed metadata and TOC URL propagation, chapter parsing from the new target, and fail-closed detail errors with no stale TOC fetch. Targeted Analyzer, book, API, and conformance tests pass.
- **Affected**: `backend/internal/analyzer`, `backend/internal/book`, compatibility tracker, and `PLAN.md`.
- **Watch out**: The bridge is intentionally absent from ordinary rule evaluation. LC-008 remains open for upstream's distinct `java.reGetBook()` precise-search-then-detail behavior.

### [2026-08-02] TOC refresh ignored `preUpdateJs`
- **Problem**: NovelReader fetched the declared TOC immediately and never executed `ruleToc.preUpdateJs`, so scripts could not update book state or replace stale `book.tocUrl`/`book.bookUrl` values before the request.
- **Fix**: Execute the script once before the first TOC request with source/session and typed book context, synchronize direct book mutations back to the domain object, and select the mutated TOC/book URL as the fetch target. Script failures stop before network I/O with contextual errors.
- **Verified**: Public TOC workflow regressions cover pre-fetch ordering, direct TOC URL and book-field mutation, parsing from the new target, and fail-closed behavior; targeted `internal/book`, `internal/api`, and `internal/conformance` tests pass.
- **Affected**: `backend/internal/book`, compatibility tracker, and `PLAN.md`.
- **Watch out**: LC-008 remains in progress. Upstream's `java.refreshTocUrl()` and `java.reGetBook()` perform guarded detail refreshes and are not silently simulated by this direct-mutation slice.

### [2026-08-01] Explicit CSS rejected Jsoup positional selectors
- **Problem**: GoQuery/Cascadia rejected active source rules using Jsoup's zero-based sibling-index `:eq(n)`, `:lt(n)`, and `:gt(n)` selectors, including selectors applied before descendant traversal.
- **Fix**: Added a focused selector translation to equivalent standard `:nth-child(...)` syntax across CSS string, list, and element paths. Translation skips attribute brackets and quoted literals so selector-like data values remain unchanged.
- **Verified**: Public Analyzer regression covers intermediate `:eq`, ordered `:lt`/`:gt`, all-element sibling indexing, element extraction, and quoted-literal preservation; full `internal/analyzer` tests pass. Pinned-corpus inventory confirms other active explicit-CSS pseudo families are already accepted by Cascadia.
- **Affected**: `backend/internal/analyzer`, compatibility tracker, and `PLAN.md`.
- **Watch out**: LC-007 is complete for audited pinned-corpus contracts, not a guarantee of universal parity with selectors added by future Jsoup versions.

### [2026-08-01] Explicit CSS conflated `html` with inner HTML and lacked `all`
- **Problem**: `@css:...@html` returned only the first match's inner HTML without removing script/style descendants, while `@all` fell through to nonexistent attribute extraction.
- **Fix**: Added aggregate selected-element outer-HTML serialization. `html` removes script/style descendants before serialization; `all` preserves them. String-list extraction returns the aggregate as one item, matching upstream.
- **Verified**: Public Analyzer regression covers multiple selected elements, outer wrappers, one-item list shape, and script/style removal versus preservation; full `internal/analyzer` tests pass.
- **Affected**: `backend/internal/analyzer`, compatibility tracker, and `PLAN.md`.
- **Watch out**: LC-007 still includes Jsoup-only selector compatibility; element-selection APIs retain their existing outer-HTML chaining contract.

### [2026-08-01] Explicit CSS treated `textNodes` as an attribute
- **Problem**: `@css:...@textNodes` was routed through arbitrary HTML attribute extraction and returned empty.
- **Fix**: Added explicit-CSS direct text-node extraction that trims each node, removes blanks, excludes descendant elements, joins nodes within an element by newline, and preserves selected-element order.
- **Verified**: Public Analyzer regression covers multi-node parents, descendant exclusion, empty-element skipping, and string/list shapes; full `internal/analyzer` tests pass.
- **Affected**: `backend/internal/analyzer`, compatibility tracker, and `PLAN.md`.
- **Watch out**: This does not alter the existing Default-mode `textNodes` behavior; LC-007 still includes HTML/all and selector slices.

### [2026-08-01] Explicit CSS treated `ownText` as an attribute
- **Problem**: `@css:...@ownText` was routed through arbitrary HTML attribute extraction and returned empty instead of Jsoup direct-child text.
- **Fix**: Added focused explicit-CSS getter dispatch that reuses the existing Jsoup-compatible `ownText` implementation for string and list extraction, preserving ordered nonempty values.
- **Verified**: Public Analyzer regression covers descendant exclusion, empty-result skipping, string joining, and list output; full `internal/analyzer` tests pass.
- **Affected**: `backend/internal/analyzer`, compatibility tracker, and `PLAN.md`.
- **Watch out**: LC-007 remains in progress; `textNodes`, HTML/all distinctions, script/style handling, and Jsoup-only selectors are separate slices.

### [2026-08-01] File-source detail parsing ignored `downloadUrls`
- **Problem**: Type-3/file sources could parse ordinary metadata but discarded their actual downloadable EPUB/TXT/archive links, leaving no typed API contract for the source's primary resource.
- **Fix**: Added transient `Book.downloadUrls` domain/API fields and file-source detail extraction through Analyzer list rules, resolving links against the final response URL. Empty file-source link results now fail explicitly; ordinary source types retain TOC behavior. No database migration was added because upstream marks the field ignored/transient.
- **Verified**: Public book-info regression covers multiple relative links, ordinary-source exclusion, and empty-link failure. Full `internal/book`, `internal/api`, and `internal/conformance` tests pass; frontend production build succeeds.
- **Affected**: `backend/internal/booksource`, `backend/internal/book`, `frontend/src/api/client.ts`, compatibility tracker, `PLAN.md`, and the user-requested `AGENTS.md` documentation update included with this milestone.
- **Watch out**: This slice exposes parsed URLs only. Download/import execution and broader file-source lifecycle remain LC-019.

### [2026-08-01] Book detail enrichment ignored `canReName`
- **Problem**: Detail parsing always replaced name and author, and the add-book API discarded the known search identity before parsing. This made the source's `ruleBookInfo.canReName` policy unobservable.
- **Fix**: Added `GetBookInfoForBook` to enrich an existing book. Detail name/author now replace nonempty identity only when `canReName` is nonblank; empty identity is always filled. Add-book enrichment and source switching use the new workflow, while URL-only callers retain empty-book behavior.
- **Verified**: Public book-info regression covers preserve, replace, and fill-empty contracts; full `internal/book` and `internal/api` package tests pass.
- **Affected**: `backend/internal/book`, `backend/internal/api`, compatibility tracker, and `PLAN.md`.
- **Watch out**: Upstream models `canReName` as string presence, not a parsed Boolean expression. Reading workflows may separately disallow renaming; NovelReader's current enrichment callers allow the source policy.

### [2026-08-01] Source cookie persistence ignored `enabledCookieJar`
- **Problem**: Source sessions automatically captured and replayed response cookies even when a source explicitly set `enabledCookieJar: false`; omission also lacked an intentional upstream-default contract.
- **Fix**: Default omitted BookSource values to enabled behavior and gate automatic response-cookie synchronization in `SourceSession`. Applied the gate across normal HTTP, JavaScript HTTP, fingerprint, and WebView response paths while preserving explicit cookie operations and headers.
- **Verified**: Public detail → TOC regression passes for omitted/default-enabled and explicit-false sources; focused sourceexec, fetcher, analyzer, fingerprint, and WebView cookie tests pass.
- **Affected**: `backend/internal/book`, `backend/internal/sourceexec`, `backend/internal/fetcher`, `backend/internal/analyzer`, `backend/internal/fingerprint`, `backend/internal/webview`, compatibility tracker, and `PLAN.md`.
- **Watch out**: `enabledCookieJar` controls automatic response persistence, not source-declared Cookie headers or explicit script cookie APIs.

### [2026-08-01] `checkKeyWord` is validation metadata, not a search override
- **Problem**: The second-pass audit inferred that ordinary searches should replace the user's query with `ruleSearch.checkKeyWord`.
- **Finding**: Current upstream `BookSource.getCheckKeyword(default)` is called by `CheckSourceService` for source-health validation only. The debug UI exposes the value as a suggested query but still submits the selected text.
- **Decision**: Preserve the field losslessly and document its future source-validator contract; do not alter `Searcher.searchSource` or user query scoring.
- **Affected**: `docs/LEGADO_COMPATIBILITY_TASKS.md`, `PLAN.md`.
- **Watch out**: If a source-validation workflow is added, reproduce upstream's invalid-marker fallback for values containing `http`, `::`, `++`, or `--` rather than reusing ordinary search behavior.

### [2026-08-01] Second-pass compatibility queue needed durable tracking
- **Problem**: The verified second audit identified a long field/runtime compatibility queue whose details and important qualifications could be lost during context compaction if they remained only in conversation history.
- **Fix**: Added `docs/LEGADO_COMPATIBILITY_TASKS.md` with stable task IDs, priorities, completed commits, focused TDD seams, design gates, and the corrected audit wording for partial CSS, WebView, sessions, non-2xx behavior, defaults, and Java bridge support.
- **Affected**: `docs/LEGADO_COMPATIBILITY_TASKS.md`, `PLAN.md`.
- **Watch out**: Use the tracker as the implementation queue, but verify exact semantics against current vendored Legado before each field; broad media, login, payment, and durable-state tasks still require explicit design decisions.

### [2026-08-01] List metadata was discarded
- **Problem**: Search and Explore result rules could declare `wordCount` and `updateTime`, but the shared list parser never evaluated them even though `SearchResult` already exposed both fields.
- **Fix**: Added a public `ParseSearchResult` regression and included both fields in the existing ordered mutable-book extraction loop used by Search and Explore.
- **Affected**: `backend/internal/book/search.go`, `backend/internal/book/search_result_fields_test.go`, `PLAN.md`.
- **Watch out**: Keep field order deterministic because later JavaScript rules can read values extracted earlier in the same result.

### [2026-08-01] Content-level replacement was ignored
- **Problem**: `ruleContent.replaceRegex` was preserved in source JSON but chapter content returned without applying it, including after `nextContentUrl` page aggregation.
- **Fix**: Added a public `GetChapterContent` regression and apply the declared replacement rule once after all content pages are joined, using the existing Analyzer rule engine.
- **Affected**: `backend/internal/book/search.go`, `backend/internal/book/content_pagination_test.go`, `PLAN.md`.
- **Watch out**: Keep this distinct from inline selector replacement syntax; content-level replacement must remain post-aggregation.

### [2026-07-31] Verified v12 compatibility fixes started
- **Problem**: The v12 audit reported six seams, but implementation must distinguish actual Legado contracts from behavior that merely makes one live source pass.
- **Fix**: Verified contracts against upstream Legado source and the official rule documentation. Approved generic TDD work for non-2xx rule execution, crypto/login bridge APIs, arbitrary Default attributes, complete bracket indexing, and blank/deduplicated Default attributes. Rejected automatic mobile-UA routing as an overfit: raw 251 omits the header its current live mobile contract needs, while Legado's default remains desktop.
- **Affected**: `PLAN.md`, pending Analyzer/Explore/Java-bridge tests and implementation, v12 audit evidence correction.
- **Watch out**: Implemented generic public contracts rather than raw-specific branches: arbitrary terminal attributes, complete singleton/list/exclusion/inclusive-range/step/negative bracket grammar including root-child indexing and empty same-side overflow, nonblank deduplicated Default and CSS-compatible attribute extraction, received HTTP-status bodies with explicit warning diagnostics, nullable base64 bytes, AES/CBC PKCS padding crypto object methods, and source-scoped lenient login-header maps with direct properties and `.get`, automatic case-insensitive overlay in staged and Java HTTP requests, and login Cookie precedence over stale jar state. The first exact rerun caught that raw 17's simple `a[1]@href` and `a@title` forms take the CSS-compatible path; generic mode detection/value collection now covers those forms. Exact production returns 15/20/10 books for raws 197/163/17. The first exact rerun caught that raw 17's simple `a[1]@href` and `a@title` forms take the CSS-compatible path; generic mode detection/value collection now covers those forms. Exact production returns 15/20/10 books for raws 197/163/17. No hostname-based UA inference, no `title`-only getter patch, no `a[1]` special case, and no conversion of DNS/TLS/timeouts into parseable responses.

### [2026-07-31] Twelfth random Explore sample finds shared gaps
- **Problem**: The disjoint v12 sample had 24 initial non-passes or diagnostics and required sequential replay plus focused live/runtime diagnosis to distinguish reusable engine mismatches from source, network, and audit-environment failures.
- **Fix**: Confirmed 27 credible identities with 2,768 distinct books and initially reported six seams. Upstream verification corrected mobile-UA routing (raw 251) to an incomplete source contract, leaving five verified reusable seams across raws 197, 163, and 17: outer non-2xx rule-driven AJAX, missing crypto/login bridge APIs, arbitrary Default attribute getters, singleton bracket indexes, and blank-skipping multi-match attributes. Classified all remaining outcomes without changing behavior.
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v12-2026-07-31.json`, `testdata/booksource/audits/explore/explore-live-audit-v12-2026-07-31.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Raw 714 is audit infrastructure, not upstream DNS: public DNS plus the exact rules returns 30 books. Raw 734 is a credible bounded result with an accurate 2,000-of-2,432 truncation diagnostic. Fixes require separate approval.
...
- **Problem**: V11 confirmed four reusable compatibility gaps across regex replacement, Default ranges, Explore timeouts, and structured JSON per-result JavaScript.
- **Fix**: Added green public-seam regressions and minimal implementations for Java identity escapes in inline replacement, Default `[start:end]` ranges, configurable Explore-only source timeout, and structured JSON per-result JavaScript. Timeout configuration is `SearcherLimits.ExploreSourceTimeout` plus positive integer `EXPLORE_SOURCE_TIMEOUT_SECONDS`, default 30 seconds; search's ten-second source deadline remains unchanged.
- **Affected**: `PLAN.md`; planned analyzer, book, config, server, tests, documentation, and v11 rerun evidence.
- **Watch out**: The first exact rerun proved that unit green was insufficient for two slices: `[8:]` also needed Default mode detection, and the Explore context timeout was still undercut by the cloned HTTP client's shorter deadline. Both are covered through public seams and exact production reruns. Raw 701/54/220/443 now return 30/180/100/12 distinct books; preserve structured HTML behavior and do not patch stale/upstream sources.

### [2026-07-23] Eleventh random Explore sample finds four shared gaps
- **Problem**: The disjoint v11 sample had 12 initial non-passes and needed sequential replay plus focused diagnosis to separate reusable engine mismatches from source and network failures.
- **Fix**: Confirmed 39 credible identities with 1,142 distinct books and four reusable seams: inline replacement Java identity escapes (701), Default positional ranges (54), the fixed ten-second source deadline (220), and structured JSON result access in per-result JavaScript (443). Classified the remaining outcomes as upstream HTTP/DNS, blocked/auth, or site drift without changing behavior.
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v11-2026-07-23.json`, `testdata/booksource/audits/explore/explore-live-audit-v11-2026-07-23.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Treat sequential raw 423 as authoritative credible output; do not promote raw 575's variable upstream status to parser evidence; fixes require separate approval.
...
- **Problem**: Raw 927's complete fallback `ruleSearch` uses `class.mh-item@a@text`; each card has an empty cover link before its named title link, but NovelReader read only the first match and dropped every book.
- **Fix**: Public-seam TDD confirmed whole-object Explore fallback already worked. Updated the shared Default string path to collect all non-empty matched getter values and join them with newlines, matching Legado; attribute getters also de-duplicate repeated values. Added exact live-shaped Analyzer and Explore regressions.
- **Affected**: `backend/internal/analyzer/modes_default.go`, `backend/internal/analyzer/default_live_compat_test.go`, `backend/internal/book/explore_compat_test.go`, `PLAN.md`.
- **Watch out**: Preserve per-element order and repeated text values; only attribute getter values are de-duplicated. Do not field-merge `ruleExplore` and `ruleSearch` or weaken URL-pattern enforcement. Fresh exact-identity raw 927 production rerun returns 28 distinct books in `连载`; the earlier 21-book `性感` observation belonged to duplicate raw 317 and is superseded.

### [2026-07-23] Partial Explore rule object bypasses whole-search fallback
- **Problem**: Raw 927 has blank `ruleExplore.bookList` and a complete `ruleSearch`; the live page contains 28 valid cards, but production returns zero because the partial Explore object is treated as complete.
- **Fix**: Audit only: reproduced initial and sequential empties, confirmed current HTTP 200 cards and valid search selectors, and classified the shared rule-selection mismatch without changing behavior.
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v10-2026-07-23.json`, `testdata/booksource/audits/explore/explore-live-audit-v10-2026-07-23.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: A future fix must select the complete `ruleSearch` object when Explore `bookList` is blank; do not field-merge partial objects or weaken stale `bookUrlPattern` enforcement.
...
- **Problem**: Live raw 66 lost later Default parents and rejected Java Unicode regex escapes; raw 57 rejected dotted JSON array wildcards; raw 581 missed simple wildcard predicates over root arrays.
- **Fix**: Translate valid Java `\\uHHHH` escapes in the shared regex compiler, clone the empty goquery traversal accumulator, normalize `.[*]` tokens outside quoted JSONPath strings, and extend the narrow ordered wildcard-filter path to root arrays. Added public Analyzer regressions for each behavior and malformed-input protection remains green.
- **Affected**: `backend/internal/analyzer/modes_regex.go`, `backend/internal/analyzer/regex_conformance_test.go`, `backend/internal/analyzer/modes_default.go`, `backend/internal/analyzer/modes_json.go`, `backend/internal/analyzer/default_live_compat_test.go`, `PLAN.md`.
- **Watch out**: Keep Unicode rewriting limited to well-formed four-hex Java escapes, dotted wildcard normalization outside quotes, and wildcard-filter support limited to simple letter/digit/underscore/hyphen property existence; comparisons, nested paths, bracket keys, and richer JSONPath stay delegated. Live reruns return 20 books for raw 66, 14 for raw 57, and 51 for raw 581.

### [2026-07-22] Ninth Explore shared fixes approved
- **Problem**: Raw 66, 57, and 581 expose four reusable rule-engine mismatches in current live contracts.
- **Fix**: User approved public-seam TDD fixes for Java Unicode regex escapes, multi-parent Default traversal, dotted JSON array wildcards, and root-array wildcard predicates, with the Legado rule reference used as contract guidance.
- **Affected**: `PLAN.md`; forthcoming analyzer/book regressions and minimal shared parser changes.
- **Watch out**: Preserve strict malformed-input handling, existing URL-pattern filtering, and richer JSONPath delegation; do not patch individual source records.

### [2026-07-22] Ninth unrestricted Explore audit started
- **Problem**: V8 found no shared engine gap, but 373 enabled Explore identities remain unsampled and another independent batch is needed to increase confidence.
- **Fix**: User selected an unrestricted deterministic 50-source batch using seed `NovelReader-explore-random-v9-2026-07-22`, stable raw-index/source-URL identities, exclusion of all 350 prior samples, four-client production execution, and sequential confirmation of every non-pass.
- **Affected**: `PLAN.md`; forthcoming `testdata/booksource/audits/explore/explore-live-audit-v9-2026-07-22.{json,md}`.
- **Watch out**: Freeze before live requests, never substitute after observing outcomes, and keep this cycle audit-only.

### [2026-07-22] Ninth unrestricted Explore audit found four shared compatibility seams
- **Problem**: Seventeen of 50 frozen identities did not produce credible books, including populated current contracts that failed at reusable Default, regex, and JSONPath boundaries.
- **Fix**: Sequentially confirmed every non-pass and reduced raw 66 to Java Unicode-regex plus multi-parent shorthand traversal mismatches, raw 57 to dotted array wildcard rejection, and raw 581 to a root-array wildcard-predicate gap. Classified all other outcomes by direct DNS/HTTP/body/script evidence without modifying production behavior.
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v9-2026-07-22.json`, `testdata/booksource/audits/explore/explore-live-audit-v9-2026-07-22.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Keep fixes generic and syntax-scoped. Do not bypass stale `bookUrlPattern` values, repair malformed source JavaScript, or reinterpret upstream 403/404/database failures as parser behavior.

### [2026-07-22] Eighth unrestricted Explore audit started
- **Problem**: The last unrestricted sample still exposed several reusable compatibility gaps, and 423 enabled Explore identities remain unsampled after seven batches.
- **Fix**: User selected an unrestricted deterministic 50-source batch using seed `NovelReader-explore-random-v8-2026-07-22`, stable raw-index/source-URL identities, exclusion of all 300 prior samples, four-client production execution, and sequential confirmation of every non-pass.
- **Affected**: `PLAN.md`; forthcoming `testdata/booksource/audits/explore/explore-live-audit-v8-2026-07-22.{json,md}`.
- **Watch out**: Freeze before live requests, never substitute after observing outcomes, and do not change source/parser behavior during this audit-only cycle.

### [2026-07-22] Eighth unrestricted Explore audit found no shared engine gap
- **Problem**: Sixteen of 50 frozen identities did not produce credible books in the initial production pass and required evidence-based confirmation rather than being labeled parser failures.
- **Fix**: Replayed every non-pass sequentially and separated 2 DNS failures, 3 upstream HTTP failures, 2 blocked/auth outcomes, 6 stale source contracts, 1 malformed source script, and 2 drifted sites. The other 34 identities returned 1,045 distinct books; no reusable engine mismatch remained.
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v8-2026-07-22.json`, `testdata/booksource/audits/explore/explore-live-audit-v8-2026-07-22.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Do not weaken `bookUrlPattern` enforcement for raw 859/92/643/680 or synthesize later-page success for raw 530; those would turn stale/upstream contracts into incorrect shared behavior.

### [2026-07-23] Review hardened seventh Explore fixes across workflows
- **Problem**: Review found executable headers remained Explore-only, `ownText()` lacked Jsoup whitespace semantics, encoded selector values could alter reserved query characters, JSON wildcard order depended on Go map iteration, and adjacent source metadata inventory omitted raw 281.
- **Fix**: Reuse bounded executable-header evaluation across search/detail/TOC/content and pagination; normalize direct-text whitespace/`<br>` in `ownText()`; select encoded page tokens without decoding; stream root JSON object entries in document order; add a public two-page Explore selector regression; and recompute the complete metadata inventory.
- **Affected**: `backend/internal/book/search.go`, `backend/internal/analyzer/js.go`, `backend/internal/analyzer/jsoup_conformance_test.go`, `backend/internal/analyzer/urlbuilder.go`, `backend/internal/analyzer/urlbuilder_conformance_test.go`, `backend/internal/analyzer/modes_json.go`, `backend/internal/analyzer/default_live_compat_test.go`, `backend/internal/book/explore_compat_test.go`, `testdata/booksource/audits/explore/explore-live-v7-fixes-rerun-2026-07-23.json`, `PLAN.md`.
- **Watch out**: Evaluate source headers once per workflow/session and reuse the map; do not execute unprefixed literals. Cross-workflow tests cover search and the detail→TOC→content pagination chain. Preserve root JSON property order and encoded query bytes in future compatibility changes; ordered wildcard parsing validates the complete JSON document before returning records.

### [2026-07-23] Seventh Explore fixes pass live and adjacent-family verification
- **Problem**: The four audited fixes required full production proof because the first repaired seam could reveal a later shared boundary in the same source.
- **Fix**: Fresh production reruns return raw 177=1, raw 5=21, raw 503=150, and raw 288=30 distinct books; raw 288 also generates 118 categories. Recorded all adjacent corpus consumers for source metadata, executable headers, JSON wildcard filters, `ownText()`, and mutable book context, while relying on deterministic public regressions rather than making unsupported live claims.
- **Affected**: `testdata/booksource/audits/explore/explore-live-v7-fixes-rerun-2026-07-23.json`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Raw 5's live response includes at least one grouped result whose metadata contains newline-joined values; this is source/API behavior, not a reason to split or rewrite results in the shared fix.

### [2026-07-22] Jsoup elements support Java-compatible ownText()
- **Problem**: Valid catalog JavaScript calls `element.ownText()`, but the bridge exposed `ownText` as a string property and included descendant text rather than only direct text nodes.
- **Fix**: Expose `ownText` as a method that concatenates direct text-node children, matching Jsoup's contract. Corpus inspection found two method consumers and no property consumers; existing collection methods, bounds, and enumeration regressions remain green.
- **Affected**: `backend/internal/analyzer/js.go`, `backend/internal/analyzer/jsoup_conformance_test.go`, `PLAN.md`.
- **Watch out**: `text()` still includes descendant text; `ownText()` intentionally excludes descendants, normalizes whitespace, treats direct `<br>` as a separator, and trims the result. Keep the two APIs distinct.

### [2026-07-22] JSON object wildcard filters select matching values
- **Problem**: The installed JSONPath engine returns no elements for valid Legado list rules such as `$.*[?(@.novelName)]` over object-valued maps, while the unfiltered `$.*` returns the records.
- **Fix**: Add a narrow elements-only compatibility path for root object wildcard existence filters, preserving the installed JSONPath engine for every other expression. A public Analyzer regression covers the exact two-branch connector used by raw 503.
- **Affected**: `backend/internal/analyzer/modes_json.go`, `backend/internal/analyzer/default_live_compat_test.go`, `PLAN.md`.
- **Watch out**: The compatibility path accepts only a simple root `$.*[?(@.key)]` existence predicate; richer filters remain delegated to the JSONPath library rather than receiving a partial custom evaluator. Ordered streaming preserves root document order instead of ranging over a Go map. Live verification showed raw 503's returned object values are arrays, so the narrow path flattens one array level while preserving direct object values.

### [2026-07-23] Search-result field JavaScript receives earlier fields
- **Problem**: After raw 5's executable header began reaching the live endpoint, result extraction failed because its `bookUrl` JavaScript reads `book.kind`, but search-result analyzers did not expose a mutable `book` context and evaluated `bookUrl` before `kind`.
- **Fix**: Give each result analyzer one source-scoped mutable book map, update it after each extracted field, and evaluate `bookUrl` after the metadata fields it may depend on. A public Explore regression derives the URL from an earlier normalized `kind` value.
- **Affected**: `backend/internal/book/search.go`, `backend/internal/book/explore_compat_test.go`, `PLAN.md`.
- **Watch out**: Field order is now an explicit compatibility contract; keep identity URL last so source JavaScript can consume previously extracted metadata without sharing state across results.

### [2026-07-22] Encoded Explore page selectors are expanded before requests
- **Problem**: Raw 503's JSON filtering worked on its populated live body, but its URL encoded `<0,150,300>` as `%3C0%2C150%2C300%3E`; URL building only expanded literal selectors and sent the encoded selector to an upstream endpoint that rejected the string offset.
- **Fix**: Expand isolated percent-encoded page selectors without decoding or re-encoding the surrounding URL. Added a URL-builder regression alongside existing literal/body selector tests.
- **Affected**: `backend/internal/analyzer/urlbuilder.go`, `backend/internal/analyzer/urlbuilder_conformance_test.go`, `PLAN.md`.
- **Watch out**: Only isolated encoded `<...,...>` segments are split; the selected token remains encoded byte-for-byte so reserved query characters cannot change meaning. A public Explore two-page regression proves offsets 0 and 150 reach transport.

### [2026-07-22] Explore evaluates executable source headers
- **Problem**: Raw 5 declares a valid `@js:` header that depends on source metadata, but literal-only parsing discarded it and sent the request without required headers.
- **Fix**: Evaluate only explicitly prefixed `@js:` source headers with the bounded Explore context, source metadata, cancellation, and session state; parse the returned value through the existing fail-closed literal header parser. Public Explore tests cover executable and single-quoted literal headers.
- **Affected**: `backend/internal/book/rules.go`, `backend/internal/book/explore_page.go`, `backend/internal/book/explore_script.go`, `backend/internal/book/explore_compat_test.go`, `PLAN.md`.
- **Watch out**: Arbitrary malformed literals are never evaluated and retain the legacy no-header fallback; only explicit `@js:` execution/returned-map failures are loud. Header JavaScript obeys the source timeout and cannot replace source helper methods.

### [2026-07-22] Rule JavaScript receives complete source metadata
- **Problem**: Valid Explore result JavaScript could read source helper methods but not typed source fields such as `bookSourceComment`, causing raw 177 to fail despite populated live data.
- **Fix**: Build source metadata from the canonical lossless `BookSource` JSON, merge it beneath existing source helper methods in the JavaScript bridge, and propagate it to catalog and result analyzers. A public Explore page regression reads `source.bookSourceComment`; pooled runtime and source-state behavior remain covered.
- **Affected**: `backend/internal/analyzer/analyzer.go`, `backend/internal/analyzer/js.go`, `backend/internal/book/context.go`, `backend/internal/book/search.go`, `backend/internal/book/explore_script.go`, `backend/internal/book/explore_compat_test.go`, `PLAN.md`.
- **Watch out**: Helper method names override imported metadata keys so a source cannot replace session/cookie behavior; an explicit regression pins this precedence. Metadata remains source-scoped and must not leak between pooled runtimes.

### [2026-07-22] Seventh Explore audit found four new shared compatibility seams
- **Problem**: A targeted 25-source batch disjoint from all first 275 identities found four valid live contracts that fail at reusable JavaScript/header/JSON/Jsoup boundaries.
- **Fix**: Sequentially reran all 13 initial non-passes and confirmed exact live bodies plus reduced production-runtime mismatches for `source.bookSourceComment` (raw 177), `@js:` source headers (5), JSON object wildcard/filter selection (503), and Jsoup `ownText()` (288). Classified all other outcomes without changing source or engine behavior.
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v7-2026-07-22.json`, `testdata/booksource/audits/explore/explore-live-audit-v7-2026-07-22.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Raw 194 is cookie/CSRF-dependent, raw 666/913 are Cloudflare-blocked, and raw 523 has malformed source JavaScript; do not patch the engine to make those contracts pass.

### [2026-07-22] Review narrowed sixth Explore fixes to exact compatibility contracts
- **Problem**: Initial review found the lenient map parser accepted missing commas, Jsoup `eq(-1)` behaved like jQuery, and optional Explore fields swallowed arbitrary parser/script failures rather than only missing/null JSON traversal.
- **Fix**: Require commas between lenient map entries, make negative `eq()` fail while positive out-of-range returns empty, restore strict evaluation for every Explore field, and classify traversal through JSON null as `ErrNoElements`. Added regressions for malformed maps, Jsoup bounds, nullable optional success, and optional script failure.
- **Affected**: `backend/internal/analyzer/rulevars.go`, `backend/internal/analyzer/lenientmap_test.go`, `backend/internal/analyzer/js.go`, `backend/internal/analyzer/jsoup_conformance_test.go`, `backend/internal/analyzer/modes_json.go`, `backend/internal/book/search.go`, `backend/internal/book/explore_compat_test.go`, `PLAN.md`.
- **Watch out**: Missing/null JSON paths are absence; syntax, script, cancellation, and other parser failures remain loud for optional as well as required fields. The exported header parser now accepts quoted string keys/values only and is deliberately separate from the broader internal `@put` grammar.

### [2026-07-22] Sixth Explore fixes pass live and adjacent-corpus verification
- **Problem**: The four shared seam fixes required production proof against all six audited identities and adjacent source scripts, without treating an empty current page as proof of engine failure.
- **Fix**: Fresh production reruns return raw 112/311=10 distinct books each, 749=20, and 757=8; raw 933/301 now generate 26/11-entry catalogs and reach empty live pages without engine diagnostics. Individually replayed all 21 corpus identities using the affected source/Jsoup methods; none retains those missing-method failures, while five unrelated boundaries remain explicitly unclaimed.
- **Affected**: `testdata/booksource/audits/explore/explore-live-v6-fixes-rerun-2026-07-22.json`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Raw 933/301 catalog generation is repaired, but their current first pages are empty; do not claim books without live evidence. Adjacent raw 37/134/216 still expose separate bootstrap/bridge gaps.

### [2026-07-21] Lenient source headers restored redirected Explore requests
- **Problem**: Raw 112/311 returned 403 only through full Explore even though the same fingerprint transport and imported headers returned 200 directly; tracing showed the Searcher sent an empty header map.
- **Fix**: Reused the analyzer's existing lenient quoted-map parser for source headers after strict JSON fails, exported through a narrow string-map function. A public Explore page test proves a single-quoted source header reaches transport; strict JSON behavior remains first.
- **Affected**: `backend/internal/analyzer/lenientmap.go`, `backend/internal/book/rules.go`, `backend/internal/book/explore_compat_test.go`, `PLAN.md`.
- **Watch out**: Header values remain strings and malformed maps still fail closed; do not eval arbitrary source header text.

### [2026-07-21] Sixth Explore shared script and extraction gaps fixed
- **Problem**: Valid Explore catalogs could not call `source.setVariable`, Jsoup selections lacked Java-compatible `size()` and `eq(index)`, and a null optional JSON field aborted an otherwise valid Explore page.
- **Fix**: Added `setVariable` as the existing source-variable setter alias, implemented non-enumerable Jsoup `size()`/`eq()` methods while preserving indexed access, and kept strict field errors only for required `name`/`bookUrl` during Explore extraction. Public production-boundary regressions cover each symptom and preserve required-field failure plus JSoup iteration behavior.
- **Affected**: `backend/internal/analyzer/js.go`, `backend/internal/analyzer/jsoup_conformance_test.go`, `backend/internal/book/search.go`, `backend/internal/book/explore_script_test.go`, `backend/internal/book/explore_compat_test.go`, `PLAN.md`.
- **Watch out**: `size` now follows Jsoup's method contract rather than a numeric property; corpus consumers using native `Map.size` are unrelated. Optional field failures are ignored only in Explore's strict result mode; required identity fields remain strict.

### [2026-07-21] Sixth Explore audit found four shared compatibility seams
- **Problem**: A deterministic unrestricted 50-source sample disjoint from all first 225 audit identities found six engine-gap identities despite the previous parser and Java-helper fixes.
- **Fix**: Sequentially reran every initial non-pass and narrowed candidates with exact imported contracts, source headers, direct live bodies, and production-runtime diagnostics. Confirmed redirected custom-header failures (raw 112/311), missing `source.setVariable` (933), Jsoup collection `size()`/`eq()` gaps (749/301), and nullable optional JSON extraction aborting a valid eight-book page (757); classified all other outcomes without changing behavior.
- **Affected**: `.agents/skills/explore-live-audit/SKILL.md`, `testdata/booksource/audits/explore/explore-live-audit-v6-2026-07-21.json`, `testdata/booksource/audits/explore/explore-live-audit-v6-2026-07-21.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Raw 22, 222, 406, and 678 have live rows but fail their imported host/pattern contracts; do not weaken URL validation to make stale sources pass. Raw 112/311 need transport-level proof before any header workaround.
...
- **Fix**: Retained the original source JSON, restored it exactly when unchanged, merged preserved fields after typed updates, and persisted the raw JSON in the canonical `book_sources.source_json` column.
...
- **Affected**: `backend/internal/booksource/json.go`, `backend/internal/booksource/json_test.go`, `backend/internal/conformance/explore_fixture_test.go`, `testdata/booksource/conformance/explore/explore-sources.json`, `testdata/booksource/README.md`, `PLAN.md`.
...
- **Affected**: `PLAN.md`, `testdata/booksource/audits/explore/explore-live-audit-2026-07-18.md`.
...
- **Affected**: `backend/internal/analyzer/ruleparser.go`, `backend/internal/analyzer/default_live_compat_test.go`, `backend/internal/book/search.go`, `backend/internal/book/explore_url_pattern_test.go`, `testdata/booksource/audits/explore/explore-live-audit-2026-07-18.json`, `testdata/booksource/audits/explore/explore-live-audit-2026-07-18.md`, `testdata/booksource/audits/explore/explore-live-audit-priority-fix-rerun-2026-07-19.json`.
...
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v2-2026-07-19.json`, `testdata/booksource/audits/explore/explore-live-audit-v2-2026-07-19.md`, `PLAN.md`.
...
### [2026-07-19] Local build verification hit host tool configuration gaps
- **Problem**: Local Vite build initially lacked Rollup's Linux optional binary, and Docker Buildx could not write its default activity directory or invoke the host credential helper in the sandbox.
...
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v3-2026-07-19.json`, `testdata/booksource/audits/explore/explore-live-audit-v3-2026-07-19.md`, `PLAN.md`.
...
### [2026-07-19] V3 fixes pass deterministic and live cross-verification
- **Problem**: Live upstream variability can make a parser repair look failed or successful without distinguishing transport/body drift from engine behavior.
- **Fix**: Ran full backend/race/frontend/build/Docker checks, reran all six raw identities through a fresh isolated engine, and independently counted their live rows with Playwright. Five identities now return expected bounded results; raw 788 remains externally variable while its deterministic analyzer contract passes.
- **Affected**: `testdata/booksource/audits/explore/explore-live-v3-fixes-rerun-2026-07-19.json`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Do not claim raw 788 as a live pass until production transport receives the same 20-row body Playwright sees; live counts are evidence, not regression fixtures.
### [2026-07-19] Review found duplicate HTML fragment contexts
- **Problem**: Analyzer serialization and the JavaScript JSoup adapter independently encoded the same table/select fragment wrappers, risking divergent selected roots.
- **Fix**: Moved fragment contextualization into one focused helper used by both serialization and JSoup parsing without changing observable behavior.
- **Affected**: `backend/internal/analyzer/html_selection.go`, `backend/internal/analyzer/analyzer.go`, `backend/internal/analyzer/js.go`, `PLAN.md`.
- **Watch out**: Any new fragment family must preserve both serialized validity and the original selected root for JS methods.
### [2026-07-21] Fourth random Explore sample finds two remaining shared gaps
- **Problem**: A fresh 50-source sample disjoint from all three prior batches produced repeatable empty results for live pages where list selection succeeded but nested positioned fields did not.
- **Fix**: Reran every non-pass sequentially, checked representative failures in Playwright, replayed three captured live bodies through production analyzers, and isolated raw 830's lost table-row context plus raw 615's `img.0@title` traversal failure without changing production behavior.
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v4-2026-07-21.json`, `testdata/booksource/audits/explore/explore-live-audit-v4-2026-07-21.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Preserve CSS attribute getters while fixing positioned Default traversal, and restore fragment context at the shared analyzer boundary rather than patching named sources.
### [2026-07-21] Table fragments and positioned attributes dropped valid Explore fields
- **Problem**: Raw 830 selected live `<tr>` rows but nested `td` rules reparsed them without table context, while raw 615 selected live cards but interpreted `img.0@title` as CSS instead of positioned Default traversal.
- **Fix**: Reuse shared HTML selection context in all Default query paths, preserve the contextualized fragment root for current-element getters, and classify/parse positioned bare elements with custom attribute getters as Default rules.
...
- **Watch out**: Custom attributes on simple unpositioned selectors remain CSS getters; contextual wrappers must never replace the selected row/cell/option as the current element.
### [2026-07-21] Fourth-audit parser fixes pass deterministic and live verification
- **Problem**: Live verification needed to distinguish repaired analyzer behavior from a separate stale source filter on raw 830.
...
- **Affected**: `testdata/booksource/audits/explore/explore-live-v4-fixes-rerun-2026-07-21.json`, `testdata/booksource/README.md`, `PLAN.md`.
...
### [2026-07-21] Final targeted Explore audit finds two focused gaps
- **Problem**: A 25-source sample disjoint from all prior 200 identities found raw 84 collapsing 20 live table rows through `tbody@tr` and raw 565 failing a valid generated catalog on missing `java.t2s`.
- **Fix**: Reproduced every non-pass sequentially; verified raw 84 against a captured live body and Playwright, and proved raw 565 generates 470 categories when only `java.t2s(t)` is replaced by identity. Classified the six other non-passes as upstream/source-contract/site outcomes.
- **Affected**: `testdata/booksource/audits/explore/explore-live-audit-v5-2026-07-21.json`, `testdata/booksource/audits/explore/explore-live-audit-v5-2026-07-21.md`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Fix bare-child routing without breaking CSS attribute getters, and implement only the documented focused conversion helper; do not use either gap to patch named sources.

### [2026-07-21] Final targeted gaps fixed at shared seams
- **Problem**: `tbody@tr` was routed as a CSS attribute getter and returned one parent instead of all rows, while valid generated catalogs failed because source JavaScript could not call Legado's `java.t2s(text)` helper.
- **Fix**: Route only structurally unambiguous bare HTML parent/child pairs through Default traversal, preserving ambiguous CSS getters; expose a lazy, shared pure-Go OpenCC `t2s` converter through the Java bridge. Public Analyzer and Explore catalog regressions cover conversion, simplified-text identity, traversal, and getter preservation.
- **Affected**: `backend/internal/analyzer/ruleparser.go`, `backend/internal/analyzer/chinese.go`, `backend/internal/analyzer/js.go`, `backend/internal/analyzer/default_live_compat_test.go`, `backend/internal/book/explore_script_test.go`, `backend/go.mod`, `backend/go.sum`, `PLAN.md`.
- **Watch out**: Ambiguous general forms such as `div@a` remain CSS-compatible; sources should use `tag.a`, positioning, exclusion, or explicit mode for general child traversal. OpenCC dictionaries increase the compiled binary and must remain initialized once.
### [2026-07-21] Final Explore fixes pass live and corpus verification
- **Problem**: The shared fixes required proof against both named live failures and adjacent corpus consumers rather than only reduced tests.
- **Fix**: Fresh production transport returns raw 84=18 distinct books and raw 565=470 categories/458 selectable entries. Replayed all nine `java.t2s` corpus identities individually; none retains the missing-`t2s` failure, and valid generated scripts produce 26–470 categories. Recorded the exact remaining unrelated boundaries instead of claiming those sources repaired.
- **Affected**: `testdata/booksource/audits/explore/explore-live-v5-fixes-rerun-2026-07-21.json`, `testdata/booksource/README.md`, `PLAN.md`.
- **Watch out**: Raw 84's two non-numeric slugs remain filtered by its source pattern; raw 21/134 expose separate Java bridge gaps and are not part of this fix.
### [2026-07-21] Final fix review removed adjacent classifier widening
- **Problem**: Review found the first condition rewrite also let explicit child suffixes route through Default without the prior element-parent guard, beyond the approved structural-pair fix.
- **Fix**: Restored the existing parent guard for explicit `tag.`/position/exclusion children while retaining only `isUnambiguousBareChild` as the new routing path; reran focused deterministic, race, and vet checks.
- **Affected**: `backend/internal/analyzer/ruleparser.go`, `PLAN.md`.
- **Watch out**: Future parser changes must separate intended new syntax from incidental classifier broadening.
