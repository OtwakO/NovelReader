...
Current state: feature tip `2128d4e` integrated into local `main` with a conflict-free `--no-ff` merge; the feature branch is retained. Nothing was pushed or deployed. Next action: separately scope the cache/prefetch work from its accepted design; no implementation was included in this wrap-up. A local merge does not change reader homes; before any later deployment, preserve incompatible older homes/backups. If a later integration rollback is required, revert the merge with its first parent rather than resetting shared history; code rollback is not a reader-schema downgrade.
