...
The concrete design must cover both ordinary source URL/options expressions and inline `data:image/` resources: `book/image.go` supports inline payloads up to its existing 10 MiB image limit. Do not assume every descriptor fits in a GET URL. The current document contract is href-only (`reading.ResourceReference` / `frontend/src/api/content-resource.ts`); changing resource transport needs coordinated backend and renderer changes. The existing cover-reference signing key is process-local (`api/cover_display.go`), so it cannot supply persistent chapter-resource validity across restarts.
