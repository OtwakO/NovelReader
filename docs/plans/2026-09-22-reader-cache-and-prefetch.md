...
Portable-cache exclusion, reader-home identity, immutable resources and the backend cache-first/shared-request/Refresh milestone are implemented. Next, implement client memory/IndexedDB persistence as one cohesive lifecycle/retention/invalidation milestone. Keep navigation feedback and two-target preparation as the following cohesive milestone rather than unrelated helper commits.

1. Put memory and IndexedDB behind one chapter-cache owner. Apply the accepted identity, three-book/five-chapter window and canonical-document policy; preserve backend remaining freshness rather than restarting TTL on copies.
2. Gate persistent writes and cross-tab invalidation transactionally across logout/account change, removal, restore and interpretation changes. Validate reader entry before reuse; do not treat home generation as a secret or rely only on tab-local request cancellation.
3. Preserve explicit Refresh bypass and graceful storage failure. Unavailable-image responses have zero reusable freshness and must not produce immediate renewal loops. The header remains optional for older clients; deployments must reload old reader tabs for the new Refresh protocol.
