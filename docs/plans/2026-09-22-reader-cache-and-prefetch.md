...
Portable-cache exclusion and storage/API/client home identity are complete through `005623d`. The resource behavior and shared SQLite storage are accepted. The complete resource-store owner/configuration/lifecycle checkpoint is verified within the unfinished backend milestone. Next, settle context capture, source execution identity/ownership and freshness interfaces, then implement image publication/resolution together with backend cache-first/Refresh. Do not mistake the storage checkpoint for a completed cached-reading feature or split the remaining work into unrelated helper milestones. Keep later client persistence and navigation/two-target preparation as cohesive milestones. Home generation is public identity, not a cryptographic secret. Before implementing affected interfaces, settle and record:
