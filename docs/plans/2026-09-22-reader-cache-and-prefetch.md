...
| BookSource responses expose `freshForMs`; client memory uses a conservative request-start deadline. Explicit Refresh input is still absent | `backend/internal/reading/service.go`, `document.go`, `backend/internal/api/reader_handlers.go`, `frontend/src/api/reader.ts` |
...
- [ ] Settle the remaining shared-request/Refresh ownership contract; storage/budgets and image-wire compatibility are recorded.
...
- [x] Implement image-bundle identity/lifetime and narrow session execution ownership. Backend cached-reading request sharing remains in the next increment.
