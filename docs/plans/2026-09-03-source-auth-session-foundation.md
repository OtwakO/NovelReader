...
- sanitized reproduction captured all eight current route probes. From the assistant sandbox, the seven HTTPS roots and probe paths returned edge/upstream HTTP 502 (one root returned 520), while the direct-IP route refused/timeout; however the user can reach the HTTPS roots and the controlled WebView can fetch the source's user-status page. This proves environment-dependent behavior and does not establish route unavailability. The unresolved comparison is normal WebView navigation versus the `data:`-document mediator inside the user's deployment;
...
Ask the user to apply/save ⚙️ 书源设置 once after the source update, then retest Explore on the current branch. For route checks, add or use bounded deployment-side diagnostics that report safe transport classification and upstream status for normal WebView versus mediated `data:` requests; sandbox HTTP 502 responses are not authoritative for the user's network. Treat active-device count separately because authenticated Search and reading already prove general qttoken propagation.
