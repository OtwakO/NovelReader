...
- This is an **internal preparation model**, not the frozen version-2 wire contract. It remains under `internal/epub` until the renderer proof establishes the final shared shape; existing version-1 endpoints are unchanged. No unvalidated binding is exposed for reading. Link/anchor binding over a completed section index, image-byte validation and per-section readability reassessment are implemented as described below. Auxiliary-document discovery, whole-book admission and staged/streamed preparation remain next.
...
Continue checkpoint 1 with bounded staged preparation orchestration, including referenced auxiliary-document discovery, whole-book admission and labeled spine-derived navigation fallback, then the version-2 prose contract/renderer proof. Section normalization, image validation and target binding over registered sections are implemented; they do not yet form a preparation entry point or authorize publication. Orchestration must reuse the archive index, process sections incrementally and retain only bounded indexes/staged results, not a whole-book content tree. Register final section ordinals/anchors before binding staged sections so forward links and note back-links resolve without retaining every tree; do not turn TOC order or link-discovery order into main reading order. Hierarchical NCX/nav inspection is implemented but is not a prepared catalog. The inventory deliberately is not a complete validity/admission check. The renderer proof must exercise links, notes, validated image dimensions/orientation and unsupported-image placeholders; test the accepted auxiliary-bookmark/main-resume isolation before storage DDL. Measure full preparation and section costs, not just metadata inspection. Confirm fresh-data schema cutover before checkpoint 2; do not reopen dependency selection without new evidence.

## Verification

Passed from `backend/`:

- `go test ./internal/epub -count=1` and `go test -race ./internal/epub -count=1`: synthetic package/order/reference cases, archive and XML boundaries, cancellation, EPUB 2 declarations/entities/UTF-16, NCX/nav hierarchy and selection, mixed labels/group headings, repeated-document fragments, spine independence/non-linear targets, unavailable targets, optional-navigation diagnostics, base overrides, fatal navigation byte/depth/entry/cumulative limits, fixed-layout declarations, font obfuscation versus encryption, spine fallback/cycle/missing-file handling, consistent versus conflicting resource aliases, semantic structure/whitespace, opaque/private reference separation, safe/inactive links/images, ambiguous anchors, SVG raster wrappers, bounded section normalization, complete static-image validation versus false declarations/corrupt pixels, animation/dimension/byte limits, repeated-resource deduplication and readability after missing images, forward/self/note-return targets, literal decoded-fragment matching, unavailable/ambiguous targets without fallback, resolved-tree/navigation privacy, repeated-resource occurrence handling and target-index ownership/limits/cancellation. Default runs skip the explicitly opt-in local fixture.
