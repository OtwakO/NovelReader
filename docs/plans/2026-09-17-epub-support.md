...
- `internal/epub`: bounded container/package inspection, URL resolution and semantic normalization; no HTTP, library mutations, home paths or worker ownership. Prefer Go ZIP/XML facilities and existing HTML parsing dependencies after checking their concrete suitability. Do not add a broad EPUB framework by default.
...
Review/accept the bounded proposal and confirm the fresh-data schema cutover policy. Then authorize implementation beginning with checkpoint 1. Resolve auxiliary bookmark behavior, exact semantic schema and EPUB 2 parser compatibility in that checkpoint before storage DDL is finalized; do not turn these implementation details into a speculative universal model.

## Verification

Performed: read-only code/document inspection and targeted EPUB 3.3 primary-reference checks. Documentation whitespace/path checks are the only intended checks in this planning change.
