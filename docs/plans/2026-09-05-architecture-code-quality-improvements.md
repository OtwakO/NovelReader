...
A08 is implemented: reader schema epoch 8 includes font-owned pending cleanup. Metadata replacement/deletion retains obsolete filenames transactionally; immediate cleanup and runtime-open retries are serialized by SQLite. Cleanup failure does not block unrelated runtime use. Upload IDs are random rather than millisecond timestamps. Font-store (including concurrent Store instances and failure/retry), API, reader-store, backup and server-command package tests pass; font-store tests also pass under the race detector. No development data was reset. Old schemas require reset or matching older code; no migration was added. Pre-publication filesystem crash atomicity remains outside this fix (see architecture).

## Next action

A08 accepted: update the fresh reader schema directly (no migration/backward compatibility; developer may reset data). Add a font-owned pending-deletion table; replacement/deletion record obsolete filenames in the metadata transaction, then cleanup immediately and retry at runtime opening. Serialize mutations with SQLite write transactions, not a global job system. Bump the reader schema epoch; do not reset data automatically. Rollback requires matching code/data versions or a development reset. Test replacement and failed deletion/retry. This guarantees retained cleanup ownership for committed removals, not atomicity of every filesystem write across process crashes. A08 is complete as recorded above. Next: logical-book identity (A09), rechecking its direct boundary first. A01–A08 and the original frontend test regression are complete. Font replacement must preserve existing behavior without losing cleanup references on failure; confirm any storage migration before implementation.
