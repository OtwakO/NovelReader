...
- Restore returns `restored: true` with a safe warning if TXT reconciliation fails after publication. The frontend clears pre-restore reader state and keeps a translated warning visible. Corrupt/incompatible archives remain rejected before replacement. The common deletion route explicitly rejects unrouted providers instead of orphaning their files/native records; TXT management routing is the next required boundary.
- Full backend tests, affected race tests, frontend build and 12 focused frontend tests pass. AFT inspection timed out. Upload/inbox scheduling, bounded intake admission, provider-aware reading/management routes, HTTP review-proof retention and import UI remain unimplemented. No existing data was reset or deployed.

## Next Action

Production lifecycle integration is implemented and verified. Next connect provider-aware TXT reading and management routes to the existing stores before exposing publication admission; remove the unsupported-provider deletion gate only when the TXT removal lifecycle is actually used. Then add bounded browser/inbox intake **outside API runtime slots**, with its own storage allowance and reader-specific drain barrier, followed by the core import/review UI.
