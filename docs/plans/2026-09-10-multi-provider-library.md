...
2. **Shared-state and durable-lifecycle checkpoint (in progress).** The bounded snapshot/file-mutation approach above and uncertain inbox cleanup policy are confirmed. Implement/verify the existing snapshot/font boundary first, then specify the shared-state cutover and recoverable publication acquisition/removal before those edits. Preserve BookSource behavior; avoid a standalone generic foundation project.
...
- Step 2 inspection and both operational decisions are recorded above. The snapshot/font coordination substep is implemented and verified; its current contract lives in [reader-storage architecture](../architecture/authentication-and-reader-storage.md#portable-reader-data-backup). No library/provider schema or acquisition implementation has started. The BookSource catalog transaction-ownership increment is complete and verified. Inbox cleanup confirmation is decided but not implemented.

## Next Action

Carry out the shared-state cutover as one coordinated BookSource-preserving persistence/API/frontend change, with concrete schema/revision contracts and recreation instructions recorded before schema edits. Do not add an unused parallel library store or restart broad architecture review. Then implement durable acquisition using the existing gate and confirmed cleanup policy. Portable inclusion/reference validation and interrupted acquisition/removal representation must be complete before exposing TXT intake. Do not repeat the completed inspection, reopen confirmed operational choices, or add advanced features as prerequisites.
...
- For isolated TXT step 1, AFT inspection timed out; scoped Go tests compiled the new package successfully. No full backend suite, frontend build, or TXT integration test is claimed. The subsequent snapshot coordination checks are listed separately below.
- Snapshot coordination verification: `go test -race ./internal/readerstore ./internal/fontstore ./internal/backup -count=1` passed; selected API/backup checks via `go test ./internal/backup ./internal/api -run 'Test.*(Font|Backup|Restore|ReaderRuntime|Upload)' -count=1` passed. Deterministic synchronized tests cover same-home exclusion across leases, other-home independence, ungated reads/progress-like writes, cancelled waits, and cancellation between copy reads. AFT inspection again timed out; Go tests/race detection supplied verification. No schema, persisted format, or HTTP shape changed.
- Catalog transaction ownership: the new composition test first reproduced `sql: transaction has already been committed or rolled back`. After the change, `go test ./internal/book -run 'Test(CatalogReplacement|ShelfAdmission|Catalogs|SaveCatalog|SwitchSource|MigrateChapter)' -count=1` and `go test ./internal/api -run 'Test.*(Candidate|Shelf|SourceSwitch|SwitchSource)' -count=1` passed. AFT diagnostics timed out again; no full backend suite or schema transition is claimed for this bounded refactor.
- TXT integration verification remains outstanding: preserved BookSource behavior, stale reading state, partial batch failures, interrupted acquisition/removal, account lifetime, and portable backup/restore. Define focused checks alongside step 2 rather than multiply hypothetical failure combinations.
