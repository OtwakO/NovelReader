...
- Production TXT schema/runtime registration, provider-aware routes, worker ownership/scheduling, HTTP review-proof retention and import UI remain unimplemented. The old blueprint is historical Git context, not an implementation checklist.

## Next Action

Completed runtime prerequisite: initialization is serialized per reader and in-flight initialization counts against capacity. The prior `acquire` could construct competing runtimes before choosing a winner; import recovery/workers must not run in such competing instances. Reserve ownership before initialization, make quiesce/shutdown wait for reservation cleanup, and keep slow initialization/close outside the manager mutex. No schema or TXT exposure change; rollback is code-only. Deterministic concurrent initialization, cancellation and close/quiesce participation are verified. Next add the TXT worker lifecycle under this single-owner boundary; do not mistake this prerequisite for completed worker integration.
