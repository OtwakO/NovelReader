...
- TXT publication persistence, acquisition recovery, inbox cleanup confirmation, import UI and managed-publication removal/backup remain unimplemented. The old blueprint is historical Git context, not an implementation checklist.

## Next Action

Complete the remaining durable-lifecycle checkpoint: specify the smallest TXT acquisition/publication records, transaction ownership, recoverable claim/finalization/removal sequence, and backup reference validation. Reuse the tested TXT analyzer, shared library operations and home mutation gate. Preserve the confirmed conservative interrupted-inbox cleanup policy and ensure restore cannot replay external deletion. Establish schema/rollback terms before changing persistence, then deliver acquisition through reading in working increments.
