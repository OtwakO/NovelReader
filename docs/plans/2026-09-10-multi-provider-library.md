...
- TXT interpretation is implemented in `backend/internal/txt` and connected to common reading routes through the persisted publication index. Browser-upload and inbox acquisition/review routes are implemented; the import UI remains pending.
...
- Normal and race runs of `txtstore`, `txtimport`, `api`, `readerstore`, `backup` and `cmd/server` pass for the inbox checkpoint. AFT inspection timed out; compiler/tests and scoped diff review are the authority. No schema change, migration, existing-data reset or deployment occurred. The core import/review UI and representative batch/UI verification remain unfinished; advanced controls and EPUB remain deferred.

## Next Action

Browser upload and inbox HTTP checkpoints are complete and verified. Next build the core import/review UI over these contracts: browser file selection with lightweight references and one admitted transfer at a time, inbox scan on opening Imports/explicit Scan, paginated status/results, bounded preview and encoding/preset correction, explicit bulk acceptance, and visible failure/cleanup controls. Keep acquisition separate from analysis/publication in the UI and preserve accepted server work across navigation. Handle stale inbox approval with fresh explicit review, never reconstructed flags or automatic deletion under a new proof. Follow existing Vue feature ownership and localization/accessibility patterns; no new backend framework or EPUB preparation. Then verify representative batch responsiveness and the complete import-to-reader/removal/restore journey.
