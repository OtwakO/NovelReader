...
**Remaining shared-state design, before schema edits:** one authoritative library row for display metadata and prose reading state; BookSource-owned bindings/catalog/cache and TXT-owned interpretations. Use revision-qualified section references rather than shared section rows or invented cross-catalog identity. Keep content revision separate from the state version protecting progress/bookmark mutations and apply previews. Cross-owner operations own one transaction and call transaction-aware store methods; helpers must not commit a caller-owned transaction. Define the coordinated backend/frontend transition and exact schema epoch in that increment, using the already confirmed disposable-home policy rather than dual writes or compatibility scaffolding.
...
- Step 2 inspection and both operational decisions are recorded above. The snapshot/font coordination substep is implemented and verified; its current contract lives in [reader-storage architecture](../architecture/authentication-and-reader-storage.md#portable-reader-data-backup). No library/provider schema or acquisition implementation has started. Inbox cleanup confirmation is decided but not implemented.

## Next Action

Specify the minimum shared-state cutover and acquisition recovery contract before their schema edits: library metadata and two revision domains, origin-owned section lookup/validation, transaction ownership, and coordinated API/frontend changes. Then implement durable acquisition using the existing gate and confirmed cleanup policy. Portable inclusion/reference validation and interrupted acquisition/removal representation must be complete before exposing TXT intake. Do not repeat the completed inspection, reopen confirmed operational choices, or add advanced features as prerequisites.
