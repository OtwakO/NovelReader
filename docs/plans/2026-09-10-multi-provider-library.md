...
- TXT analysis/index persistence and library admission, inbox acquisition/cleanup confirmation, workers, import UI and publication-aware backup validation remain unimplemented. The old blueprint is historical Git context, not an implementation checklist.

## Next Action

Continue from the tested `txtstore` receipt component; do not replace it with a generic import framework. Next persist analyzed sections/options and add transactional library admission plus bounded TXT section reads, reusing `txt.Analyze`, `txt.ReadSection` and library operations. Complete feature-owned portable-reference validation at snapshot/replacement boundaries before exposing intake. Inbox acquisition must then extend durable intent with the confirmed cleanup-confirmation policy and strip external cleanup instructions from exported/restored state. Worker/runtime ownership and cancellation remain a separate integration requirement. Register the complete TXT schema with explicit new-epoch/rollback terms before production exposure; do not compose this partial receipt schema into the running epoch-10 app.
