...
Current bounded implementation: persisted analysis scheduling plus a concrete fair TXT analysis pool and its quiesce/resume/forget/close interface. Component tests, their race runs, and the composed API-runtime concurrency check pass. No production schema registration, startup/intake recovery orchestration, upload/inbox scheduler, or HTTP exposure in this increment. Those remain a coordinated integration step; epoch 10 stays unchanged. Rollback is code-only while unregistered. Verify claimed-version cancellation/recovery, fairness/global and per-reader bounds, lost-wake avoidance, idle-state cleanup, and release of home leases on errors/cancellation/shutdown. Do not turn this into an exhaustive scheduling framework or a performance guarantee.
...
- `backend/internal/txtstore` now implements managed upload receipts: persisted intent, bounded streaming into disposable work, gated finalization, quiescent recovery and retryable discard. Synthetic reader-home tests cover portable pending originals and reader isolation. It is not registered in production schemas/runtime; epoch 10 and public HTTP behavior are unchanged. The subsequent inbox component is described below.
...
- The independent `backend/internal/txtimport` pool and persisted analysis scheduling are implemented as an unregistered component. Analysis claims load original metadata/options atomically, complete their short claim independently of cancellation, and return interrupted attempts to `received` without overwriting a later version. Indexed selection takes one pending file; active homes/results are released between fair reader turns. Idle scheduling entries and completed job state are not retained.
- Component tests, affected race tests and a composed foreground-capacity regression pass. Production TXT schema/runtime registration, startup/intake recovery orchestration, upload/inbox scheduling, provider-aware routes, HTTP review-proof retention and import UI remain unimplemented. The old blueprint is historical Git context, not an implementation checklist.

## Next Action

The independent analysis-pool increment is verified. Next integrate startup/intake recovery, reader-specific restore/deletion barriers and explicit foreground/background home-capacity composition before production schema registration. A pool wake-up is not durable acquisition or permission to recover a live transfer; intake must own those transitions. Keep epoch 10 until that coordinated integration has its exact-version/rollback terms.

Completed runtime prerequisite: initialization is serialized per reader and in-flight initialization counts against capacity; quiesce/shutdown wait for initialization cleanup. TXT workers will use the separately owned pool described above, not pin these API runtimes. The prerequisite remains relevant to safe API initialization and restore.

Completed bounded component: backend leftover review, guarded deletion confirmation and explicit journal release. Review carries a server-owned, non-serializable proof scoped to the current reader database; future HTTP code must retain that proof rather than reconstruct it from client fields. Compare file identity and bounded streaming hashes at review/confirmation; require a matching surviving managed original before deleting an inbox duplicate. Missing/changed input invalidates deletion approval. Release preserves bytes and permits a later scan to acquire through the existing path; it does not silently retry or discard managed receipts. No separate retry engine, durable review-token registry, schema change or runtime exposure is needed here. Tests cover stale/changed review, missing managed copies, scoped/idempotent confirmation, and release/re-acquisition. Rollback is code-only; existing claims stay blocked until explicitly resolved.

**Accepted inbox transfer decision:** rename on the same filesystem; stream-copy only on a cross-device rename error. The completed-copy convention is already confirmed (finish copying before opening Imports/Scan); it is not an open decision.

**Completed bounded component:** acquire one completed inbox TXT with durable intent, without routes/workers yet. Keep reader-specific inboxes at `DATA_DIR/inbox/<reader-id>/`, outside replaceable reader homes and portable files; this also keeps bind mounts out of home replacement. Store unresolved names/receipt IDs in a TXT-owned operational journal, atomically with receipt creation. Preserve this journal independently of managed-receipt removal to prevent silent re-import of uncertain leftovers. Normal uninterrupted completion clears its journal; failures/restart never automatically delete external leftovers. Strip the journal only from copied export/restore databases. The subsequent review/confirmation and release component is now tested; runtime and HTTP controls still precede exposure. No production epoch change in this isolated component; rollback is a code revert, not a data reset. Verify actual same-filesystem movement, copy fallback, interruption/deduplication and portable non-replay with synthetic homes.

Continue from the tested `txtstore` receipt component; do not replace it with a generic import framework. Saved interpretation, guarded admission and indexed reads are complete. Portable-reference validation is complete at snapshot, staging and pre-publication boundaries. Next establish and wire bounded worker/runtime ownership before intake exposure, reusing the completed acquisition, analysis and resolution operations. HTTP confirmation must use a retained server-issued review proof and must not reconstruct approval from client display fields. The completed-copy producer convention is already confirmed; do not ask it again. The operational journal and copied-database preparation hook now exist; extend their concrete behavior rather than adding another import or backup framework. Confirmation must reject changed reviewed inputs and must not delete an inbox original when its managed copy has been discarded. Worker/runtime ownership and cancellation remain a separate integration requirement. Register the complete TXT schema with explicit new-epoch/rollback terms before production exposure; do not compose this partial receipt schema into the running epoch-10 app.
