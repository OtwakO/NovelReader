...
- Production TXT schema/runtime registration, provider-aware routes, inbox acquisition/cleanup confirmation, workers and import UI remain unimplemented. The old blueprint is historical Git context, not an implementation checklist.

## Next Action

Continue from the tested `txtstore` receipt component; do not replace it with a generic import framework. Saved interpretation, guarded admission and indexed reads are complete. Portable-reference validation is complete at snapshot, staging and pre-publication boundaries. Next establish the concrete inbox acquisition/recovery and bounded worker/runtime contracts, including the reliable producer-completion convention, before consuming external originals or exposing intake. Do not add empty export-preparation hooks until inbox cleanup metadata actually exists. Inbox acquisition must then extend durable intent with the confirmed cleanup-confirmation policy and strip external cleanup instructions from exported/restored state. Worker/runtime ownership and cancellation remain a separate integration requirement. Register the complete TXT schema with explicit new-epoch/rollback terms before production exposure; do not compose this partial receipt schema into the running epoch-10 app.
