...
- Normal and race runs of the complete `txt`, `txtstore`, `txtimport`, `api` and `cmd/server` packages pass. AFT inspection was unavailable/timed out; compiler/tests and scoped diff review are the authority. No schema change, migration, existing-data reset or deployment occurred. Inbox scan/acquisition, bounded server-retained proof endpoints and import/review UI remain unimplemented.

## Next Action

The browser upload HTTP checkpoint is complete and verified. Next connect inbox scan/acquisition and bounded server-retained review-proof controls before exposing external-file consumption, then build the core import/review UI over the existing HTTP contracts. Reuse acquisition/analysis/acceptance and worker owners; do not introduce another job framework or readiness/migration layer. Inbox resolution must review settled acquisition, retain the actual server-issued proof for the current reader-home lifetime, and invalidate it across replacement/removal; client JSON is never deletion authority. Keep scan/results bounded and respect the producer-completion convention. Browser batches retain only local file references until acquisition; only explicit pre-acquisition rejections are safe for automatic retry.
