...
Interactive browser closure preserves each returned cookie's URL/domain/path scope instead of collapsing multi-domain cookies onto the final browser URL, and this scope survives durable capture/hydration. Source-generated opaque HTML contexts receive stored scoped cookies before mediated requests run, but not global login headers that could leak across probe origins. Normal HTTP(S) browser sessions retain source/login header hydration. Browser-generated opaque HTML diagnostics use a bounded fetch/XHR mediator that rejects non-public hostname resolutions and connected server addresses, follows no redirects, and preserves timeout/body limits; Chromium web security remains enabled.
