...
The authenticated server also owns the disposable `chapterresource.Store` at `data/cache/chapter-resources.sqlite`. It is independent of runtime eviction and portable homes, with atomic per-reader/installation admission and expiry cleanup. The server closes it after draining reader runtimes; account deletion removes that reader's bundles before declaring cleanup complete. Failed store initialization leaves its files intact and does not block unrelated reading; cache-cleanup failures keep account deletion retryable. BookSource documents publish bundle-qualified URLs only after admission. Resolution checks reader/home/book/revision/source-definition ownership and uses captured image recipes and script inputs, never the latest chapter row. No remote image bytes are cached. Explicit admission failures preserve prose with unavailable figures; image-only chapters fail. Freshness metadata prevents client memory reuse after the resource promise expires. Configuration is documented in [README](../../README.md#chapter-resource-cache-capacity); remaining integration belongs to the [cache plan](../plans/2026-09-22-reader-cache-and-prefetch.md).
...
Book detail, TOC, chapter and image/cover workflows acquire registry-owned session leases. A lease pins identity before waiting and orders whole workflows sharing that session; unrelated sessions can proceed. Idle expiry/capacity pruning skips active and waiting leases and trims again after release. Nested TOC helpers reuse the outer lease. Source invalidation retires queued work and suppresses alias publication from active retired sessions, but cannot prove a running script has stopped; source/interpretation checks are still required before publishing results. Chapter cache recheck, processing and publication/save now remain inside this same lease. Matching chapter-result sharing belongs to the runtime-bound reading provider, not the session registry; see [reading coordination](discovery-and-reading.md#reading-documents-and-resources). Its initiating handler retains the reader-runtime lease through completion even after disconnect, keeping restore/shutdown outside active publication. Search and Explore retain their independent workflow ownership.
...
Opaque browser-launch requests are bound to their emitting Source ID. The runtime owns a launch before worker I/O begins, not only after a browser frame returns. Source invalidation retires matching pending requests and starting/active sessions without discarding another source's pending request. A detached launch cannot publish a late frame; its bounded worker response is allowed to return so the created context can be closed without saving authentication. Worker cleanup has a separate bounded timeout and does not depend on the initiating request remaining connected.
...
`ReaderSchema.PreparePortable` strips disposable caches and installation-local operational authority from the copied database on export and import, without modifying live records. BookSource removes fetched chapter-cache rows and clears catalog cache flags, preserving catalog identities and reader state. Compatible older archives containing cached chapters are sanitized on staging too. TXT uses it for unresolved inbox claims; even a discarded receipt's leftover claim remains local until explicitly resolved. EPUB strips attempt-local staging paths while retaining installed finalization evidence for recovery. After the callbacks commit, the independent staged database is compacted so deleted payloads are not retained in unused pages. Copies containing installation-local authority are rejected at publication validation.

Features can contribute `ReaderSchema.ValidatePortableFiles` to check references against the copied read-only database and confined files. Checks run after snapshot copying, after replacement staging, and before replacement publication—not on ordinary home opens. TXT validates file/publication ownership, legal interpretation roles/generations, completed section ranges and original files. Interrupted acquisition/removal records retain their lifecycle-specific allowances for missing or damaged originals. TXT validation never decodes the novel or executes stored patterns; EPUB validates bounded prepared semantics and image resources without re-preparing the book. TXT published reparse is implemented; its verification is recorded in the completed [multi-provider plan](../plans/2026-09-10-multi-provider-library.md). No live records are repaired or deleted by these checks.
