...
| Stealth | CloakHQ claims source-level Chromium patches and strong headless results. Its own README nevertheless recommends `headless=False` for some protected sites. Discussion #154 contains a user benchmark where plain Patchright beat CloakBrowser for KillBot, while the maintainer says Patchright added no measurable benefit across 50+ systems. These are target-specific, non-controlled reports—not proof for NovelReader. | Mixed/unknown for NovelReader |
| Footprint | CloakHQ says the binary archive is about 200 MB. The official Dockerfile also installs Node, Xvfb, Openbox, fonts, both Python and JS wrappers, and examples. Registry inspection of official `latest` linux/amd64 layers totals **625,255,600 bytes (~596 MiB)** compressed—effectively the same class as NovelReader's measured ~613 MB image, not evidence of a reduction. Runtime RSS and unpacked size were not published or benchmarked here. | Image compressed size proven; RSS/unpacked unknown |
...
NovelReader's existing fingerprint-scan results (Patchright bundled Chromium: 100 headless; Chrome+Xvfb: 51.5 headful under the same IP) are more directly relevant than CloakHQ's cross-site marketing table because they measure the actual worker environment and target scanner. They also show that “headful” is not automatically stealthier. Discussion #154 reinforces that browser choice can reverse by detector: the maintainer reports no Patchright benefit across its suite, while a user reports Patchright winning strongly on KillBot.
...
Keep Patchright as the production default. Consider CloakBrowser only behind an experimental image/configuration after:
