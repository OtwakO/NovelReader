...
func TestParseSearchResultPreservesListWordCountAndUpdateTime(t *testing.T) {
	...
	html := `<article class="book"><a class="name" href="/book/1">Book</a><p class="intro">First&nbsp;line<br>Second line</p><span class="words">123万字</span><time class="updated">2026-08-01</time></article>`

	results, err := searcher.ParseSearchResult(source, html)
	if err != nil {
		t.Fatal(err)
	}
	if len(results) != 1 || results[0].WordCount != "123万字" || results[0].UpdateTime != "2026-08-01" || results[0].Intro != "First line\nSecond line" {
		t.Fatalf("results=%+v, want normalized intro plus list word count and update time", results)
	}
}
