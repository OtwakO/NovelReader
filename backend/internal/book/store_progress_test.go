...
func TestStoreUpdateProgressValidatesAndReportsMissingBooks(t *testing.T) {
	...
	if version, err := store.UpdateProgress("book-1", 0, 0, 3, 0.75); err != nil || version != 1 {
	...
	if _, err := store.UpdateProgress("missing", 0, 0, 0, 0); !errors.Is(err, ErrBookNotFound) {
		t.Fatalf("missing error=%v", err)
	}
	if _, err := store.UpdateProgress("book-1", 99, 1, 0, 0); !errors.Is(err, ErrBookStateChanged) {
		t.Fatalf("stale content revision error=%v", err)
	}
	for _, invalid := range []struct {
		chapter int
		pos     float64
	}{{-1, 0}, {0, -0.1}, {0, 1.1}, {0, math.NaN()}, {0, math.Inf(1)}} {
		if _, err := store.UpdateProgress("book-1", 0, 1, invalid.chapter, invalid.pos); !errors.Is(err, ErrInvalidProgress) {
