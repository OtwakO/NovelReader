...
func TestExecutorCarriesCrawlContextIntoURLAndBodyJS(t *testing.T) {
	...
	executor.SetURLContext(&analyzer.URLContext{
		Source:      map[string]interface{}{"bookSourceUrl": "https://example.test/", "bookSourceName": "Source"},
	...
	response, err := executor.Execute(context.Background(), `{{source.bookSourceUrl}}{{book.name}}/{{chapter.index}},{"bodyJs":"marker(result)+'|'+book.bookUrl+'|'+chapter.url+'|'+nextChapter.url+'|'+source.getKey()+'|'+source.bookSourceName"}`, "", 1, "https://example.test/")
	if err != nil {
		t.Fatal(err)
	}
	...
	wantBody := "fixture-lib|https://example.test/book|https://example.test/read/4|https://example.test/read/5|https://example.test/|Source"
