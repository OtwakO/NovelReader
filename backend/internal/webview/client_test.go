...
func TestInteractiveClientStartsInputsAndClosesSession(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/sessions":
			...
			if request.URL != "https://example.test/login" || len(request.Cookies) != 1 || request.Viewport.Width != 430 || request.Viewport.Height != 800 || request.Viewport.DeviceScaleFactor != 3 {
	...
	frame, err := client.StartInteractive(t.Context(), "https://example.test/login", "Login", InteractiveViewport{Width: 1200, Height: 800, DeviceScaleFactor: 4}, session)
	if err != nil {
		t.Fatal(err)
	}
