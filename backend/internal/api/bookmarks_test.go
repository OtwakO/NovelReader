...
func TestBookmarkAPIAddsListsAndDeletesValidatedNotes(t *testing.T) {
	...
	body := []byte(`{"id":"mark-1","sourceUrl":"source","stateVersion":0,"chapterIndex":0,"position":0.5,"note":"  note  "}`)
	for range 2 {
	...
	unicodeNote, _ := json.Marshal(map[string]interface{}{"id": "unicode", "sourceUrl": "source", "stateVersion": 0, "chapterIndex": 0, "position": 0, "note": strings.Repeat("书", 1000)})
	...
	malformed := append([]byte(`{"id":"bad","sourceUrl":"source","stateVersion":0,"chapterIndex":0,"position":0,"note":"`), 0xff)
	...
	tooLong, _ := json.Marshal(map[string]interface{}{"id": "long", "sourceUrl": "source", "stateVersion": 0, "chapterIndex": 0, "position": 0, "note": strings.Repeat("x", 1001)})
