...
func TestBookmarkAPIAddsListsAndDeletesValidatedNotes(t *testing.T) {
	...
	body := []byte(`{"id":"mark-1","contentRevision":1,"stateVersion":0,"chapterIndex":0,"position":0.5,"note":"  note  "}`)
	for range 2 {
	...
	unicodeNote, _ := json.Marshal(map[string]interface{}{"id": "unicode", "contentRevision": 1, "stateVersion": 1, "chapterIndex": 0, "position": 0, "note": strings.Repeat("书", 1000)})
	...
	malformed := append([]byte(`{"id":"bad","contentRevision":1,"stateVersion":0,"chapterIndex":0,"position":0,"note":"`), 0xff)
	...
	tooLong, _ := json.Marshal(map[string]interface{}{"id": "long", "contentRevision": 1, "stateVersion": 0, "chapterIndex": 0, "position": 0, "note": strings.Repeat("x", 1001)})
	...
	if response := performAPIRequest(server, http.MethodDelete, "/api/books/book/bookmarks/mark-1", []byte(`{"contentRevision":1,"stateVersion":2}`)); response.Code != http.StatusOK {
		t.Fatalf("delete status=%d body=%s", response.Code, response.Body.String())
	}
	if response := performAPIRequest(server, http.MethodDelete, "/api/books/book/bookmarks/mark-1", []byte(`{"contentRevision":1,"stateVersion":3}`)); response.Code != http.StatusNotFound {
