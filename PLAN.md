...
**In progress:** The user selected an unrestricted deterministic 50-identity tenth Explore live audit because v9 exposed four shared seams. The frozen seed is `NovelReader-explore-random-v10-2026-07-23`; all 400 prior stable identities are excluded, leaving 321 eligible before selection. The sample is fixed by SHA-256 rank and must not be substituted after results are observed. Production execution uses an unmodified corpus, fresh isolated SQLite database, four-client initial pass, 90-second page timeout, and sequential confirmation of every non-pass. This is audit-only; source/parser behavior remains frozen. Java `\uXXXX` regex escapes translate to Go-compatible code points while malformed escapes still fail; Default traversal uses an independent accumulator; dotted `.[*]` JSON wildcards normalize outside quoted strings; and simple root-array wildcard predicates preserve order. Evidence is in `testdata/booksource/explore-live-v9-fixes-rerun-2026-07-22.json`. Stale contracts and URL-pattern enforcement remain unchanged.
...
**Next action:** Commit this v10 plan and frozen selection, run all 50 identities through production Explore, sequentially confirm and diagnose every non-pass, then preserve v10 JSON/Markdown evidence and stop at the fix approval gate.
