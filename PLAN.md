...
**Next action:** Continue LC-007 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: explicit CSS getter compatibility is complete; inventory the pinned corpus and upstream selector contracts to choose the smallest high-value Jsoup-only selector slice, then pin it through an Analyzer public-seam regression. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
