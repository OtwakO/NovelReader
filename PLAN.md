...
**Explore audit v13 in progress:** The user selected another unrestricted 50-source round. Seed `NovelReader-explore-random-v13-2026-08-11` ranks eligible `(rawIndex, bookSourceUrl)` identities from unchanged `test_booksource4.json` after excluding all 550 identities in Explore audits v1–v12. The frozen sample takes the first 50 of 171 remaining eligible identities with no substitutions. Execution uses a fresh disposable reader root, the production Explore catalog/page APIs, four initial clients, a 90-second audit timeout, and sequential confirmation of every non-pass or diagnostic. Sampling stops after page 1 of the first selectable category and changes no source/parser behavior.
