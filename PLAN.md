...
**Next action:** Commit the live-rerun correction that simple CSS-compatible `a@attr` extraction must also skip blank/duplicate values and bracket-indexed simple selectors must route through Default mode. Preserve exact rerun evidence (raws 197/163/17 now return 15/20/10), then run full verification and review.
...
- **Watch out**: Implemented generic public contracts rather than raw-specific branches: arbitrary terminal attributes, complete singleton/list/exclusion/inclusive-range/step/negative bracket grammar including root-child indexing, nonblank deduplicated Default and CSS-compatible attribute extraction, received HTTP-status bodies with explicit warning diagnostics, nullable base64 bytes, AES/CBC PKCS padding crypto object methods, and source-scoped login-header storage/map access. The first exact rerun caught that raw 17's simple `a[1]@href` and `a@title` forms take the CSS-compatible path; generic mode detection/value collection now covers those forms. Exact production returns 15/20/10 books for raws 197/163/17. No hostname-based UA inference, no `title`-only getter patch, no `a[1]` special case, and no conversion of DNS/TLS/timeouts into parseable responses.
