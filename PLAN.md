...
**Next action:** Follow LC-005 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: verify `ruleBookInfo.canReName` against current upstream mutation behavior and add the smallest public book-info regression before changing the local contract. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
