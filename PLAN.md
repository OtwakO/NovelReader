...
**Next action:** The Phase 2.2 persistent session-storage core is complete: each login creates an independent 32-byte opaque token; schema v2 stores only constrained SHA-256 hashes; sessions do not expire or rotate; activity writes are coarsened; active-account identity is validated; logout-one and logout-all delete rows; and password/status changes revoke all sessions behind one canonical-path process barrier. Proceed with the HTTP authentication boundary: login creates a session cookie, logout removes one session, request middleware exposes identity, and small body/deadline/rate limits plus same-origin credential policy are enforced. First-Administrator setup and Reader Data ownership remain closed. This replaces the earlier 30-day rotating design outright; no legacy migration is in scope. The accepted model uses `system.db`, plaintext self-contained per-user `reader.db` directories, separately encrypted non-exportable source credentials, one-time browser Administrator setup, stored `reader`/`admin` roles, an environment recovery token, persistent independently revocable sessions, and protected Administrator accounts. LC-016 remains blocked until this boundary exists. LC-015 is complete.
