...
Audit checkpoint (2026-07-15, updated 2026-07-16): final/source URL continuity, `bookInfoInit`, complete typed book/current/next-chapter context, TOC semantics, fail-closed TOC pagination, content pagination, declared-rule-first extraction, and API failure propagation are complete with deterministic coverage. The required raw-source first/middle/last matrix and broader conformance matrix remain partial. Close them in this order: raw-source first/middle/last E2E matrix; broader conformance matrix; dead compatibility fallback cleanup.
...
**In progress:** The deterministic Phase 4 crawl and API failure slices are complete. Content supports Legado's single-chain versus fixed multi-page modes, typed fail-closed pagination errors, request options/retries/bodyJs, cycle and TOC-boundary handling, declared-rule-first SPA diagnostics, and stable API status/code diagnostics. Full Go tests and analyzer/book/sourceexec/API/conformance race tests pass.

**Next action:** Build the raw-source first/middle/last E2E matrix, then close the broader conformance gaps before removing compatibility fallbacks.
