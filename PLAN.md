...
- Stopped-server copy of the complete `data/` directory is a supported backup. Portable per-reader exports exclude credentials and authentication authority. Losing administrator access or the credential key must not make Reader Data unreadable.
...
**Next action:** Phase 2.2 account persistence is complete: workflows supply one validated immutable `UserID`; ordinary creation can create only active `reader` accounts; normalized usernames are unique under serialized concurrent creation; authentication performs one real or dummy bounded password derivation and returns generic failures for unknown, wrong-password, disabled, and deleting accounts; stored identity enum/ID drift fails as corruption; and privileged password replacement atomically revokes all live sessions. Proceed with the separate browser-session core: opaque token generation/hash storage, current/previous rotation with five-minute grace, 30-day absolute expiry, logout/revocation, status checks, and deterministic concurrent replay tests. HTTP routes, request limits/rate limits, cookies, origin policy, first-Administrator setup, and Reader Data ownership remain closed. This is a clean replacement; no legacy migration is in scope. The accepted model uses `system.db`, plaintext self-contained per-user `reader.db` directories, separately encrypted non-exportable source credentials, one-time browser Administrator setup, stored `reader`/`admin` roles, environment recovery token, 30-day rotating sessions with bounded grace, and protected Administrator accounts. LC-016 remains blocked until this boundary exists. LC-015 is complete.
