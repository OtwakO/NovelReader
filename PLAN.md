...
**In progress:** Shared compatibility phases 1–3 are complete. In addition to generic Default and fallback behavior, the focused bridge now matches Legado's nullable/int32 `toNumChapter` semantics and exact numeral map, exposes non-blocking structured `toast`, and lets `java.ajax` execute shared URL-option method/body/header metadata with relative resolution, list-first behavior, context cancellation, response cookies, and no nested runtime. Double-brace connector depth, mixed single/double interpolation, template-local `##`, and wrapped `##regex` empty replacement are covered by regressions found during review. Analyzer/book/sourceexec/conformance and race suites pass. Authenticated Pixiv/browser/cache/login work for raw 134 remains explicitly deferred. Unrelated manual-testing changes in `frontend/dist` and `frontend/package-lock.json` remain untouched.

**Next action:** Commit the focused bridge, then rerun affected v1/v2 live identities and the deterministic audit manifests before full repository and Playwright verification.
