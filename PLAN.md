...
- `backend/internal/fileimport/` — bounded intake admission and preparation-worker lifecycle (currently TXT dispatch; shared EPUB dispatch pending).
...
[EPUB support](docs/plans/2026-09-17-epub-support.md) — implementation authorized for structured DRM-free reflowable EPUB 2/3 through browser and server-inbox import, using the shared library and prose reader. First checkpoint in progress: the user selected focused Go parsing after the Readium build-footprint proof. Bounded archive/XML, EPUB 2/3 package inventory and hierarchical NCX/nav inspection and package support checks (layout, encryption, spine fallbacks) plus semantic/image/target binding, staged preparation with auxiliary discovery, embedded cover selection and a candidate version-2 backend reading projection pass focused normal/race tests. The shared frontend renderer and candidate contract/conversion proof also pass targeted tests and isolated desktop/mobile browser checks. Optional auxiliary membership, shared progress exclusion and a tested nested note-return model are now implemented. ReaderView note-return/anchor lifecycle wiring now passes focused integration tests; the hierarchical catalog projection/parser now has a shared Go/frontend contract fixture. Shared expanded TOC presentation is also integrated in Reader/Book Detail and checked with synthetic desktop/mobile browser data; the Go-projected structured wire fixture and composed desktop/mobile ReaderView proof now pass. Client transport admits versions 1 and 2, while current backend providers still emit version 1. Remaining contract-profile/payload checks, authorized resources and durable integration are unfinished. Schema cutover is gated separately; existing homes/backups remain untouched.
...
1. Implement the [EPUB content/navigation contract checkpoint](docs/plans/2026-09-17-epub-support.md), orientation-aware images, labeled cover placeholders and representative payload/cache measurements are now verified; auxiliary bookmark reopen is also verified through the shared reader; image-memory measurement is complete and the existing 32 Mi-pixel cap is accepted; optional WebP 92 / 2048-pixel image optimization is accepted (Original remains default). Shared imageproc optimization and EPUB caller-owned derivative emission are implemented, with once-per-resource conversion and separate fallback evidence; durable storage/lifecycle integration is pending. The controlled Go/Alpine proof justifies native encoding with an explicitly disclosed portable fallback. Native container encoding and allocation-lifetime checks pass. Epoch-14 fresh-data storage work is authorized; original images will be served from retained EPUB members through the verified bounded read helper. Epoch 13 remains active pending coherent persistence/recovery/portable integration. Attempt-owned staging, acquisition recovery and persisted preparation-generation claims/retries now pass isolated tests; the DDL remains unregistered. The indexed section-stream representation is accepted; staging and bounded range reads now pass normal/race/portable-encoder tests. SQLite section/resource persistence, ready-state installation and startup reconciliation now pass isolated tests. Export/restore archive limits are now enforced consistently. Portable copied-state cleanup and receipt/preparation ownership checks pass isolated snapshot/recovery tests. Section-index, physical stream, image-resource and decoded-summary checks also pass; shared JSON limits and publication-wide semantic/binding/anchor checks pass in isolation; the composed portable hook passes isolated snapshot/replacement checks; the import lifecycle owner is promoted to `fileimport` with unchanged two-worker/two-transfer limits and passing scoped race tests, while mixed-format dispatch/recovery remains before schema activation, then finalize the supported profile/contract; confirm schema cutover before storage changes. TXT/shared-library work is complete at its recorded verification scope; do not reopen its historical plan. Existing data remains untouched; the user handles development resets.
