...
**In progress:** One Compose contract now supports GHCR pulls or local builds. The rootless app publishes only loopback port 8888, persists `/data`, and uses measured backend limits; the optional `webview` profile starts the separately constrained worker on a private bridge with no host port. HTTP-only and profile-enabled startup, service health, app-to-worker connectivity, and worker port privacy pass with local images.

**Next action:** Add a deterministic Compose E2E fixture and script proving frontend assets, readiness, rendered-only search, graceful stop, and volume persistence from a clean checkout.
