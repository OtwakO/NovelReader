...
**In progress:** The first reading-state slice is complete. Progress writes now require an existing book, an exact stored non-volume raw chapter index, and both bounded normalized fields; older databases gain `alternate_sources` without a migration framework. Shelf/detail expose Continue, volume rows are not opened as chapters, direct links resolve explicit → saved → first-readable indices, and the Reader serializes progress per book across component lifetimes while restoring scroll after rendered layout. Fresh 390px Playwright verified a stored 0.60 position after reload, an immediate pre-debounce 0.42 scroll surviving Shelf → Continue, next-chapter persistence to raw index 1, direct no-chapter resume, and a clean console. Full backend, race, frontend tests, and warning-free production build pass. Phase 7 remains complete with its raw index-1 live evidence, deterministic WebView coverage, and catalog-load measurements recorded in the preceding history. Broad aesthetic polish remains deferred as requested; the unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Implement safe alternate-source switching with per-source state, target-TOC validation before commit, normalized title matching, and clamped raw-index fallback. Bookmarks and bounded server-side offline cache follow; frontend aesthetic polish remains a dedicated later pass.
