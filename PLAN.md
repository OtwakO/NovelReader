...
**In progress:** The approved shared Explore compatibility implementation and verification are complete. Generic fixes cover Default traversal/exclusion, connector and interpolation semantics, optional mixed-mode fallback, exact focused Java helpers, URL-option AJAX, transformed element fallbacks, JSON-list extraction, `@@` Default forcing, and JSoup numeric enumeration. Targeted live reruns repaired 13 engine identities: raw 76, 82, 204, 209, 342, 410, 429, 471, 516, 614, 816, 897, and 920 now return distinct books. Raw 742 now parses but remains empty solely because its unchanged raw pattern requires obsolete `qudushu.com` while live links use `qudushu.la`. Full fixed-manifest reruns observed 32/50 and 30/50 live non-empty results for v1/v2 respectively; these are time-dependent evidence, not corpus pass rates. Full backend, race, frontend tests, production build, Docker E2E, and 390×844/desktop Playwright pass; representative UI counts were 20/20/10/3 with zero console warnings/errors. Authenticated Pixiv/browser/cache/login work for raw 134 remains explicitly deferred. Evidence is in `testdata/booksource/explore-live-shared-fixes-rerun-2026-07-19.json`. The user's pre-existing generated frontend/package-lock changes remain unstaged and were restored after local build verification.

**Next action:** Commit verification evidence and report the completed implementation for user review; do not begin raw134 or stale-source rewrites without explicit approval.
