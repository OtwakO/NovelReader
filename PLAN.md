...
- [ ] Audit every Phase 4 checkbox against current code, deterministic tests, and Legado behavior; mark only evidenced behavior complete and turn confirmed gaps into focused TDD slices.
...
**In progress:** Phase 4 closure has started as the hard gate for the next major milestone. The immediate work is an evidence-based reconciliation of current detail, TOC, content, and conformance behavior against every Phase 4 task; already-implemented behavior will be marked complete only when deterministic and live evidence satisfies the contract. The future Phase 7 milestone is fixed as strict Legado-compatible, per-source Explore/Discover with no unified feed or custom source schema.

**Next action:** Audit `GetBookInfo`, `ChapterListParser`, `GetChapterContent`, their callers/tests, raw fixture coverage, and the Legado reference end-to-end. Produce the ordered TDD gap list before changing production code; then close one shared root-cause gap at a time and run the mandatory deterministic/live verification loop.
