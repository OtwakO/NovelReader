...
- **Evidence:** Nine sources completed Search and Book Info. Exact-request cross-checks confirmed 20 persistent timeouts and the remaining upstream, blocked, WebView, source-contract, DNS, transport, and Rhino/JVM outcomes. Raw 72 proves the public `java.encodeURI` bridge omits Legado's optional charset argument, but correcting only its GB2312 output still produced an explicit zero-result page.
- **Decision:** Report zero recoverable shared-engine gaps for v4. Keep charset-aware `java.encodeURI` as a separate non-recovering compatibility observation and require user approval before any implementation.
- **Verified:** The v4 verifier reconstructs the disjoint ranking and exact definitions, recomputes the complete 50-source histogram, requires all 41 sequential replays, validates timeout/browser evidence, and rejects any shared-gap classification.
