...
- Storage foundation comes first, but authentication and conversion of every global Reader Data route ship as one atomic release boundary. Do not add globally keyed login state, expose authenticated-looking routes over global stores, accept client-supplied user IDs as authorization, or hardcode reader-directory paths outside `readerstore`.
