...
**Next action:** Commit final review corrections: lenient login-header maps with direct properties and `.get`, login Cookie precedence over stale jar state, and session/login-header merge in staged HTTP transport beneath explicit request options. Then rerun exact evidence, full verification, and final reviews.
