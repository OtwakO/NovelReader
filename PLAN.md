...
- **Watch out**: `text()` still includes descendant text; `ownText()` intentionally does not. Keep the two APIs distinct.
...
- **Watch out**: Only isolated encoded `<...,...>` segments are decoded; unrelated percent-encoding and query structure remain byte-for-byte unchanged.
