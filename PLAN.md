...
**Next action:** Audit LC-016's automatic-login lifecycle and stop for product/security decisions before implementation. LC-015's approved portable chapter-image slice is complete: additive ordered blocks, offline cache persistence/migration, stored indexed image endpoints, reader rendering, portable byte decoding, and explicit Android bitmap-transform rejection. It does not expose arbitrary URLs or claim bitmap support. LC-014 is complete for stored-book covers; LC-013 is dormant and LC-012 remains intentionally deferred. The affected analyzer/fetcher/processor/book/API tests, frontend 13 tests/build, and UI detector pass.
