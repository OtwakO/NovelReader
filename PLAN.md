...
**Next action:** Phase 2.1 is complete: `backend/internal/auth` owns a fixed, versioned `data/system.db`, startup validates it before feature stores, its canonical schema contains only identity/control records, and the storage seam enforces `reader`/`admin` plus legal `active`/`disabled`/`deleting` transitions. Proceed with Phase 2.2 authentication core: username normalization, bounded Argon2id, opaque rotating sessions, login/logout/password change/revocation, request-context identity, endpoint limits/rate controls, and same-origin policy. No existing Reader Data route may open behind authentication until every globally keyed store and Source Session is converted in Phase 2.4. Old development databases are not migrated; developers reset `DATA_DIR` and later re-import test BookSources. The accepted model uses `system.db`, plaintext self-contained per-user `reader.db` directories, separately encrypted non-exportable source credentials, one-time browser Administrator setup, stored `reader`/`admin` roles, environment recovery token, 30-day rotating sessions with bounded grace, and protected Administrator accounts. LC-016 remains blocked until this boundary exists. LC-015 is complete.
