...
1. Implement the [EPUB content/navigation contract checkpoint](docs/plans/2026-09-17-epub-support.md), orientation-aware images, labeled cover placeholders and representative payload/cache measurements are now verified; auxiliary bookmark reopen is also verified through the shared reader; image-memory measurement is complete and the existing 32 Mi-pixel cap is accepted; optional WebP 92 / 2048-pixel image optimization is accepted (Original remains default). Shared raster validation/orientation is now extracted into imageproc and used by EPUB; optimization itself is not implemented. The controlled Go/Alpine proof justifies native encoding with an explicitly disclosed portable fallback. Next implement single-decode optimization with allocation-lifetime checks and tested runtime provisioning, then finalize the supported profile/contract; confirm schema cutover before storage changes. TXT/shared-library work is complete at its recorded verification scope; do not reopen its historical plan. Existing data remains untouched; the user handles development resets.
