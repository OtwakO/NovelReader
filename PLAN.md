...
**Next action:** Follow LC-006 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: define the smallest typed representation for `ruleBookInfo.downloadUrls`, then add a public detail-parsing regression before implementation. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
