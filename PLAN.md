...
- [ ] Add deterministic raw-source fixtures covering normal HTML, JSON, XPath/Regex, POST/charset, JavaScript, and WebView exploration where those capabilities occur in real sources.
  ...
  - [ ] Add captured/local response fixtures with executable category, request, paging, and result expectations.
...
**In progress:** Phase 7 audit/contracts merged through PR #2 (`80c9154`). On `feat/phase7-explore-fixtures`, seven exact raw entries are now extracted into `testdata/booksource/explore-sources.json` with compilation/index/URL/per-entry identity guards and required capability coverage. Import now preserves Legado's omitted `enabledExplore=true` default while respecting explicit false. The unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Add deterministic response/request expectations for the URL-category slice: lenient JSON and legacy category parsing first, then HTML/JSONPath/XPath-Regex/POST-charset/JS/WebView page execution and pagination before the Explore service is exposed through API/frontend.
