...
Decisions: browser overload waits within the existing request deadline; workflow sessions use TTL plus a bounded LRU. The browser worker remains bounded by pages, pending requests, and browser recycling; normal HTTP and fingerprint work must reuse connection pools while keeping cookie state session-scoped.

Tasks:

- [ ] Add bounded-wait/retry for transient browser-worker overload.
- [ ] Add TTL/LRU eviction to the workflow session registry.
- [ ] Reuse stateless HTTP connection pools across source sessions.
- [ ] Cache fingerprint clients within SourceSession lifetime.
- [ ] Add per-request and process-wide capacity metrics.
- [ ] Verify hundreds-source search and concurrent-user behavior with deterministic load tests.
