...
**Next action:** Rerun focused live/corpus verification after review hardening, then run full deterministic/race/vet/frontend/build checks and final Standards/Spec re-review. The first review caught malformed lenient-map acceptance, non-Jsoup negative `eq()` behavior, and over-broad optional-field error swallowing; all three now have red-green regressions and narrowed fixes. The v6 targeted rerun evidence is already committed. Then run full deterministic/race/vet/frontend/build checks and final Standards/Spec review. Do not rewrite stale source contracts.
...
- **Watch out**: Missing/null JSON paths are absence; syntax, script, cancellation, and other parser failures remain loud for optional as well as required fields.
