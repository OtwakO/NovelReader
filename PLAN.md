...
**Latest Search → Book Info audit:** v3 sampled 50 unrestricted deterministic identities from unchanged `test_booksource3.json`, disjoint from all 50 v1/v2 identities, with fixed query `凡人修仙传`. Exactly the frozen raw definitions were executed through the production conformance path; all 39 initial non-passes replayed sequentially. Eleven completed both stages. Two identities prove recoverable shared regular-engine gaps: standalone OnlyOne regex replacement preserves surrounding input (raw 375), and declared GET-query charset is ignored during query encoding (raw 364). Counterfactuals changing only the disputed seam returned 20 Search results and successful Book Info for both. Other outcomes are 4 deferred WebView, 2 deferred Rhino/JVM, 4 blocked/auth, 3 legitimate empty, 13 timeouts, 2 transport, 4 HTTP, 1 DNS, 3 source/site drift, and 1 invalid contract. Unsupported whole/inline `<js>` URL syntax and omitted Search `jsLib` are preserved as compatibility observations, but current sampled workflows have independent site/HTTP/JVM blockers and are not counted as recoverable gaps. Playwright was used only for four browser-bypass ambiguities and confirmed Cloudflare/Baidu verification pages. Evidence is in `testdata/booksource/audits/search-bookinfo/search-bookinfo-live-audit-v3-2026-08-12.{json,md}`. No production behavior changed; the two shared fixes remain approval-gated.
...
- **Change:** Preserved JSON/Markdown evidence and versioned v3 scripts. No production source, parser, or transport behavior changed.
- **Reason:** Raw 375 proves OnlyOne regex replacement must return only the first matched-and-replaced value; raw 364 proves URL-option charset must encode GET query variables, not only decode responses or encode POST bodies. Unsupported `<js>` URL syntax and omitted Search `jsLib` were observed but not promoted because current sampled workflows retain independent site/HTTP/JVM blockers.
- **Verified:** Eleven sources completed both stages; every one of the 39 non-passes replayed sequentially. Exact-request checks, four targeted Chromium checks, upstream Legado/rulebook comparison, and captured counterfactual Search+Book Info runs classified all 50 identities. The verifier independently recomputes ranking/disjointness, exact frozen definitions and hashes, the complete histogram, replay coverage, browser evidence, and the exact allowed mutation in both recovering counterfactuals.
- **Watch out:** Audit findings are evidence-only. Present the two recoverable seams for approval before implementation; keep unsupported `<js>`, `jsLib`, WebView, and Rhino/JVM observations separate until a current satisfiable workflow proves recovery.
