...
**Next action:** Follow LC-004 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: add a focused multi-request session regression for `enabledCookieJar`, then centralize automatic cookie persistence policy at the shared source-session/transport boundary. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
