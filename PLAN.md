...
**Next action:** Continue LC-008 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: pre-update ordering and direct typed book/TOC URL mutation are complete; design the smallest recursion-guarded detail-refresh bridge for upstream's pre-update-only `java.refreshTocUrl()` before implementing it. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
