...
Audit checkpoint (2026-07-15, updated 2026-07-16): final/source URL continuity, `bookInfoInit`, complete typed book/current/next-chapter context, TOC semantics, fail-closed TOC pagination, content pagination, and declared-rule-first extraction are complete with deterministic coverage. The required raw-source first/middle/last matrix is missing. API failure propagation and the broader conformance matrix remain partial. Close them in this order: API failure propagation; conformance matrix; dead compatibility fallback cleanup.
...
**In progress:** TOC and content crawl semantics are complete for the deterministic slice. Content supports Legado's single-chain versus fixed multi-page modes, typed fail-closed pagination errors, request options/retries/bodyJs, cycle and TOC-boundary handling, and declared-rule-first SPA diagnostics. Full Go tests and analyzer/book/sourceexec/API/conformance race tests pass.

**Next action:** Close API failure semantics: preserve typed TOC/content diagnostics at HTTP boundaries, distinguish incomplete crawl failures from invalid input/not-found responses, and add deterministic handler tests before the raw-source first/middle/last E2E matrix.
