...
**Next action:** Run full deterministic/race/vet/frontend/build verification after preserving the legacy behavior that malformed non-JS header literals are ignored, validate and commit v7 rerun evidence, then complete final Standards/Spec review and report. Do not rewrite stale source contracts.
...
- **Watch out**: Helper method names override imported metadata keys so a source cannot replace session/cookie behavior; metadata remains source-scoped and must not leak between pooled runtimes.
