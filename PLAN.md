...
The completed [architecture and code quality improvements](docs/plans/2026-09-05-architecture-code-quality-improvements.md) workstream corrected lifecycle/isolation defects, upload/font/identity contracts and reader-handler ownership, and implemented measured narrow chapter/progress lookups. That checkpoint introduced reader schema epoch 9; the active library cutover advances it to epoch 10. Its plan records scoped verification and the approved local-only integration; hosted CI and deployment verification remain unperformed. Frontend decomposition stays evidence-gated.

## Active Work

[Multi-provider library and imported books](docs/plans/2026-09-10-multi-provider-library.md) — methodical implementation is active. TXT analysis/range reading, per-home snapshot/file-mutation coordination, and composable BookSource catalog transactions are implemented and tested. The epoch-10 shared-library cutover is implemented through runtime, provider-neutral list/detail APIs, separate BookSource context, and revision-aware reader consumers. Full backend tests, affected race tests and frontend build/203 tests pass; focused independent review and epoch/backup/cascade checks are complete. The isolated TXT component now has tested receipt recovery, persisted interpretation, guarded library admission, indexed reads, shared state and retryable removal. It remains unregistered in production runtime/schema. Portable-reference validation is tested at snapshot/restore boundaries. Rename-first inbox acquisition and portable journal stripping are component-tested. Guarded leftover confirmation, explicit release/re-acquisition, and an independent bounded TXT analysis pool are component-tested, including affected race tests and foreground-capacity separation. Production worker/recovery wiring, provider-aware routes and import UI remain unfinished. Backup serialization and confirmation of uncertain inbox cleanup are confirmed. Next is bounded worker/runtime integration before TXT intake exposure; see the plan for boundaries and verification.
...
1. Continue the [multi-provider library workstream](docs/plans/2026-09-10-multi-provider-library.md#resume-here) at delivery step 2: implement the coordinated shared-state cutover around the tested TXT interpretation, snapshot coordination, and caller-owned catalog transactions; record the exact schema/revision and recreation contract before schema edits. Resolve consequential choices before schema/lifecycle edits; do not repeat broad reassessment or reopen settled product choices.
