...
**In progress:** `/api/healthz` now verifies SQLite readiness under a one-second deadline, returns a stable 503 without leaking database errors, and is covered through the registered route. Container delivery remains targeted at two public `linux/amd64` GHCR images with optional private WebView.

**Next action:** Add graceful SIGTERM/SIGINT server shutdown, then prove the runtime contract in rootless production images.
