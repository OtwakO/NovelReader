...
**Next action:** Plan LC-012 in `docs/LEGADO_COMPATIBILITY_TASKS.md` before implementation: define explicit consent, idempotency, failure reporting, and credential/payment safety for `payAction`; ordinary content fetch must not trigger a transactional action. LC-011 is complete for the audited browser resource-sniffing contract. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
