...
**In progress:** The ordered reading workflow is complete: canonical progress, safe source switching, annotated bookmarks, and bounded offline chapter fallback. Successful reads update exact book/source/raw-index/chapter-URL processed copies; requests remain network-first, then expose an `Offline copy` notice on fallback. Transactional LRU retention is capped at 100 chapters per book and 500 globally; late requests cannot recreate cache after deletion/source change, and empty extraction cannot overwrite valid content. Fresh 390px Playwright cached a 60-paragraph chapter, forced upstream HTTP 503, reloaded the same content with the explicit offline notice, and recorded no console warnings/errors; prior progress, source-switch, and bookmark evidence remains valid. Full backend, race, frontend tests, and warning-free production build pass. Phase 7 remains complete with its raw index-1 live evidence, deterministic WebView coverage, and catalog-load measurements recorded in the preceding history. Broad aesthetic polish remains deferred as requested; the unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** The requested four-part reading-state sequence is complete. Resume Phase 6 with the source-debug API and redacted request/response preview, unless frontend aesthetic polish is reprioritized.
