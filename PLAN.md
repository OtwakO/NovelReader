...
**Latest audit:** Search → Book Info live audit v1 sampled 25 unrestricted deterministic identities from `test_booksource3.json` with fixed query `凡人修仙传`. Ten completed both stages; one currently working source confirmed a shared detail-stage gap where lenient Legado URL-option objects with unquoted keys were encoded in the URL instead of becoming request metadata. The shared parser now preserves balanced search-result option suffixes and normalizes bare map keys before typed request-option decoding. A fresh raw-267 search and Book Info replay succeeds; evidence is in `testdata/booksource/audits/search-bookinfo/search-bookinfo-live-v1-fixes-rerun-2026-08-11.json`. Fourteen audit outcomes remain upstream, blocked/authenticated, stale/drifted, or legitimately empty. The separately observed whole-URL `<js>` difference remains unimplemented because its sampled source requires login and did not prove working impact.
