...
**Next action:** Continue LC-007 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: explicit CSS `ownText` is complete; verify and implement the focused `textNodes` getter through an Analyzer public-seam regression next. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
