...
**In progress:** Shared compatibility phases 1–2 are complete. Default traversal/exclusion is generic and per-parent. Strict `||` now falls through only on no-match or typed JSON compatibility errors while preserving JavaScript/cancellation failures; blank connector branches are ignored. Single-brace JSON interpolation supports multiple placeholders and existing regex replacement. An intermediate invalid-JSON extraction may feed an empty value only to an immediately following JavaScript default, while terminal JSONPath errors remain loud. `@js:` bodies are protected from connector splitting. Captured tests cover raw 76/82/410/429/471/742/816 shapes, and analyzer/book plus analyzer race suites pass. The focused Java bridge phase remains; authenticated Pixiv/browser/cache/login work for raw 134 stays explicitly deferred. Unrelated manual-testing changes in `frontend/dist` and `frontend/package-lock.json` remain untouched.

**Next action:** Add failing focused bridge tests for `toNumChapter`, non-blocking `toast`, and Legado URL-option POST/header/body handling in `java.ajax`, then implement them by sharing URL-option decoding without nested JS evaluation.
