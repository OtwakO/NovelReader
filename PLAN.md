...
**Latest Search → Book Info audit:** v3 sampled 50 unrestricted deterministic identities from unchanged `test_booksource3.json`, disjoint from all 50 v1/v2 identities, with fixed query `凡人修仙传`. Exactly the frozen raw definitions were executed through the production conformance path; all 39 initial non-passes replayed sequentially. Eleven completed both stages. The approved shared fixes are now complete: standalone OnlyOne regex replacement returns only the first matched-and-replaced value, declared GET-query charset encodes query variables before execution, whole/inline URL `<js>...</js>` segments execute through the existing URL-JavaScript evaluator, and Search/Explore URL construction receives source `jsLib`. Exact untouched frozen reruns recover raw 375 and raw 364 to 20 Search results plus successful Book Info. URL `<js>`/`jsLib` support is deterministic-only for the sampled affected sources because they retain independent site/HTTP/Rhino blockers; no live recovery is claimed for raws 46/47/51/192. WebView and Rhino/JVM remain separate. Audit evidence is `testdata/booksource/audits/search-bookinfo/search-bookinfo-live-audit-v3-2026-08-12.{json,md}` and post-fix evidence is `search-bookinfo-live-v3-fixes-rerun-2026-08-12.json`.
