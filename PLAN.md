...
[Multi-provider library and imported books](docs/plans/2026-09-10-multi-provider-library.md) — shared library ownership, revision-aware BookSource consumers, managed TXT receipt/interpretation/publication/removal, portable-reference validation, and guarded inbox resolution are implemented and tested. **Reader epoch 11** registers TXT storage plus an independent bounded analysis pool, startup/restore recovery, combined restore/deletion barriers and joined shutdown. Restore reconciliation problems produce a successful replacement with a visible warning, not a misleading failed-restore result. Full backend tests, affected race tests, frontend build and 12 targeted frontend tests pass. No existing data was changed or deployment performed. Provider-aware TXT reading/management routes, bounded intake admission, HTTP review-proof retention and import UI remain pending; the generic delete route rejects unrouted providers to preserve their owned data. See the scoped plan for the next concrete boundary and exact verification/rollback limits.
...
1. Continue the [multi-provider library workstream](docs/plans/2026-09-10-multi-provider-library.md#next-action) through provider-aware TXT reading/management routes, then bounded intake and the core import/review UI. Reuse the completed storage and lifecycle owners. Intake must not consume API runtime slots or bypass reader quiescence. Do not repeat broad reassessment or reopen settled product choices.
