...
**Next action:** Phase 2.2 computation policy is complete: usernames use trimmed NFKC → case-fold → NFKC normalization with 3–32 permitted code points; passwords remain untrimmed/unnormalized at 12–128 valid Unicode code points; and light Argon2id uses 19 MiB, 2 iterations, parallelism 1, fresh salts, fixed PHC parsing, two process-wide slots, dummy verification, and quick overload refusal. Proceed with the next bounded Phase 2.2 slice: account creation/authentication lookup in `system.db`, normalized-username uniqueness, generic credential failure behavior, password replacement with transactional session revocation, and account-status checks. Browser token creation/rotation remains a separate following slice; HTTP routes, cookies, and Reader Data ownership remain closed. This is a clean replacement; no legacy migration is in scope. The accepted model uses `system.db`, plaintext self-contained per-user `reader.db` directories, separately encrypted non-exportable source credentials, one-time browser Administrator setup, stored `reader`/`admin` roles, environment recovery token, 30-day rotating sessions with bounded grace, and protected Administrator accounts. LC-016 remains blocked until this boundary exists. LC-015 is complete.
