...
**Next action:** Commit the green generic TDD slices for complete Default bracket grammar and attributes, received non-2xx Explore body evaluation with diagnostics, and exact byte-array crypto/login bridge contracts. Then correct v12 raw 251 classification, exact-rerun verified raws 197, 163, and 17, and run full verification/review.
...
- **Watch out**: Implemented generic public contracts rather than raw-specific branches: arbitrary terminal attributes, complete singleton/list/exclusion/inclusive-range/step/negative bracket grammar including root-child indexing, nonblank deduplicated attributes, received HTTP-status bodies with explicit warning diagnostics, nullable base64 bytes, AES/CBC PKCS padding crypto object methods, and source-scoped login-header storage/map access. No hostname-based UA inference, no `title`-only getter patch, no `a[1]` special case, and no conversion of DNS/TLS/timeouts into parseable responses.
