...
**In progress:** Canonical progress, safe alternate-source switching, and bookmarks are complete. Bookmarks use idempotent client IDs, exact readable raw chapter locations, normalized positions, optional 1,000-rune notes, and a Reader add/open/delete panel. Source switches migrate only normalized-title matches; unmatched bookmarks remain visible and disabled as explicit orphans, and recover when a later source restores their title. Version guards prevent stale capture during switches. Fresh 390px Playwright saved an annotated bookmark at 0.55, reloaded it, opened the same chapter/location at 0.5502, deleted it, and recorded no console warnings/errors; prior Primary ↔ Target source-switch evidence remains valid. Full backend, race, frontend tests, and warning-free production build pass. Phase 7 remains complete with its raw index-1 live evidence, deterministic WebView coverage, and catalog-load measurements recorded in the preceding history. Broad aesthetic polish remains deferred as requested; the unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Implement bounded server-side offline chapter caching with explicit retention and upstream-outage fallback. Frontend aesthetic polish remains a dedicated later pass.
