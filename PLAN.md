...
[Multi-provider library and imported books](docs/plans/2026-09-10-multi-provider-library.md) — methodical implementation is active. TXT analysis/range reading, per-home snapshot/file-mutation coordination, and composable BookSource catalog transactions are implemented and tested. The epoch-10 shared-library cutover is implemented through runtime, provider-neutral list/detail APIs, separate BookSource context, and revision-aware reader consumers. Full backend tests, affected race tests and frontend build/203 tests pass; focused independent review and epoch/backup/cascade checks are complete. A TXT upload-receipt component now has focused acquisition/recovery/discard and pending-file portability tests, but is not registered in production runtime/schema. TXT analysis/admission integration, inbox acquisition, workers, import UI and publication-aware backup validation remain unfinished. Backup serialization and confirmation of uncertain inbox cleanup are confirmed. Next is persisted TXT analysis/admission and portable-reference validation before intake exposure; see the plan for boundaries and verification.
