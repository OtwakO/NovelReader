...
**Latest audit:** Search → Book Info live audit v2 sampled 25 unrestricted deterministic identities from `test_booksource3.json`, disjoint from v1, with fixed query `凡人修仙传`. Exactly the frozen raw definitions were imported so duplicate `bookSourceUrl` keys could not replace sampled contracts. Eight completed both stages. Two currently working responses confirm separate shared gaps: URL-valued Search fields use generic newline-joining instead of Legado first-value URL semantics (raw 49), and an empty Search list does not fall back to Book Info on the final response page (raw 179). Raw 396 explicitly depends on deferred WebView cookie-challenge execution and is not promoted as a regular/JS source regression. The other 14 outcomes are upstream HTTP/timeouts, blocked/authenticated, stale/drifted, or legitimately empty. Evidence is in `testdata/booksource/audits/search-bookinfo/search-bookinfo-live-audit-v2-2026-08-11.{json,md}`. No production behavior changed during the audit; both shared fixes require user approval.
