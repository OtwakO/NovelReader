...
**In progress:** Phase 7's domain gate is complete. Session-bound `text`, `button`, `toggle`, and `select` updates now validate client values, retain `infoMap` state, execute bounded source actions, honor `java.refreshExplore()`, atomically regenerate catalogs, reset page state, and invalidate queued stale entry IDs with a new generation. The exact index-916 select refreshes from 完结榜 to 收藏榜 in the same opaque session. Actions, raw rules, and failure causes remain server-side. A root `run-local.bat` now installs/builds the frontend and starts the Go server for Windows development. The unrelated local `AGENTS.md` modification remains uncommitted and untouched.
