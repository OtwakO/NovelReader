...
**Next action:** Finish LC-008 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: pre-update ordering, direct typed mutation, and workflow-scoped `java.refreshTocUrl()` are complete; implement `java.reGetBook()` through the existing precise-search then detail-refresh workflows and close the item after targeted verification. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
