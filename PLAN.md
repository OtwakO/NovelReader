...
- **Decision**: Audit only. Recommend one shared URL-builder TDD fix after approval. Do not patch either source and do not bundle the separately observed whole-URL `<js>` difference, which appeared only on a login-required source returning anonymous HTTP 401.
- **Verified**: All 25 identities have initial evidence; all 15 non-passes have sequential replay; counts, stable identities, frozen raw indices, corpus SHA-256, JSON parsing, and source/parser non-mutation are verified by `scripts/search-bookinfo-audit/v1/verify.mjs`.
