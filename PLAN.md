...
**Atomic ownership cutover plan (in progress):** Completion means the server has only public health/static/setup/auth/recovery boundaries before identity; every Reader Data API request authenticates first, derives immutable `UserID` only from context, opens that user's `readerstore.Home`, and constructs feature access exclusively from `reader.db`/`files/`. The cutover will: (1) register the complete reader schema through `readerstore` so home creation and opening validate/migrate feature tables centrally; (2) revise font files to use `Home.Files()` and remove global path construction; (3) introduce request-scoped reader dependencies while retaining process-wide fetch/JS/admission capacity and user-scoping all workflow/Explore sessions; (4) mount auth, setup, configured recovery, and authenticated Reader Data routes in one server composition; (5) delete the legacy global database startup/configuration and add equal-ID cross-user isolation plus unauthenticated-route regressions; and (6) mount the frontend account gate only after the backend boundary is complete. Rollback is the branch commit before this plan; no legacy Reader Data migration or dual-mode fallback will be added.

**Next action:** The Phase 2.2 authentication core and fail-closed HTTP module are complete. The unmounted module provides strict bounded login, persistent secure-cookie sessions, logout-one, request-context identity, `RequireIdentity`, `RequireAdmin`, direct-peer rate limiting, and bounded response deadlines; the live server has replaced wildcard CORS with browser-origin/request-host matching and an optional exact `PUBLIC_URL` override for host-rewriting proxies. Proceed with the atomic ownership cutover: mount authentication, first-Administrator setup, and configured Administrator recovery only while wrapping every non-public Reader Data route and replacing global feature stores with stores selected by immutable Reader Account ID. Public registration remains closed. This replaces the earlier 30-day rotating design outright; no legacy migration is in scope. The accepted model uses `system.db`, plaintext self-contained per-user `reader.db` directories, separately encrypted non-exportable source credentials, one-time browser Administrator setup, stored `reader`/`admin` roles, an environment recovery token, persistent independently revocable sessions, and protected Administrator accounts. LC-016 remains blocked until this boundary exists. LC-015 is complete.
