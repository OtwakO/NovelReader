...
**In progress:** A second Explore compatibility audit is planned against a disjoint deterministic 50-source sample from the same hash-pinned raw corpus, using seed `NovelReader-explore-audit-v2`. The sample excludes the first 50 identities and malformed catalogs, deduplicates source URLs, and covers catalog, request, transport, and result-rule strata. It will execute against the engine containing only the first approved shared fixes. This is audit-only: no production parser behavior or raw source rule will change before findings are reported. Unrelated manual-testing changes in `frontend/dist` and `frontend/package-lock.json` remain untouched.

**Next action:** Run the v2 sample in a fresh isolated database, preserve per-source engine evidence, reproduce failures with direct browser evidence, classify shared gaps versus stale/upstream failures, and report before proposing or implementing fixes.
