...
**In progress:** Batched SSE now validates batch and concurrency inputs before streaming, emits start/per-source/done progress metadata, returns retry and continuation cursors, reports effective concurrency, and rejects stale source revisions before executing source work. Query-only requests retain the legacy results/merged/done contract. Frontend cumulative merging is next.

**Next action:** Add failing dependency-free frontend tests for exact retry deduplication, cross-batch same-book merging, alternate preservation, primary promotion, and relevance ordering.
