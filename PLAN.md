...
1. The [core TXT workflow](docs/plans/2026-09-10-multi-provider-library.md#next-action) is implemented and verified at the recorded scope. Separately scope deferred advanced controls/reparse before starting another increment; do not add more core architecture as a prerequisite. Reuse the completed admission, reading, storage and lifecycle owners. Transfers must not consume API runtime slots or bypass reader quiescence. Do not repeat broad reassessment or reopen settled product choices.
