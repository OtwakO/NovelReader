...
**Next action:** Commit raw-927 live evidence and documentation, run full backend deterministic/race/vet plus frontend tests/build, then complete Standards/Spec review.
...
- **Problem**: Raw 927 has blank `ruleExplore.bookList` and a complete `ruleSearch`; the live page contains 28 valid cards, but production returns zero because the first matched title element is empty.
- **Fix**: Audit diagnosis was corrected during public-seam TDD: the whole-search fallback already works; the actual shared mismatch is recorded in the entry above.
...
- **Watch out**: Preserve the evidence history; use the corrected root cause above for implementation and follow-up.
