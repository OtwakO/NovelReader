...
1. Implement the [EPUB content/navigation contract checkpoint](docs/plans/2026-09-17-epub-support.md), orientation-aware images, labeled cover placeholders and representative payload/cache measurements are now verified; auxiliary bookmark reopen is also verified through the shared reader; image-memory measurement is complete and the existing 32 Mi-pixel cap is accepted; optional WebP 92 / 2048-pixel image optimization is accepted (Original remains default). Shared imageproc optimization and EPUB caller-owned derivative emission are implemented, with once-per-resource conversion and separate fallback evidence; durable storage/lifecycle integration is pending. The controlled Go/Alpine proof justifies native encoding with an explicitly disclosed portable fallback. Native container encoding and allocation-lifetime checks pass. Epoch-14 fresh-data storage work is authorized; original images will be served from retained EPUB members through the verified bounded read helper. Epoch 13 remains active pending coherent persistence/recovery/portable integration. Attempt-owned staging, acquisition recovery and persisted preparation-generation claims/retries now pass isolated tests; the DDL remains unregistered. The indexed section-stream representation is accepted; staging and bounded range reads now pass normal/race/portable-encoder tests. SQLite section/resource persistence, ready-state installation and startup reconciliation now pass isolated tests. Next implement EPUB portable validation/snapshot hooks and export/restore limit compatibility before schema/application activation, then finalize the supported profile/contract; confirm schema cutover before storage changes. TXT/shared-library work is complete at its recorded verification scope; do not reopen its historical plan. Existing data remains untouched; the user handles development resets.
