...
**In progress:** The Phase 7 live compatibility audit is complete and awaiting user review before fixes. A fixed-seed stratified 50-source sample from the hash-pinned raw 939-source compilation produced 20 credible non-empty passes, 16 shared result/analyzer gaps, 2 advanced Java-bridge gaps, 3 invalid raw scripts, 8 upstream/WAF/stale-rule failures, and 1 legitimate empty upstream response. Direct Playwright reproduced the reported symptoms: raw 15 `笔趣阁 · 常` exposes 50 upstream nodes but the engine returns zero because it filters relative links before URL resolution; raw 89 `中文看书（优）` exposes 23 nodes but the API/UI show one concatenated book; both `夜伴书屋` identities currently end in a lander or HTTP 403. The UI faithfully renders API output and emitted no warnings/errors. Evidence and all 50 classifications are in `testdata/booksource/explore-live-audit-2026-07-18.md`. No production behavior or source rule changed. The ordered reading workflow remains complete and verified. Unrelated manual-testing changes in `frontend/dist` and `frontend/package-lock.json` predate this audit and remain untouched.

**Next action:** After user approval, capture minimal fixtures and fix URL resolution before pattern filtering plus Default-mode detection first; rerun affected strata and the fixed 50-source manifest before proceeding to connector/range and JSON compatibility gaps.
