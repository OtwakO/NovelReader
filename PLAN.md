...
**Next action:** Report the completed sixth Explore fixes. The fail-closed header parser is separate, the pre-existing broader `@put` grammar is restored with a regression, live/corpus verification and all full checks are complete, and final Standards and Spec reviews approve the diff. Do not rewrite stale source contracts.
