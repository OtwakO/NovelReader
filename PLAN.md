...
**In progress:** Deterministic `docker-e2e.sh` verification now builds both production images and proves frontend assets, SQLite readiness, private app-to-worker health, JavaScript-rendered search through a local fixture, zero-exit graceful stop, container recreation, and named-volume source persistence. The profile publishes no fixture or worker host port and contacts no live novel site.

**Next action:** Add the SHA-pinned GitHub Actions workflow that verifies the repository and publishes both lowercase GHCR images on `main`, `v*`, and manual dispatch.
