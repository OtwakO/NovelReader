...
**In progress:** Phase 4 is closed. The completed matrix protects declared-rule TOC/content behavior while dead URL-option/content-pagination wrappers and heuristic TOC/script replacements have been removed. Required Legado fallbacks remain: empty `tocUrl` uses the book page, source defaults remain compatible, and script diagnostics remain available only when no content selector is declared. Full Go tests and analyzer/book/sourceexec/API/conformance race tests pass.

**Next action:** Start Phase 7 by auditing Legado Explore URL/category grammar, raw-source shapes, paging/context semantics, and failure behavior before defining typed contracts.
