...
**Next action:** Commit the corrected Default getter fix with PLAN.md, live-rerun raw 927 through production Explore, then run full backend/frontend verification and Standards/Spec review.
...
- **Watch out**: Preserve per-element order and repeated text values; only attribute getter values are de-duplicated. Do not field-merge `ruleExplore` and `ruleSearch` or weaken URL-pattern enforcement.
