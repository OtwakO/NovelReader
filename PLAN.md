...
**Status:** active; Phase 4's crawl-pipeline completion gate passed on 2026-07-16.
...
- [ ] Add deterministic raw-source fixtures covering normal HTML, JSON, XPath/Regex, POST/charset, JavaScript, and WebView exploration where those capabilities occur in real sources.
  ...
  - [ ] Add captured/local response fixtures with executable request, paging, and result expectations.
- [ ] Implement the smallest Explore domain service on the existing executor/analyzer/session boundary; do not route it through search fan-out or duplicate request/rule logic.
  ...
  - [ ] Execute top-level JavaScript category generators and interactive control actions with the same session bindings.
...
**In progress:** Phase 7's first URL-category domain slice now lists independently eligible sources, opens bounded opaque sessions, and executes source-scoped pages through existing request options, routing transport, source state, and uncapped shared result parsing. Paging is serialized, capacity-bounded, leased against eviction, idempotently replayable, deduplicated across pages, and supports whole-rule search fallback, reverse lists, and single-book detail pages. Top-level category JavaScript remains an explicit typed unsupported error. The unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Make the remaining raw execution classes deterministic: top-level JavaScript category generation first, then JSONPath, XPath/Regex, POST/charset, and WebView-routed page fixtures. After those pass, implement interactive `text`/`button`/`toggle`/`select` actions before API/frontend exposure.
...
- **Watch out**: Script-generated categories and interactive controls are still intentionally unavailable and must not be exposed as working until their session-bound fixtures pass.
