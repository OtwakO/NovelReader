...
**Next action:** Begin LC-009 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: verify upstream `ruleToc.formatJs` execution order and chapter context, then pin one public TOC formatting regression before implementation. LC-008 is complete for the audited upstream contract. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
