...
**In progress:** The SHA-pinned GitHub Actions workflow runs backend/race, frontend, worker, and Compose E2E gates before authenticating to GHCR. It lowercases `github.repository_owner` and publishes app/worker `linux/amd64` images with `edge` on `main`, semantic versions on `v*`, `manual` on dispatch, and immutable SHA tags. Actionlint passes locally; publication cannot run until the future remote has a `main` branch.

**Next action:** Document GHCR package visibility, image tags, Compose pull/build commands, persistent-data ownership/backup, health, and HTTP-only/WebView operation.
