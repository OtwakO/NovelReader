...
**In progress:** Stateless batch planning now deterministically orders eligible sources, hashes their ordered identities, validates versioned cursors, rejects stale revisions, selects exact non-overlapping partitions, and clamps requested concurrency to the deployment per-search limit. Batch execution reuses the existing global admission and source workflow. SSE exposure is next.

**Next action:** Add API tests for parameter validation, start/progress/done events, continuation, stale revisions, and legacy stream compatibility.
