...
**Next action:** Re-run focused/full verification after review-requested semantic tests and evidence corrections, obtain final Standards/Spec approval, then report completion.
...
- **Watch out**: Preserve per-element order and repeated text values; only attribute getter values are de-duplicated. Do not field-merge `ruleExplore` and `ruleSearch` or weaken URL-pattern enforcement. Fresh raw 927 production rerun returns 21 distinct books.
