...
Audit checkpoint (2026-07-15, updated 2026-07-16): final/source URL continuity and `bookInfoInit` are complete. The required raw-source first/middle/last matrix is missing. TOC field semantics, TOC pagination diagnostics, content pagination, complete rule context, and declared-content fallback behavior are partial. Close them in this order: book-info initialization; centralized complete analyzer context; TOC model/field semantics; TOC pagination result contract; content correctness; API failure propagation; conformance matrix; dead compatibility fallback cleanup.
...
**In progress:** `bookInfoInit` is complete. Structured selector/JSON/JavaScript/JSoup/XPath initialization, context-sensitive fragments, null failure, connector semantics, ordered `@put`/`@get`, lenient raw maps, session persistence, dynamic replacements, and all-in-one regex captures have deterministic coverage. Full backend tests and analyzer/book race tests pass.

**Next action:** Begin the centralized analyzer-context slice: add failing tests for complete book/current chapter/exact next chapter bindings in detail, TOC item rules, TOC pagination, content, and content pagination; then pass typed workflow context through the shared boundaries without source-global chapter guessing.
