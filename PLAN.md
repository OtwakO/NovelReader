...
[Multi-provider library and imported books](docs/plans/2026-09-10-multi-provider-library.md) — the core TXT user workflow is complete: browser/inbox acquisition, bounded processing, dedicated review and explicit bulk acceptance, shared reading/bookmarks, guarded cleanup and portable restore. A feature-owned frontend queue continues across navigation without browser parsing or persistent file copies. Scoped frontend lint/build and 27 tests pass; isolated real-server desktop/mobile checks cover batch upload, inbox intake, admission, reading, removal and content/bookmark restore. Earlier backend normal/race evidence remains recorded in the plan. The accepted advanced TXT work has completed its epoch-12 storage-separation checkpoint, with affected normal/race backend suites passing. No migration layer or existing-data changes. Custom matching for pending imports is also implemented and verified. Published candidate preparation, impact review and atomic Apply are now verified at the storage boundary. Revision-coherent reader loading, qualified navigation and stale-session shutdown are also verified. Reparse HTTP/UI controls are next; EPUB remains later. No host-load or 1,000-user throughput claim is made.
...
1. The core TXT workflow is implemented and verified at the recorded scope. The [custom-pattern and safe-reparse design](docs/plans/2026-09-10-multi-provider-library.md#advanced-txt-patterns-and-reparse--design-proposal) is accepted, including epoch 12 without migrations and the reviewed public contracts. Storage separation is verified. Custom matching for pending imports is verified. Published-reparse storage transactions are verified. The reader's coherent-load and revision-qualified navigation prerequisite is also verified. Next connect reparse HTTP/UI controls using these boundaries; reparse remains unexposed until that integration. The user handles development resets; do not modify existing data. Reuse the completed admission, reading, storage and lifecycle owners. Transfers must not consume API runtime slots or bypass reader quiescence. Do not repeat broad reassessment or reopen settled product choices.
