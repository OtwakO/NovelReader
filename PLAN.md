...
**Maintainability cleanup backlog:** `docs/CODEBASE_CLEANUP_AUDIT.md` records the evidence and required contract decisions for a future behavior-neutral cleanup of dead/remnant code. Initial confirmed active-tree candidates are the superseded Vue `addBook`, `addReadableBook`, `enrichBook`, `searchBooks`, and `searchBooksStream` wrappers. Corresponding backend endpoint removal is not approved until the HTTP API is explicitly classified as private or externally supported. Unused-symbol reports remain hints rather than proof because Vue templates, Pinia aliases, routes, types, and test-only entry points produce false positives.
...
| Discovered-book detail | Tab-scoped non-persistent preview; canonical stored detail remains ID-based | Inspecting a result must not imply shelf ownership, while stored reading/progress contracts keep one explicit identity. |
