...
**In progress:** Legacy `SearchStream` now delegates to a focused shared bounded fan-out while retaining its public signature, all-source candidate selection, capacity accounting, global admission, callbacks, cancellation, and diagnostics. Cursor/revision batch selection is next. System-wide concurrency remains bounded, observable, and covered by deterministic 400-source concurrent search and registry stress tests; Docker-constrained measurement remains pending after this feature slice.

**Next action:** Add failing cursor, source-revision, deterministic-order, and exact-partition tests before implementing batch selection.
