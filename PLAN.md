...
**In progress:** Phase 7's domain gate is complete. Session-bound `text`, `button`, `toggle`, and `select` updates now validate client values, retain `infoMap` state, execute bounded source actions, honor `java.refreshExplore()`, atomically regenerate catalogs, reset page state, and invalidate queued stale entry IDs with a new generation. The exact index-916 select refreshes from 完结榜 to 收藏榜 in the same opaque session. Actions, raw rules, and failure causes remain server-side. Root `run-local.bat` and `run-webview-worker.bat` launch the app and optional private browser worker separately for Windows development; the app launcher targets the local worker automatically. The unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Add thin stable API endpoints for Explore source listing, catalog opening, control updates, pages, and safe typed diagnostics; then build the accessible per-source frontend flow.
