...
1. Implement the [EPUB content/navigation contract checkpoint](docs/plans/2026-09-17-epub-support.md), orientation-aware images, labeled cover placeholders and representative payload/cache measurements are now verified; auxiliary bookmark reopen is also verified through the shared reader; image-memory measurement is complete and the existing 32 Mi-pixel cap is accepted; optional WebP 92 / 2048-pixel image optimization is accepted (Original remains default). Shared imageproc validation/orientation and bounded WebP optimization are implemented; EPUB still uses original images until derivative/lifecycle integration lands. The controlled Go/Alpine proof justifies native encoding with an explicitly disclosed portable fallback. Native container encoding and allocation-lifetime checks pass. Next integrate immutable image policy, bounded derivative emission and explicit fallback evidence into preparation, then finalize the supported profile/contract; confirm schema cutover before storage changes. TXT/shared-library work is complete at its recorded verification scope; do not reopen its historical plan. Existing data remains untouched; the user handles development resets.
