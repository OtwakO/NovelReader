...
**In progress:** The user approved the shared v2 compatibility fixes: Default traversal/exclusion, connector and interpolation semantics, optional JSONPath-to-JavaScript fallback, focused `toNumChapter`/`toast` helpers, and `java.ajax` request options. Authenticated Pixiv/browser/cache/login work for raw 134 is explicitly deferred. Implementation will follow captured-fixture TDD in four atomic green phases; no source-specific production logic or raw source rewrites are permitted. Unrelated manual-testing changes in `frontend/dist` and `frontend/package-lock.json` remain untouched.

**Next action:** Capture reduced failing rule/response fixtures and tests for the approved raw shapes, then implement Default traversal/exclusion first.
