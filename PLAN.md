...
**In progress:** Batched streaming search is implemented end to end. The 2-vCPU/4-GB Docker HTTP-search gate exposed and corrected per-source fingerprint connection retention; after the fix, 48,000 deterministic source requests across 16 concurrent users completed without failures, retained established connections, OOM, or restart. Median batch latency remained about 2.59s, p95 at or below 2.82s, and observed backend memory peaked at 27.32 MiB. Current 16-per-search/32-global limits remain conservative.

**Next action:** Run the separately constrained Patchright worker gate before closing small-container measurement.
