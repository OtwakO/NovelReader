...
**Next action:** Begin LC-008 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: verify upstream `ruleToc.preUpdateJs` ordering, context, and permitted URL/book mutations, then pin one public TOC workflow regression before implementation. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
