...
**Next action:** Audit LC-015 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: inventory current chapter `imageDecode` handling, active scripts, and the preserved chapter-image/resource output contract before expanding type-2/image behavior. LC-014 is complete for stored-book covers through a server-derived, bounded decode endpoint; search and Explore previews remain explicitly out of scope. LC-013 is dormant and intentionally unimplemented, while LC-012 remains intentionally deferred. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
