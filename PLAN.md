...
**Next action:** Commit this implementation plan, complete four red-green slices at the approved Analyzer and Explore seams, rerun exact raws 701/54/220/443 through production, run full verification/review, update evidence and report.
...
- **Fix**: User approved test-first fixes at existing Analyzer and production Explore seams. Timeout configuration is `SearcherLimits.ExploreSourceTimeout` plus positive integer `EXPLORE_SOURCE_TIMEOUT_SECONDS`, default 30 seconds; search's ten-second source deadline remains unchanged.
