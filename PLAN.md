...
**In progress:** Rootless `linux/amd64` production images now build from the latest official base tags. The combined app image is 15 MB, runs as `10001:10001`, contains the tested frontend and static Go binary, owns writable `/data`, reports SQLite readiness, and exits zero on graceful stop. The 521 MB Patchright image runs as UID 10001, keeps port 8787 internal, reports browser readiness, and exits zero. Both image health checks pass.

**Next action:** Add the single Compose contract with GHCR/local image overrides, persistent `/data`, measured limits, and an optional private WebView profile.
