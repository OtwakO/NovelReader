...
[Multi-provider library and imported books foundation](docs/plans/2026-09-10-multi-provider-library.md) — accepted design for one provider-neutral library with cohesive BookSource and TXT ownership, one human-readable durable content file per TXT publication, a consuming bind-mountable inbox, asynchronous multi-method TOC analysis, bulk-ready review/optional automatic addition, complete owned-file deletion, and reader-home-level portable backup consistency. The branch `feat/multi-provider-library` still contains planning only; the next checkpoint is the concrete schema/interfaces and backup/delete coordination before production edits.
...
1. Define the concrete provider-neutral schema/interfaces, rooted publication-file operations, backup generation barrier/reference validation, complete deletion workflow, HTTP transition, and focused regression matrix for the accepted [multi-provider library workstream](docs/plans/2026-09-10-multi-provider-library.md) before production edits.
2. Extract provider capability interfaces only where BookSource and the first concrete TXT implementation create real variation; keep TXT ingestion, encoding, TOC methods/indexing/reparse and owned-file lifecycle cohesive, preserve one unified library with origin filters, and never let backup or successful deletion leave database/file generations inconsistent.
