...
**In progress:** Shared compatibility phase 1 is complete. Default mode now recognizes and executes `@tag.*`, `@.class`/`@#id`, and terminal `!index` selectors generically. Positional selection and exclusion are applied per current parent in document order, negative exclusions are supported, and CSS attribute getters remain unchanged. Captured-shape tests cover raw 342/516/614/897 patterns plus multi-parent indexing; analyzer and book suites pass. Connector/interpolation/mixed-mode and focused Java bridge phases remain. Authenticated Pixiv/browser/cache/login work for raw 134 stays explicitly deferred. Unrelated manual-testing changes in `frontend/dist` and `frontend/package-lock.json` remain untouched.

**Next action:** Add failing connector, interpolation, and optional JSONPath-to-JavaScript pipeline tests, then implement their shared semantics without swallowing terminal or required-field errors.
