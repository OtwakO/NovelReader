...
**In progress:** Batched streaming search is implemented end to end. The UI persists 1–500 source batch sizing and Gentle/Balanced/Fast or advanced concurrency, streams cumulative progress and merged results, retries interrupted batches without duplicate progress, rejects stale source revisions with Restart, and preserves alternate sources when shelving. Backend/domain/API tests, race tests, dependency-free frontend merge tests, production builds, and mocked-browser Search more, persistence, disconnect Retry, and stale Restart checks pass.
