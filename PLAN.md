...
**In progress:** The Phase 6 production-container slice is locally complete. Rootless app/worker images, SQLite readiness, graceful shutdown, one persistent Compose contract, optional private WebView, deterministic rendered-search E2E, SHA-pinned GHCR publishing, and deployment/backup documentation are implemented and verified. The workflow lowercases `OtwakO` to `otwako`; Actionlint passes. Actual GHCR publication and public visibility remain externally pending because no remote or `main` branch exists yet.

**Next action:** After the owner creates the remote and `main`, run `Verify and publish containers` manually, mark both GHCR packages public, and record the successful run before selecting the next diagnostics slice.
