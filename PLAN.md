...
**In progress:** All four user-approved v9 shared compatibility fixes are implemented through public Analyzer regressions, live-verified, and approved by final Standards and Spec reviews: raw 66 now returns 20 books, raw 57 returns 14, and raw 581 returns 51. Java `\uXXXX` regex escapes translate to Go-compatible code points while malformed escapes still fail; Default traversal uses an independent accumulator; dotted `.[*]` JSON wildcards normalize outside quoted strings; and simple root-array wildcard predicates preserve order. Evidence is in `testdata/booksource/explore-live-v9-fixes-rerun-2026-07-22.json`. Stale contracts and URL-pattern enforcement remain unchanged.
...
**Next action:** Report the completed ninth Explore fixes. Full backend deterministic/race/vet, frontend tests/build, live rerun evidence, and final Standards and Spec reviews approve the diff.
