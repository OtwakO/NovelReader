...
**Active audit:** Run the first bounded Search → Book Info live audit before further feature work. The frozen v1 scope is 25 unrestricted deterministic identities from `test_booksource3.json`, seed `NovelReader-search-bookinfo-random-v1-2026-08-11`, stable identity `(rawIndex, bookSourceUrl)`, and fixed query `凡人修仙传`. Each selected source gets page-1 production search followed only by Book Info for the first credible result; every non-pass is replayed sequentially. Classification treats the imported BookSource contract as authoritative while consulting the official rule documentation and `reference/legado`; only valid, live contracts that fail at a reusable NovelReader seam qualify as engine gaps. No source/parser fixes occur during sampling.

**Next action after the audit:** Present the Search → Book Info evidence and stop for approval. Then complete the two remaining ownership-boundary checks: centralized frontend handling for revoked/expired sessions, followed by one clean-data-root browser workflow covering setup → login → source import/search → add/read → logout and private-data denial. Registration, password change, ordinary-account administration, and durable reader deletion are complete. Public registration remains disabled by default. No legacy reader-data migration is in scope. After these checks, resume the paused compatibility queue at focused explicit CSS/Jsoup compatibility; LC-016 remains blocked until the ownership boundary is fully verified, and LC-015 is complete.
