...
[EPUB support](docs/plans/2026-09-17-epub-support.md) — implementation authorized for structured DRM-free reflowable EPUB 2/3 through browser and server-inbox import, using the shared library and prose reader. First checkpoint in progress: the user selected focused Go parsing after the Readium build-footprint proof. Bounded archive/XML and EPUB 2/3 package inventory inspection pass focused normal/race tests; navigation/semantic rendering remain unfinished. Schema cutover is gated separately; existing homes/backups remain untouched.
...
1. Implement the [EPUB content/navigation contract checkpoint](docs/plans/2026-09-17-epub-support.md), starting with the bounded parser reuse proof; confirm schema cutover before storage changes. TXT/shared-library work is complete at its recorded verification scope; do not reopen its historical plan. Existing data remains untouched; the user handles development resets.
