...
**In progress:** The user-approved first Explore compatibility fix batch is complete and ready for review. Book URLs are now resolved before `bookUrlPattern` filtering, and Default-mode detection recognizes standalone `class.*`/`id.*` plus CSS-shorthand element traversal without misclassifying CSS attribute getters. Captured-shape tests cover both defects. The fixed 50-source rerun repaired eight sources: raw 15 (0→50), 89 (1 concatenated→20), 99 (1 fallback→10), 286 (0→20), 370 (0→20), 418 (1 concatenated→50), 823 (0→20), and 909 (1→10). The stable credible non-empty baseline rose from 20/50 to 28/50; raw 869 also transiently recovered from HTTP 503, producing 29 live non-empty results. Raw 897 remains zero due to a separate exclusion-suffix gap, and raw 576 was corrected to stale-pattern classification. Fresh 390×844 Playwright verified `笔趣阁 · 常` at 50 books and `中文看书（优）` at 20 with no console warnings/errors. Full backend and analyzer/book race suites pass. Connector/range, JSON, Java bridge, stale-source, and upstream failures remain untouched. Evidence is in `testdata/booksource/explore-live-audit-2026-07-18.md` and its priority-fix rerun JSON. Unrelated manual-testing changes in `frontend/dist` and `frontend/package-lock.json` remain untouched.

**Next action:** Report the priority-fix results and stop for user review before beginning any connector/range, JSON, exclusion-suffix, Java-bridge, or source-rule work.
