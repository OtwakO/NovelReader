...
- [ ] Implement the smallest Explore domain service on the existing executor/analyzer/session boundary; do not route it through search fan-out or duplicate request/rule logic.
  ...
  - [ ] Execute interactive control actions with those session bindings.
...
**In progress:** Phase 7 now executes literal, legacy, and top-level JavaScript catalogs in bounded source sessions. The exact index-916 generator produces its select control metadata; local fixtures prove source headers, `java.ajax`, cache aliases, JSLib-compatible bindings, tagged scripts, typed generation/parse failures, CPU-loop interruption, and reuse of the same retained state for pages. Interactive actions remain nonselectable and intentionally unexecuted. The unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Make JSONPath, XPath/Regex, POST/charset, and WebView-routed Explore pages deterministic against the pinned source classes, then implement interactive `text`/`button`/`toggle`/`select` updates before API/frontend exposure.
