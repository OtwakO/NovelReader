...
**In progress:** Canonical reading progress and safe alternate-source switching are complete. A switch now accepts only a stored alternate, proves its book info and readable TOC before mutation, maps normalized chapter title then nearest clamped readable raw index, preserves in-chapter position, reports exact versus approximate mapping, and atomically makes the old source reversible. A monotonic `state_version` prevents slow source validation or stale A→B→A writes from overwriting newer progress; old databases gain it idempotently. Fresh 390px Playwright switched Primary → Target at the matching chapter and restored 0.40 scroll, then captured an immediate 0.72 Reader scroll, switched Target → Primary, restored 0.72, and recorded no console warnings/errors. Full backend, race, frontend tests, and warning-free production build pass. Phase 7 remains complete with its raw index-1 live evidence, deterministic WebView coverage, and catalog-load measurements recorded in the preceding history. Broad aesthetic polish remains deferred as requested; the unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Implement book-scoped bookmarks against raw chapter index plus normalized in-chapter position, with explicit orphan handling after source switches. Bounded server-side offline chapter cache follows; frontend aesthetic polish remains a dedicated later pass.
