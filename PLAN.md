...
**In progress:** A dependency-free frontend merge module now incrementally deduplicates exact retry identities, combines same-book results across batches, preserves alternate sources through primary promotion, and mirrors backend relevance ordering. Batched SSE and legacy compatibility remain covered. SearchPage state and controls are next.

**Next action:** Add the typed batch-event client, persistent search preferences, and SearchPage lifecycle for initial search, Search more, Stop, Retry, and stale-source Restart.
