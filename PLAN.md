...
For a concise new-session checkpoint, exact Git/working-tree state, and next action, start with [`HANDOFF.md`](HANDOFF.md). This plan remains the comprehensive architecture and project-state source of truth.
...
**Next action:** Complete the remaining ownership-boundary verification gate: one clean-data-root browser workflow covering first-Administrator setup → logout/login → real source import/search → add/read → logout and private-data denial. Centralized frontend handling for revoked/expired sessions, registration, password change, ordinary-account administration, and durable reader deletion are complete. Then finish the remaining Phase 2 legacy-removal checks in `docs/USER_STORAGE_IMPLEMENTATION_TASKS.md`. Public registration remains disabled by default; no legacy reader-data migration is in scope. After this gate, resume the paused compatibility queue at focused explicit CSS/Jsoup compatibility. LC-016 remains blocked until the ownership boundary is fully verified, and LC-015 is complete.
