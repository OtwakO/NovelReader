...
**Next action:** Follow LC-003 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: add a public search-source regression for `ruleSearch.checkKeyWord`, then apply Legado's valid override rules before URL construction. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
