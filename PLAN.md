...
**Next action:** Audit LC-014 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: locate the owned cover image fetch/decode boundary, trace upstream `coverDecodeJs` byte-array semantics, and pin the active corpus script before implementation. LC-013 is dormant in the audited corpus and intentionally unimplemented until a real source enables its broad lifecycle callback contract. LC-012 remains intentionally deferred and unsupported by product decision. LC-011 is complete for the audited browser resource-sniffing contract. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
