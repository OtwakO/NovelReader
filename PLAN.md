...
**Next action:** Commit this corrected implementation plan, complete one red-green vertical slice at each approved seam without raw-specific branches, update the v12 evidence to reclassify raw 251, exact-rerun verified raws 197, 163, and 17, then run full verification and independent Standards/Spec review.
...
- **Watch out**: No hostname-based UA inference, no `title`-only getter patch, no `a[1]` special case, no raw-197-only AES shortcut, and no conversion of DNS/TLS/timeouts into parseable HTTP responses.
