...
**Next action:** Complete final Standards approval, then report. The fail-closed header parser is separate, the pre-existing broader `@put` grammar is restored with a regression, and live/corpus verification, full deterministic/race/vet/frontend/build checks, the v6 targeted rerun evidence commit, and final Spec approval are complete. Do not rewrite stale source contracts.
