...
**Atomic ownership cutover (complete):** The server has only public health/static/setup/auth/configured-recovery boundaries before identity; every Reader Data API request authenticates first, derives immutable `UserID` from context, acquires a bounded per-reader runtime/home, and constructs feature access exclusively from `reader.db`/`files/`. Reader migrations are ordered and transactional; font files use `Home.Files()`; Search/Explore/workflow and JavaScript compatibility state are per-reader while process capacity remains shared; global feature startup and `DATABASE_PATH` are removed; and equal-ID isolation/anonymous-denial regressions pass. No legacy Reader Data migration or dual-mode fallback exists.

**Next action:** Complete account administration and lifecycle on the mounted ownership boundary: registration policy/UI, password change, ordinary-account list/disable/reset/delete with durable home deletion and runtime purge, centralized frontend session-revocation handling, and one setup→login→read→logout browser workflow. Public registration remains closed. This replaces the earlier 30-day rotating design outright; no legacy migration is in scope. The accepted model uses `system.db`, plaintext self-contained per-user `reader.db` directories, separately encrypted non-exportable source credentials, one-time browser Administrator setup, stored `reader`/`admin` roles, an environment recovery token, persistent independently revocable sessions, and protected Administrator accounts. LC-016 remains blocked until this boundary exists. LC-015 is complete.
...
| Storage seam | One deep `readerstore` module owns paths, DB lifecycle, migrations, backup, export/import, and deletion coordination | Future features add owned tables/files without hardcoded paths or coupling HTTP/authentication to storage layout. |
