...
**In progress:** Phase 7's API gate is complete. `GET /api/explore/sources` and bounded strict-JSON catalog/control/page endpoints now expose opaque session/catalog IDs, empty-array diagnostics/results, safe typed failures with status mapping, and explicit unsupported-WebView capability without serializing or logging imported rules/actions/causes. A real SQLite/searcher API round trip covers JavaScript catalog refresh, result paging, exhaustion, malformed/oversized requests, and capability failure. Root Windows launchers remain available; the unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Build the accessible per-source frontend Explore flow using the stable API, then run the final cross-layer invariant, live Playwright, performance, security, and documentation gates.
