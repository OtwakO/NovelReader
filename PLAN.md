...
**Next action:** Audit LC-013 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: enumerate the actual `ruleContent.callBackJs` and source `eventListener` events, lifecycle order, active corpus scripts, and side-effect boundaries before choosing any callback interface. LC-012 is intentionally deferred and remains unsupported by product decision; its future consent, authentication, no-retry, idempotency, outcome, and cache-refresh requirements are recorded. LC-011 is complete for the audited browser resource-sniffing contract. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
