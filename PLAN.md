...
- [ ] Enrich book info with `bookInfoInit` behavior.
  ...
  - [ ] Implement shared Legado `@put:{...}` / `@get:{...}` rule variables used by raw `ruleBookInfo.init` entries; then verify regex initialization and close the parent task.
...
Audit checkpoint (2026-07-15): final/source URL continuity is complete. `bookInfoInit` and the required raw-source first/middle/last matrix are missing. TOC field semantics, TOC pagination diagnostics, content pagination, complete rule context, and declared-content fallback behavior are partial. Close them in this order: book-info initialization; centralized complete analyzer context; TOC model/field semantics; TOC pagination result contract; content correctness; API failure propagation; conformance matrix; dead compatibility fallback cleanup.
...
**Status:** blocked until every confirmed Phase 4 crawl-pipeline gap is closed and its completion gate passes.
...
- [ ] Audit Legado's Explore URL/category grammar, screen/layout metadata, pagination, rule context, result model, failure semantics, and source enablement behavior against raw sources and the local upstream reference.
- [ ] Define typed per-source Explore contracts and data flow before implementation, including category navigation, paging state, source identity, and diagnostics.
- [ ] Add deterministic raw-source fixtures covering normal HTML, JSON, XPath/Regex, POST/charset, JavaScript, and WebView exploration where those capabilities occur in real sources.
- [ ] Implement the smallest Explore domain service on the existing executor/analyzer/session boundary; do not route it through search fan-out or duplicate request/rule logic.
- [ ] Add stable API endpoints for enabled Explore sources, source-native screens/categories, paged results, and explicit partial/failure diagnostics.
- [ ] Add an accessible per-source frontend flow that preserves source-native navigation and paging without exposing rule syntax.
- [ ] Verify raw-source round trips and live per-source Explore behavior through Playwright, recording exact source identity and request/rule evidence.

Out of scope: unified cross-source feeds, recommendation ranking, cross-source deduplication, editorial metadata, and NovelReader-only source schema extensions.
...
**Phase:** Phase 4 closure — complete and verify the crawl pipeline before Phase 7 Explore/Discover; Phase 0 compatibility baseline and the production-container slice are complete.
...
**In progress:** The first `ruleBookInfo.init` TDD slice now preserves selector/XPath collections and structured JSON, JavaScript, JSoup, and `java.setContent` values before detail rules. It handles `&&` merge, `||` alternatives, context-sensitive table fragments, null failure, JSON arrays, and selections beyond 50 nodes. Full backend tests and analyzer/book race tests pass. The parent task remains open because real raw sources use shared `@put:{...}` / `@get:{...}` rule variables that the analyzer does not yet support.

**Next action:** Add failing analyzer and raw book-info tests for standard/nonstandard `@put` maps and `@get` substitution, implement them once in the shared rule engine with analyzer-local/book-session state as required by Legado, then verify regex initialization and close `bookInfoInit`.
