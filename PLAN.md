...
**In progress:** Phase 7 audit/contracts merged through PR #2 (`80c9154`). On `feat/phase7-explore-fixtures`, seven exact raw entries are hash-guarded and import preserves Legado's omitted `enabledExplore=true` default. Lenient data-only parsing now handles the pinned multiline JSON fixture plus single quotes, unquoted keys, and trailing commas without executing array values; a full raw-corpus audit accepts 371 of 373 array-shaped definitions and leaves the two syntactically broken entries as diagnostics. The unrelated local `AGENTS.md` modification remains uncommitted and untouched.

**Next action:** Add deterministic local response/request expectations and the smallest source-scoped Explore service for the URL-category slice, then execute HTML/JSONPath/XPath-Regex/POST-charset/JS/WebView pages and pagination before exposing API/frontend routes.
