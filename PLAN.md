...
**In progress:** The centralized analyzer-context slice is complete. Detail, every TOC page/item rule, URL/body JavaScript, content, and content pagination now share typed book/current/next-chapter bindings; exact next-chapter redirects stop before concatenation; mutable Legado objects persist through field evaluation; chapter context fields survive SQLite round trips. Full Go tests and analyzer/book/sourceexec/API/conformance race tests pass.

**Next action:** Close TOC model semantics: preserve update timestamps and chapter metadata, match volume/non-volume URL fallback, reversal, URL-only deduplication, VIP/pay truth values, and explicit partial-pagination diagnostics with table-driven tests before changing content fallback behavior.
