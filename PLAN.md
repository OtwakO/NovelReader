...
**Next action:** Commit the green Jsoup `ownText()` slice, then verify raw 177/5/503/288 and adjacent corpus users before full deterministic/race/vet/frontend/build checks and final review. Do not rewrite stale source contracts.
...
- **Watch out**: The compatibility path accepts only a simple root `$.*[?(@.key)]` existence predicate; richer filters remain delegated to the JSONPath library rather than receiving a partial custom evaluator.
