...
**In progress:** System-wide concurrency is bounded, observable, and covered by deterministic 400-source concurrent search and registry stress tests. Browser conformance now covers WebView search plus detail → TOC → content workflow execution; production tuning remains dependent on real deployment measurements. The worker was validated from a temporary environment using Patchright 1.59.1 and Chromium 147.0.7727.15; those runtime artifacts are not stored in the repository.

**Next action:** Tune capacity defaults from deployment measurements and validate a source whose detail/TOC/content URLs themselves carry `webView:true`.
...
- **Watch out**: The verified index 788 uses WebView for search; validate sources with WebView flags on later workflow stages before declaring full browser-stage parity.