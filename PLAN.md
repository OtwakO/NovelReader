...
**Next action:** Begin LC-010 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: verify the distinct `ruleContent.webJs` browser execution contract and its relationship to already-supported URL-option `webJs`, then pin a public content workflow regression with an injected browser transport before implementation. LC-009 is complete for the audited upstream contract. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
