...
**Next action:** Run final focused/full verification after review hardening, then complete Standards/Spec re-review and report. Full backend/race/vet, frontend tests, isolated production build, and committed v7 rerun evidence are otherwise complete. Do not rewrite stale source contracts.
...
- **Watch out**: Evaluate source headers once per workflow/session and reuse the map; do not execute unprefixed literals. Preserve root JSON property order and encoded query bytes in future compatibility changes.
