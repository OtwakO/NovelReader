...
**Next action:** Phase 1.1 in `docs/USER_STORAGE_IMPLEMENTATION_TASKS.md` is complete: startup now classifies/initializes the versioned data root before opening SQLite, refuses legacy/newer/interrupted/invalid layouts without modifying them, and requires `DATABASE_PATH` inside `DATA_DIR`. Proceed with Phase 1.2 only: typed immutable user IDs, safe reader-directory resolution, staged/idempotent creation, manifests, plaintext reader/credential databases, and bounded open/close lifecycle. Phase 1.3 cold-backup/reset handling follows. Do not expose account or existing Reader Data routes until authentication and every globally keyed store/session are converted together in Phase 2. The accepted model uses `system.db`, plaintext self-contained per-user `reader.db` directories, separately encrypted non-exportable source credentials, one-time browser Administrator setup, stored `reader`/`admin` roles, environment recovery token, 30-day rotating sessions with bounded grace, and protected Administrator accounts. LC-016 remains blocked until this boundary exists. LC-015 is complete.
