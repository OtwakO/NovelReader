...
**Next action:** Follow LC-007 in `docs/LEGADO_COMPATIBILITY_TASKS.md`: select one focused explicit CSS/Jsoup getter or selector mismatch, verify the exact current upstream contract, and add an Analyzer public-seam regression before implementation. The prior v12 production reruns, full backend deterministic/race/vet, frontend 13 tests/build, and evidence validators remain recorded; two final independent review retries were blocked by the review provider's 429 usage limit.
