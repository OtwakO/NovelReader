...
1. Reassess the [multi-provider library workstream](docs/plans/2026-09-10-multi-provider-library.md) from requirements and current code, treating its previous architecture as a second opinion. Report alternatives and tradeoffs for discussion before implementation.
2. Define the necessary shared contracts and bounded implementation steps for the confirmed core TXT delivery. Keep reparse simple and advanced controls subsequent; do not inherit the former schema split, lifecycle machinery, or foundation-first sequence without revalidation.
