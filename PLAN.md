...
**In progress:** The user approved focused fixes for all four v9 shared compatibility seams. Implementation will use public Analyzer/Explore seams and vertical TDD: Java `\uXXXX` regex normalization, multi-parent Default traversal, dotted JSON array wildcards, and root-array wildcard predicates. The Legado rule reference confirms unpositioned Default selectors return all matches and JSONPath is a supported rule mode. Fixes must remain generic and syntax-scoped; stale source contracts and URL-pattern enforcement remain unchanged.
...
**Next action:** Add failing public-seam regressions for the four v9 gaps, implement the smallest shared fixes one slice at a time, rerun affected and adjacent syntax families, then complete full verification and review.
