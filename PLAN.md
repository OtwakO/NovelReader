...
**In progress:** `/api/healthz` verifies SQLite readiness without leaking internal errors. The server now handles SIGTERM/SIGINT with a ten-second graceful shutdown and a deterministic in-flight request test, allowing container stop to flush active work and close SQLite cleanly.

**Next action:** Add pinned, rootless multi-stage app and worker images, then inspect their runtime user, paths, writable data, health, and static assets.
