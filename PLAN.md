...
**In progress:** Phase 7 is undergoing a live compatibility audit after manual testing found parse failures, result-rule failures, empty pages, and multi-book pages collapsing into one result. The approved scope is a fixed-seed, stratified 50-source sample from the hash-pinned raw 939-source compilation in a fresh isolated database, including raw indices 9, 89, and 752 plus stable-identity `笔趣阁` samples. Each engine result will be compared with direct HTTP and Playwright evidence and classified by layer. This pass is diagnostic only until its findings are reported; shared fixes require user review and captured-fixture TDD. The ordered reading workflow remains complete and verified. Unrelated manual-testing changes in `frontend/dist` and `frontend/package-lock.json` predate this audit and remain untouched.

**Next action:** Build the deterministic sample manifest, execute one first-category page per source through the raw engine, reproduce apparent failures directly with Playwright/HTTP, and report the failure distribution plus ranked shared compatibility gaps before editing production code.
