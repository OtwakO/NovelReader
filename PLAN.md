...
**In progress:** The Phase 4 audit is complete. Source/final URL continuity is evidenced and closed; the remaining seven tasks contain two missing and five partial contracts. The future Phase 7 milestone remains strict Legado-compatible, per-source Explore/Discover with no unified feed or custom source schema. No Explore production code starts until the Phase 4 completion gate passes.

**Next action:** Start the first red-green slice for `ruleBookInfo.init`: add selector/JSON and JavaScript initialization fixtures that prove analyzer content is replaced before every detail-field rule, then implement the single shared initialization step and run focused/full Go verification.
